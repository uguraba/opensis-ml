...
echo "<meta charset='UTF-8'><script src='js/".$filename."'></script>";
}
}
...
$css = trim(getCSS());
if($lang["dir"] == 'rtl'):
echo "<link rel='stylesheet' type='text/css' href='themes/".trim($css)."_rtl/".trim($css).".css?i=".rand(000, 99999)."'>";
echo "<link rel='stylesheet' type='text/css' href='styles/help_rtl.css?i=".rand(000, 99999)."'>";
else:
echo "<link rel='stylesheet' type='text/css' href='themes/".trim($css)."/".trim($css).".css?i=".rand(000, 99999)."'>";
echo "<link rel='stylesheet' type='text/css' href='styles/help.css?i=".rand(000, 99999)."'>";
endif;
      ...
      <td ><table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\">
          <tr>
            <td class=\"banner\" valign=\"top\"><table width='100%' border='0' cellspacing='0' cellpadding='0'>
                <tr>
                  <td align=\"left\">&nbsp;</td>
                  <td align=\"right\"><div class=\"user_info\">".date('l F j, Y')." &nbsp;&nbsp;|&nbsp;&nbsp;".User('NAME')." &nbsp;&nbsp;|&nbsp;&nbsp; <a href='index.php?modfunc=logout' class='logout'>"._('Log Out')."</a>";
/****************************************************************************************/
if(User('PROFILE')=='teacher')
{  
	echo "<br />
	<table cellspacing=\"0\" style=\"padding-top:6px;\"><tr><td><FORM name=head_frm id=head_frm action=Side.php?modfunc=update&btnn=$btn&nsc=$ns&act=school method=POST><INPUT type=hidden name=modcat value='' id=modcat_input>";
...
echo '<tr><td style="color:#fff;">'._('Subject').'</td><td></td>
          <td style="color:#fff">'._('Course').'</td><td></td>
		  <td style="color:#fff">'._('Course Period').'</td></tr>';
...
require('Menu.php');
echo "<li><a style='cursor:hand;' href='#' onmouseup='check_content(\"ajax.php?modname=misc/Portal.php\");' target=body onmousedown='document.getElementById(\"header\").innerHTML = \""._('Home')."\";document.getElementById(\"cframe\").src = \"Bottom.php?modcat=home\"'><span>" .""._('Home')."". "</span></a></li>";
foreach($_openSIS['Menu'] as $modcat=>$programs)
{
	if(count($_openSIS['Menu'][$modcat]))
	{
		...
		{
			echo "<li><a style='cursor:hand;' HREF=# onmouseup='check_content(\"ajax.php?modname=$modcat/Search.php\");' target=body onmousedown='document.getElementById(\"header\").innerHTML = \""._(str_replace('_',' ',$modcat))."\";document.getElementById(\"cframe\").src = \"Bottom.php?modcat=".$modcat."\"'><span>"._('School Info')."</span></a></li>";
		}
		elseif(User('PROFILE')!='admin' && $modcat=='Users')
		{
			echo "<li><a style='cursor:hand;' HREF=# onmouseup='check_content(\"ajax.php?modname=$modcat/Search.php\");' target=body onmousedown='document.getElementById(\"header\").innerHTML = \""._(str_replace('_',' ',$modcat))."\";document.getElementById(\"cframe\").src = \"Bottom.php?modcat=".$modcat."\"'><span>"._('My Info')."</span></a></li>";
		}
		...
		{
			echo "<li><a style='cursor:hand;' HREF=# onmouseup='check_content(\"ajax.php?modname=$modcat/Search.php\");' target=body onmousedown='document.getElementById(\"header\").innerHTML = \""._(str_replace('_',' ',$modcat))."\";document.getElementById(\"cframe\").src = \"Bottom.php?modcat=".$modcat."\"'><span>"._('Schedule')."</span></a></li>";
		}
		else
		{
			echo "<li><a style='cursor:hand;' HREF=# onmouseup='check_content(\"ajax.php?modname=$modcat/Search.php\");' target=body onmousedown='document.getElementById(\"header\").innerHTML = \""._(str_replace('_',' ',$modcat))."\";document.getElementById(\"cframe\").src = \"Bottom.php?modcat=".$modcat."\"'><span>"._(str_replace('_',' ',$modcat))."</span></a></li>";
		}
...
<table cellspacing=0 celpadding=0 border=0 width=100%><tr><td width=60% valign=middle class='welcome'><b>"._('Welcome to openSIS Student Information System')."</b></td><td width=40% class='version'>"._('Version')." : ".$openSISVersion." | "._('Release Date')." : ".$builddate."</td></tr></table> 
...
echo "<div id='submenu_".$i."' style='display:none'>"; 
		...
		foreach($keys as $key_index=>$file)
		{
			...
			if(substr($file,0,7)=='http://')
				echo "<A ".$style." HREF=$file target=body >"._($title)."</A> &nbsp;&nbsp;|&nbsp;&nbsp;";
			elseif(substr($file,0,7)=='HTTP://')
				echo "<A ".$style." HREF=$file target=_blank>"._($title)."</A> &nbsp;&nbsp;|&nbsp;&nbsp;";
			elseif(!is_numeric($file))
				if(User('PROFILE')=='student' && $title=="Student Info"){
				echo "<A ".$style." id=hm HREF=# onClick='check_content(\"ajax.php?modname=".$file." \");' target=body onmousedown='document.getElementById(\"header\").innerHTML = \""._(str_replace('_',' ',$modcat))." >> ".""._($title)."\"' onmouseup=\"changeColors(); this.className='submenu_link'; document.getElementById('cframe').src='Bottom.php?modname=".$file."';\">"._('My Information')."</A> &nbsp;&nbsp;|&nbsp;&nbsp;";	
				}
				elseif(User('PROFILE')=='student' && $title=="Schedule"){
				echo "<A ".$style." id=hm HREF=# onClick='check_content(\"ajax.php?modname=".$file." \");' target=body onmousedown='document.getElementById(\"header\").innerHTML = \""._(str_replace('_',' ',$modcat))." >> ".""._($title)."\"' onmouseup=\"changeColors(); this.className='submenu_link'; document.getElementById('cframe').src='Bottom.php?modname=".$file."';\">"._('My Schedule')."</A> &nbsp;&nbsp;|&nbsp;&nbsp;";	
				}				
				elseif(User('PROFILE')=='student' && $title=="Student Requests"){
				echo "<A ".$style." id=hm HREF=# onClick='check_content(\"ajax.php?modname=".$file." \");' target=body onmousedown='document.getElementById(\"header\").innerHTML = \""._(str_replace('_',' ',$modcat))." >> ".""._($title)."\"' onmouseup=\"changeColors(); this.className='submenu_link'; document.getElementById('cframe').src='Bottom.php?modname=".$file."';\">"._('My Requests')."</A> &nbsp;&nbsp;|&nbsp;&nbsp;";	
				}
				else
				{
				echo "<A ".$style." id=hm HREF=# onClick='check_content(\"ajax.php?modname=".$file." \");' target=body onmousedown='document.getElementById(\"header\").innerHTML = \""._(str_replace('_',' ',$modcat))." >> ".""._($title)."\"' onmouseup=\"changeColors(); this.className='submenu_link'; document.getElementById('cframe').src='Bottom.php?modname=".str_replace('&', '?',$file)."';\">"._($title)."</A> &nbsp;&nbsp;|&nbsp;&nbsp;";
				}
			elseif($keys[$key_index+1] && !is_numeric($keys[$key_index+1]))
			{
			 $mm=$mm+1;
			echo '<label class="dd_menuitem" id="mm_'.$modcat.'_'.$mm.'"><b>'._($title).'</b>&nbsp;<img src="themes/'.trim($css).'/mnu_drpdwn.gif" />&nbsp;&nbsp;|&nbsp;&nbsp;</label>&nbsp;'.'<div id="menu_child_'.$modcat.'_'.$mm.'" style="position: absolute; visibility: hidden; width:200px;">';
			}
			...
			$menumm = $mm; 
			if(substr($file,0,7)=='http://')
				echo "<A id=dd class='dd_submenuitem' HREF=$file target=body >"._($title)."</A>";
			elseif(substr($file,0,7)=='HTTP://')
				echo "<A id=dd class='dd_submenuitem' HREF=$file target=_blank>"._($title)."</A>";
			elseif(!is_numeric($file))
				echo "<A id=dd class='dd_submenuitem' HREF=# onClick='check_content(\"ajax.php?modname=".$file." \");' target=body onmousedown='document.getElementById(\"header\").innerHTML = \""._(str_replace('_',' ',$modcat))." >> "."$title\"' onmouseup=\"document.getElementById('cframe').src='Bottom.php?modname=".$file."';\">"._($title)."</A>";
			elseif($keys[$key_index+1] && !is_numeric($keys[$key_index+1]))
			{
			 $mm=$mm+1;
			echo '</div><label class="dd_menuitem" id="mm_'.$modcat.'_'.$mm.'"><b>'._($title).'</b>&nbsp;<img src="themes/'.trim($css).'/mnu_drpdwn.gif" />&nbsp;&nbsp;|&nbsp;&nbsp;</label>'.'<div id="menu_child_'.$modcat.'_'.$mm.'" style="position: absolute; visibility: hidden; width=200px;">';
			}
...
echo '<div id="showhelp" style="padding-top:33px;"><a href="javascript:void(0);" onclick="inter=setInterval(\'ShowBox(helpdiv, 380, 499, 499, 211, showhelp)\',1);return false;"><b>'._('Help').'</b></a></div></td></tr></table>';
...
<div class="help_div">
<table width=100% border=0 cellspacing=0 cellpadding=0><tr><td align=right style=" height:25px;  padding-right:5px;"><a href="javascript:void(0);" onclick="inter=setInterval(\'HideBox(helpdiv, showhelp)\',1);return false;"><b>'._('Hide Help').'</b></a></td></tr></table>
</div>
...
{       /*******************back to list****************************/
	...
	else
	{
		if(User('USERNAME'))
		{
			echo ""._('You are not allowed to use this program! This attempted violation has been logged and your IP address was captured.')."";
			Warehouse('footer');
