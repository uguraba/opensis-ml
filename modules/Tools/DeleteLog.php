...
if(isset($_REQUEST['del']))	
{
        ...
        if(isset($conv_st_date) && isset($conv_end_date))
	{
		...
		echo '<center><font color="red"><b>'._('Log deleted successfully').'</b></font></center>';
	}
	...
	if(!isset($conv_st_date) && !isset($conv_end_date))
	{
		echo '<center><font color="red"><b>'._('You have to select atleast one date from the date range').'</b></font></center>';
	...
	echo "<br><FORM name=del id=del action=Modules.php?modname=$_REQUEST[modname] method=POST>";
		PopTable ('header', ''._('Delete Log').'');
		
	echo '<div align=center style="padding-top:10px; font-size:12px;"><strong>'._('Please Select Date Range').'</strong></div>
	...
	echo '</TD><TD valign=middle style=padding-top:25px;><strong>'._('To').' :</strong> </TD><TD valign=middle>';
	DrawHeader(PrepareDate($end_date,'_end'));

		
	echo '</TD></TR></TABLE><div style=height:10px></div>';
			
			echo '<center><input type="submit" class=btn_medium value="'._('Delete').'" name="del"></center>';
