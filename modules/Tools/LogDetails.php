...
if(!$_REQUEST['modfunc'])
{
	$start_date = '01-'.strtoupper(date('M-y'));
        ...
        PopTable('header',_('Log Details'));
	echo '<div align=center style="padding-top:20px; font-size:12px;"><strong>'._('Please Select Date Range').'</strong></div>
	<TABLE border=0 width=100% align=center><tr><TD valign=top style=padding-top:14px;>';
	echo '<strong>'._('From').' :</strong> </TD><TD valign=middle>';
	DrawHeader(PrepareDate($start_date,'_start'));
	echo '</TD><TD valign=top style=padding-top:14px;><strong>'._('To').' :</strong> </TD><TD valign=middle>';
	DrawHeader(PrepareDate($end_date,'_end'));
	echo '</TD></TR></TABLE><div style=height:10px></div>';
	echo '<center><input type="submit" class=btn_medium value="'._('Generate').'" name="generate"></center>';
	PopTable('footer');
	echo '</FORM>';
}
	...
	#$alllogs_RET = DBGet(DBQuery("SELECT DISTINCT FIRST_NAME,LAST_NAME,LOGIN_TIME,PROFILE,FAILLOG_COUNT,FAILLOG_TIME,USER_NAME,IP_ADDRESS,STATUS FROM login_records WHERE LOGIN_TIME >='".$conv_st_date."' AND LOGIN_TIME <='".$conv_end_date."' AND SCHOOL_ID=".UserSchool()." OR STATUS='Failed'  ORDER BY LOGIN_TIME DESC"));
		...
		if(count($alllogs_RET))
		{
			echo '<div>';
                        ...
                        ListOutput($alllogs_RET,array('LOGIN_TIME'=>_('Login Time'),'USER_NAME'=>_('User Name'),'FIRST_NAME'=>_('First Name'),'LAST_NAME'=>_('Last Name'),'PROFILE'=>_('Profile'),'FAILLOG_COUNT'=>_('Failure Count'),'STATUS'=>_('Status'),'IP_ADDRESS'=>_('IP Address')),_('login record'),_('login records'),array(),array(),array('count'=>true,'save'=>true));
		...
		echo '<table border=0 width=90%><tr><td class="alert"></td><td class="alert_msg"><b>'._('No login records were found.').'</b></td></tr></table>';
		
		}
}
if((!isset($conv_st_date) || !isset($conv_end_date)))
	{
		echo '<center><font color="red"><b>'._('You have to select date from the date range').'</b></font></center>';
