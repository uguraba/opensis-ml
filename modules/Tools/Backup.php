...
function _mysql_test($mysql_host,$mysql_database, $mysql_username, $mysql_password)
{
	...
	if (!$link)
	{
	   array_push($output_messages, ''._('Could not connect').': ' . mysql_error());
	}
	else
	{
		array_push ($output_messages,""._('Connected with MySQL server').":$mysql_username@$mysql_host "._('successfully')."");
		$db_selected = mysql_select_db($mysql_database, $link);
		if (!$db_selected)
		{
			array_push ($output_messages,''._('Can\'t use $mysql_database').' : ' . mysql_error());
		}
		else
			array_push ($output_messages,""._('Connected with MySQL database').":$mysql_database "._('successfully')."");
	}
}
...
echo '<a href=Modules.php?modname='.$modname.'&modfunc=cancel STYLE="TEXT-DECORATION: NONE"> <INPUT type=button class=btn_medium name=Cancel value='._('Cancel').'></a>';
?>
</td></tr></table>
</form>
