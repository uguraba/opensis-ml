...
echo '<table width="80%" cellpadding="6" cellspacing="6"><tr><td width="50%" valign="top"><div id="calculating" style="display: none; padding-top:60px;" align="center"><img src="assets/rollover_anim.gif" /><br/><br/><strong>'._('School year rolling over, please wait...').'</strong></div><div id="response" style="font-size:14px"></div></td>';
...
//    if(trim($_REQUEST['month_roll_school_start_date'])!='')
//    {
...
$table_list .= '</TABLE>';
    //===============================
    if(Prompt_rollover(''._('Confirm Rollover').'',''._('Are you sure you want to roll the data for ').''.$cur_session.''._(' to the next school year?').'',$table_list))
{
       echo "<script type='text/javascript'>ajax_rollover('staff');</script>";
        ...
        Prompt_rollover_back(''._('Rollover Completed').'',''._('Data has been rolledover for ').''.$cur_session.''._(' for ').''.  GetSchool(UserSchool()).'');
}
