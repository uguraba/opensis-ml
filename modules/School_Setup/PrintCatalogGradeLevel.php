...
##############################################List Output Generation##################################################
		...
		echo '<table width=100%><tr><td align=center><font color=red face=verdana size=2><strong>'._('No Courses were found in this Grade Level').'</strong></font></td></tr></table>';
		
		
	}
	else
	{
	PopTable('header',''._('Print')._(' ')._('Catalog')._(' ')._('by')._(' ')._('Grade Level').'','width=700');
	...
	echo "<FORM id='search' name='search' method=POST action=Modules.php?modname=$_REQUEST[modname]>";
                ...
                {
                        echo CreateSelect($grade_level_RET, 'id', ''._('All').'', ''._('Select ').''._('Grade Level').':', 'Modules.php?modname='.$_REQUEST['modname'].'&id=');
                }	
	echo '</td></tr></table>';
	if(clean_param($_REQUEST['id'],PARAM_ALPHANUM))
	{
                    $grade_level_RET=  DBGet(DBQuery('SELECT TITLE FROM school_gradelevels WHERE id=\''.$_REQUEST['id'].'\''));
                    $grade_title = $grade_level_RET[1]['TITLE'];
                    DrawHeader('<div align="center">'._('Report generated for ').''._($grade_title ).' '._('Grade Level').'</div>');
	}
	else
	DrawHeader('<div align="center">'._('Report generated for all ').''._('Grade Levels').'</div>');	
	echo '</form>';
