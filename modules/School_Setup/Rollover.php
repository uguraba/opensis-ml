...
$table_list .= '<tr><td colspan=3>* '._("You").' <i>'._("must").'</i>'._(" roll users, school periods, marking periods, calendars, and report card").'<br>'._("codes at the same time or before rolling courses").'<BR><BR>* '._("You").' <i>'._("must").'</i>'._(" roll courses at the same time or before rolling report card comments").'<BR><BR>'._("Red items have already have data in the next school year (They might have been rolled).").'<BR><BR>'._("Rolling red items will delete already existing data in the next school year.").'</td></tr>';
...
if(Prompt_rollover(_('Confirm Rollover'),_('Are you sure you want to roll the data for ').UserSyear().'-'.(UserSyear()+1)._( 'to the next school year?'),$table_list))
{
	...
	DrawHeaderHome('<IMG SRC=assets/check.gif>The data have been rolled.','<input type=button onclick=document.location.href="index.php?modfunc=logout" value='._("Please login again").' class=btn_large >');
