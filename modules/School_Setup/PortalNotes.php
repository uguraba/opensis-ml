...
if(clean_param($_REQUEST['values'],PARAM_NOTAGS) && ($_POST['values'] || $_REQUEST['ajax']) && AllowEdit() && !$_REQUEST['portal_search'])
{
	...
	{ 
		...
		if($id!='new')
		{
                                                    ...
                                                    {
                                                        ShowErrPhp(''._('Data not saved because  date range is not valid').'');
                                                    }
                                                    else
                                                    {
...
if($_REQUEST['modfunc']!='remove')
{
	...
	ListOutput($notes_RET,$columns,''._('Note').'',''._('Notes').'',$link);
	echo '<br><CENTER>'.SubmitButton(_('Save'),'','class=btn_medium onclick="formcheck_school_setup_portalnotes();"').'</CENTER>';
	echo '</FORM>';
}
...
{	global $THIS_RET,$profiles_RET;
	...
	$return = '<TABLE width=216><TR><TD class=LO_field align="center"><b>'._('Visible').''._(' ').''._('Between').':</b></TD></tr><tr><TD align="center">';

	$return = '<TABLE width=216><TR><TD class=LO_field align="center"><b>'._('Visible').''._(' ').''._('Between').':</b></TD></tr><tr><TD align="center">';
	$return .= DateInput($value,"values[$id][$name]").'</TD></tr><tr><TD class=LO_field align="center"><b>'._('&').'</b></TD></tr><tr><TD align="center">';
	...
	foreach(array('all'=>''._('All School').'','admin'=>''._('Administrator w/Custom').'','teacher'=>''._('Teacher').''.''._(' w/Custom').'','parent'=>''._('Parent w/Custom').'') as $profile_id=>$profile)
	...
	$i = 3;	
//$return .= '<TABLE border=0 cellspacing=0 cellpadding=0 width=96% class=LO_field><TR><TD colspan=4><b>'._('Visible').' '._('To').': </b></TD></TR>';
	//foreach(array('all'=>''._('All School').'','admin'=>''._('Administrator w/Custom').'','teacher'=>''._('Teacher w/Custom').'','parent'=>''._('Parent w/Custom')).'' as $profile_id=>$profile)
	//	$return .= "<tr><TD colspan=4><INPUT type=checkbox name=profiles[$id][$profile_id] value=Y".(strpos($THIS_RET['PUBLISHED_PROFILES'],",$profile_id,")!==false?' CHECKED':'').">"._( $profile)." </TD></tr>";
	//$i = 3;
	foreach($profiles_RET as $profile)
	{
		$i++;
		$return .= '<tr><TD colspan=4><INPUT type=checkbox name=profiles['.$id.']['.$profile['ID'].'] value=Y'.(strpos($THIS_RET['PUBLISHED_PROFILES'],",$profile[ID],")!==false?' CHECKED':'').">"._($profile[TITLE])."</TD></tr>";
