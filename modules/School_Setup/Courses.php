...
if(clean_param($_REQUEST['course_modfunc'],PARAM_ALPHAMOD)=='search')
{
	...
	echo '<TABLE><TR><TD><INPUT type=text class=cell_floating name=search_term value="'.$_REQUEST['search_term'].'"></TD><TD><INPUT type=submit class=btn_medium value='._('Search').' onclick=\'formload_ajax("F1")\';></TD></TR></TABLE>';
	echo '</FORM>';
	PopTable('footer');
	...
	if($_REQUEST['search_term'])
	{
		...
		ListOutput($subjects_RET,array('TITLE'=>'Subject'),_('Subject'),_('Subjects'),$link,array(),array('search'=>false,'save'=>false));
		echo '</TD><TD valign=top>';
		$link['TITLE']['link'] = "Modules.php?modname=$_REQUEST[modname]&modfunc=$_REQUEST[modfunc]";
		$link['TITLE']['variables'] = array('subject_id'=>'SUBJECT_ID','course_id'=>'COURSE_ID');
		ListOutput($courses_RET,array('TITLE'=>'Course'),_('Course'),_('Courses'),$link,array(),array('search'=>false,'save'=>false));
                echo '</TD><TD valign=top>';
		$link['TITLE']['link'] = "Modules.php?modname=$_REQUEST[modname]&modfunc=$_REQUEST[modfunc]";
		$link['TITLE']['variables'] = array('subject_id'=>'SUBJECT_ID','course_id'=>'COURSE_ID','course_period_id'=>'COURSE_PERIOD_ID');
		ListOutput($periods_RET,array('TITLE'=>'Course Period'),_('Course Period'),_('Course Periods'),$link,array(),array('search'=>false,'save'=>false));
		echo '</TD></TR></TABLE>';
	}
}
		...
		foreach($tables as $id=>$columns)
		{
			...
			if($id!='new')
			{
				...
				if($table_name=='course_periods')
				{
					...
					$current = DBGet(DBQuery('SELECT TEACHER_ID,PERIOD_ID,MARKING_PERIOD_ID,DAYS,SHORT_NAME,TOTAL_SEATS,FILLED_SEATS FROM course_periods WHERE '.$where[$table_name].'=\''.$id.'\''));
                                                                    ...
                                                                    if(!(isset($columns['TITLE']) && trim($columns['TITLE'])==''))
                                                                    {
                                                                        foreach($columns as $column=>$value)
                                                                        {
                                                                                if(($column=='TEACHER_ID') && $scheduleAssociation){
                                                                                    $asso_err[]=''._('To Change teacher go to School Setup->Courses->Teacher Re-Assignment').'';
                                                                                    break 2;
                                                                                }
                                                                                if(($column=='PERIOD_ID') && $scheduleAssociation){
                                                                                    $asso_err[]=''._('You can not edit Period because students are scheduled in it').'';
                                                                                    break 2;
                                                                                }
                                                                                elseif($column=='DAYS' && $scheduleAssociation){
                                                                                    $asso_err[]=''._('You can not edit Days because students are scheduled in it').'';
                                                                                    break 2;
                                                                                }
                                                                                elseif($column=='MARKING_PERIOD_ID' && $scheduleAssociation){
                                                                                    $asso_err[]=''._('You can not edit Marking Period because students are scheduled in it').'';
                                                                                    break 2;
                                                                                }
                                                                                elseif($column=='GRADE_SCALE_ID')
                                                                                {
                                                                                    ...
                                                                                    {
                                                                                        $asso_err[]=''._('You can not edit Grading Scale because final grading was done').'';
                                                                                        break 2;
                                                                                    }
                                                                                ...
                                                                                elseif($column=='CREDITS' && $gradeAssociation){
                                                                                    $asso_err[]=''._('You can not edit Credits because final grading was done').'';
                                                                                    break 2;
                                                                                }
...
//                                                                                }
                                                                                elseif (isset ($columns['DOES_ATTENDANCE']) && $columns['DOES_ATTENDANCE']=='') 
                                                                                {
                                                                                    ...
                                                                                    if($attn[1]['TOTAL']>0)
                                                                                    {
                                                                                        $asso_err[]=''._('You can not turn off take attendance as attendance has been taken for this course period.').'';
                                                                                        break 2;
                                                                                    }
                                                                                    else
                                                                                    {
                                                                                ...
                                                                                elseif($column=='TOTAL_SEATS' && isset($cur_total_seat) && $value<$cur_filled_seat){
                                                                                    $asso_err[]=''._('You can not reduce Seats because students are scheduled in it').'';
                                                                                    break 2;
                                                                                }
...
if(clean_param($_REQUEST['modfunc'],PARAM_ALPHAMOD)=='delete' && AllowEdit())
{
	...
	elseif(clean_param($_REQUEST['course_id'],PARAM_ALPHANUM))
	{
		...
		{	//echo 'hii'exit;
			...
			if ($course1['COURSE_PERIOD_ID']!='') {
				PopTable('header',''._('Unable to Delete').'');
					DrawHeaderHome('<font color=red>'._('Course cannot be deleted').'.</font>');
					echo '<div align=right><a href=Modules.php?modname=School_Setup/Courses.php&subject_id='.$subject_id.'&course_id='.$course_id.' style="text-decoration:none"><b>'._('back to Course').'</b></a></div>';
					PopTable('footer');
	...
	elseif(clean_param($_REQUEST['subject_id'],PARAM_ALPHANUM))
	{
		...
		foreach($subject as $subject1) 
			...
			if($subject1['COURSE_ID']!='') {
				PopTable('header',''._('Unable to Delete').'');
					DrawHeaderHome('<font color=red>'._('Subject cannot be deleted.').'</font>');
					echo '<div align=right><a href=Modules.php?modname=School_Setup/Courses.php&subject_id='.$subject_id.' style="text-decoration:none"><b>'._('back to Subject').'</b></a></div>';
					PopTable('footer');
...
if((!$_REQUEST['modfunc'] || clean_param($_REQUEST['modfunc'],PARAM_ALPHAMOD)=='choose_course') && !$_REQUEST['course_modfunc'])
{
	...
	{
		if(AllowEdit())
			$delete_button = "<INPUT type=button class=btn_medium value="._('Delete')." onClick='javascript:window.location=\"Modules.php?modname=$_REQUEST[modname]&modfunc=delete&subject_id=$_REQUEST[subject_id]&course_id=$_REQUEST[course_id]&course_period_id=$_REQUEST[course_period_id]\"'> ";
		// ADDING & EDITING FORM
		if(clean_param($_REQUEST['course_period_id'],PARAM_ALPHANUM))
		{
			if($_REQUEST['course_period_id']!='new')
			...
			else
			{
				...
				$QI = DBQuery($sql);
				$RET = DBGet($QI);
				$title = $RET[1]['TITLE']._(' - ')._('New Period');
				unset($delete_button);
				unset($RET);
				$checked = 'CHECKED';
				$new = true;
			...
			echo "<FORM name=F2 id=F2 action=Modules.php?modname=$_REQUEST[modname]&subject_id=$_REQUEST[subject_id]&course_id=$_REQUEST[course_id]&course_period_id=$_REQUEST[course_period_id] method=POST>";
                ...
                DrawHeaderHome($title,$delete_button.SubmitButton(_('Save'),'','class=btn_medium onclick="return formcheck_scheduling_course_F2();"'));
			
			$header .= '<TABLE cellpadding=3 width=760 >';
			$header .= '<TR>';

			$header .= '<TD>' . TextInput($RET['SHORT_NAME'],'tables[course_periods]['.$_REQUEST['course_period_id'].'][SHORT_NAME]',''._('Short Name').'','class=cell_floating') . '</TD>';
			...
			$header .= '<TD>' . SelectInput($RET['TEACHER_ID'],'tables[course_periods]['.$_REQUEST['course_period_id'].'][TEACHER_ID]',''._('Teacher').'',$teachers) . '</TD>';
			
			$header .= '<TD>' . SelectInput($RET['SECONDARY_TEACHER_ID'],'tables[course_periods]['.$_REQUEST['course_period_id'].'][SECONDARY_TEACHER_ID]',''._('Secondary Teacher').'',$teachers) . '</TD>';
			$header .= '<TD></TD>';
                        $header .= '</TR><TR>';
                        
			$header .= '<TD>' . TextInput($RET['ROOM'],'tables[course_periods]['.$_REQUEST['course_period_id'].'][ROOM]',''._('Room').'','class=cell_floating') . '</TD>';
			...
			$header .= '<TD>' . SelectInput($RET['PERIOD_ID'],'tables[course_periods]['.$_REQUEST['course_period_id'].'][PERIOD_ID]',''._('Period').'',$periods,'N/A','id="cp_period" onchange="formcheck_periods_F2();"') . '</TD>';
			...
			$header .= '<TD>' . SelectInput($RET['MARKING_PERIOD_ID'],'tables[course_periods]['.$_REQUEST['course_period_id'].'][MARKING_PERIOD_ID]',''._('Marking Period').'',$options) . '</TD>';

                        $header .= '<TD><TABLE cellpadding=0 cellspacing=0><TR>';
                        $header .= '<TD>' . TextInput($RET['TOTAL_SEATS'],'tables[course_periods]['.$_REQUEST['course_period_id'].'][TOTAL_SEATS]',''._('Seats').'','size=4 class=cell_floating') . '</TD>';
			if($_REQUEST['course_period_id']!='new')
			$header .= '<TD style=padding-left:10px;><FONT color=green>' .$available_seats. '</FONT><BR><FONT color=gray><SMALL>'._('Available Seats').'</SMALL></FONT></TD>';
                        $header .= '</TR></TABLE>';
                        $header .= '</TD>';
...
//			$header .= '<TD>' . SelectInput($RET['GRADE_SCALE_ID'],'tables[course_periods]['.$_REQUEST['course_period_id'].'][GRADE_SCALE_ID]','Grading Scale',$options,'Not Graded') . '</TD>';
                        ...
                        $header .= '<TD>' . SelectInput($RET['GRADE_SCALE_ID'],'tables[course_periods]['.$_REQUEST['course_period_id'].'][GRADE_SCALE_ID]',''._('Grading Scale').'',$options,false,'id=def_grade onClick=show_this_msg("def_grade","cal_msg","grade");') . '</TD>';
                        else
			...
			foreach($options_RET as $option)
				$options[$option['CALENDAR_ID']] = $option['TITLE'];
                        ...
                        $header .= '<TD>' . SelectInput($RET['CALENDAR_ID'],'tables[course_periods]['.$_REQUEST['course_period_id'].'][CALENDAR_ID]',''._('Calendar').'',$options,false,'id=def_cal onClick=show_this_msg("def_cal","cal_msg","calendar");') . '</TD>';
                        else
         ...
         //		--------------------------------------------- Temp Coment ------------------------------------------------- 	//
		     ...
		     # $header .= "<TD colspan=2><DIV id=course_div>".$parent."</DIV> ".($parent!='N/A'?"<A HREF=# onclick='window.open(\"for_window.php?modname=".$_REQUEST['modname']."&modfunc=choose_course\",\"\",\"scrollbars=yes,resizable=yes,width=800,height=400\");'>Choose</A><BR>":'')."<small><FONT color=".Preferences('TITLES').">Parent Course Period</FONT></small></TD>";
                if($_REQUEST['course_period_id']!='new' && $RET['PARENT_ID']!=$_REQUEST['course_period_id']){
                    $header .= "<TD colspan=2><DIV id=course_div>".$parent."</DIV> ".($parent!='N/A' && AllowEdit()?"<A HREF=# onclick='window.open(\"for_window.php?modname=misc/ChooseParentCourse.php\",\"\",\"scrollbars=yes,resizable=yes,width=800,height=400\");'>"._('Choose')."</A>"."&nbsp;&nbsp;"."<INPUT type=checkbox name='tables[course_periods][".$_REQUEST['course_period_id']."][PARENT_ID]' value='".$_REQUEST['course_period_id']."' >&nbsp;"._('Remove')."" ."<BR>":'')."<small><FONT color=".Preferences('TITLES').">"._('Parent Course Period')."</FONT></small></TD>";
                }else{
                    $header .= "<TD colspan=2><DIV id=course_div>".$parent."</DIV> ".($parent!='N/A' && AllowEdit()?"<A HREF=# onclick='window.open(\"for_window.php?modname=misc/ChooseParentCourse.php\",\"\",\"scrollbars=yes,resizable=yes,width=800,height=400\");'>"._('Choose')."</A><BR>":'')."<small><FONT color=".Preferences('TITLES').">"._('Parent Course Period')."</FONT></small></TD>";
                }
		...
		elseif(clean_param($_REQUEST['course_id'],PARAM_ALPHANUM))
		{
			...
			$header .= '<TD>' . TextInput($RET['TITLE'],'tables[courses]['.$_REQUEST['course_id'].'][TITLE]',''._('Title').'','class=cell_mod_wide') . '</TD>';
			$header .= '<TD>' . TextInput($RET['SHORT_NAME'],'tables[courses]['.$_REQUEST['course_id'].'][SHORT_NAME]',''._('Short Name').'','class=cell_floating') . '</TD>';
                                                      
                                                      $header .= '<TD>' . SelectInput($RET['GRADE_LEVEL'],'tables[courses]['.$_REQUEST['course_id'].'][GRADE_LEVEL]',''._('Grade Level').'',$grade_levels) . '</TD>';
			if($_REQUEST['modfunc']!='choose_course')
			{
		...
		elseif(clean_param($_REQUEST['subject_id'],PARAM_ALPHANUM))
		{
			...
			$header .= '<TD>' . TextInput($RET['TITLE'],'tables[course_subjects]['.$_REQUEST['subject_id'].'][TITLE]',_('Title'),'class=cell_wide') . '</TD>';
	...
	if(!$_REQUEST['subject_id'] || clean_param($_REQUEST['modfunc'],PARAM_ALPHAMOD)=='choose_course')
		#DrawHeader('Courses');
	DrawHeaderHome(''._('Courses').'',"<A HREF=Modules.php?modname=$_REQUEST[modname]&modfunc=$_REQUEST[modfunc]&course_modfunc=search>"._('Search')."</A>");
	...
	ListOutput($subjects_RET,$columns,_('Subject'),_('Subjects'),$link,array(),$LO_options);
	echo '</TD>';
	...
	if(clean_param($_REQUEST['subject_id'],PARAM_ALPHANUM) && $_REQUEST['subject_id']!='new')
	{
		...
		echo '<TD valign=top>';
		$columns = array('GRADE_COURSE'=>_('Course'));
		$link = array();
		...
		if(clean_param($_REQUEST['course_id'],PARAM_ALPHANUM) && $_REQUEST['course_id']!='new')
		{
                ...
                ListOutput($periods_RET,$columns,_('Period'),_('Periods'),$link,array(),$LO_options);
                echo '</TD>';
