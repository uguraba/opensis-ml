            ...
            else
            {
                ...
                else 
                {
                    ...
                    else 
                    {
                        ...
                        {
                            $err_msg=''._("Sort order already exists").'';
                            break;
                        }
                        else 
                        {
...
if(clean_param($_REQUEST['modfunc'],PARAM_ALPHAMOD)=='remove')
{
	...
	if($has_assigned>0){
	UnableDeletePrompt(_('Cannot delete because grade levels are associated.'));
	}else{
...
if($_REQUEST['modfunc']!='remove')
{
	...
	ListOutput($grades_RET,$columns,_('Grade Level'),_('Grade Levels'),$link, true, array('search'=>false));
	echo '<br><CENTER><INPUT class="btn_medium" type=submit value='._("Save").' onclick="formcheck_school_setup_grade_levels();"></CENTER>';
	echo '</FORM>';
}
