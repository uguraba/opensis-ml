...
{
    DrawBC(""._('Courses')." -> ".$_REQUEST['draw_header']);
    ...
    $RET = DBGet($QI);
    $RET = $RET[1];
    $title = $RET['TITLE']." , <b>"._('Course').":</b> ".$C_RET[1]['TITLE'].", <b>"._('Number of Credits').":</b> ".$C_RET[1]['NUMBER_OF_CREDITS'];
    $new = false;
    //print_r($RET);
    if(count($RET))
    {
        ...
        $header .= '<TD><b>' .$RET['SHORT_NAME'] . '</b><br>'._('Short Name').'</TD>';
        ...
        $header .= '<TD><b>' . $teachers_RET[1]['TEACHER'] . '</b><br>'._('Teacher').'</TD>';
        $header .= '<TD><b>' . $RET['ROOM'] . '</b><br>'._('Location').'</TD>';
        $sql = "SELECT TITLE,START_TIME,END_TIME FROM school_periods WHERE PERIOD_ID='".$RET['PERIOD_ID']."'";
        $periods_RET = DBGet(DBQuery($sql));
        $header .= '<TD><b>' . $periods_RET[1]['TITLE'] . '</b><br>'._('Period').'</TD>';
        $header .= '<TD><b>'. $RET['DAYS'] .'</b><br>Days</td>';
        $header .= '<TD><b>' . $periods_RET[1]['TITLE'] . '</b><br>'._('Period').'</TD>';
        $header .= '<TD><b>'. $RET['DAYS'] .'</b><br>'._('Days').'</td>';
        $header .= '</TR><TR>';
        ...
        $header .= '<TD><b>' . $mp_RET[1]['TITLE']. '</b><br>'._('Marking Period').'</TD>';
        $header .= '<TD><b>' . $RET['TOTAL_SEATS'] . '</b><br>'._('Total Seats').'</TD>';

        $header .= '<TD><b>' . gr($RET['GENDER_RESTRICTION']).'</b><br>'._('Gender Restriction').'' . '</TD>';
        ...
        if($RET['GRADE_SCALE_ID']!='')
        {
            ...
            $header .= '<TD><b>' . $options_RET[1]['TITLE'] . '</b><br>'._('Grading Scale').'</TD>';
        }
        else
        $header .= '<TD><b>' . ''._('Not Graded').'' . '</b><br>'._('Grading Scale').'</TD>';
        ...
        if($RET['CALENDAR_ID']!='')
        {
            ...
            $options_RET = DBGet(DBQuery($sql));
            $header .= '<TD><b>' . $options_RET[1]['TITLE'].'</b><br>'._('Calendar').''. '</TD>';
        }
        else
        $header .= '<TD><b>' . ''._('No Calendar Selected').''.'</b><br>'._('Calendar').'' . '</TD>';
        $header .= "</TR><TR>";
        $header .= '<TD><b>' . cbr($RET['DOES_ATTENDANCE']) .'</b><br>'._('Takes Attendance').'' . '</TD>';
        $header .= '<TD><b>' . cbr($RET['DOES_HONOR_ROLL']).'</b><br>'._('Affects Honor Roll').'' . '</TD>';
        $header .= '<TD><b>' . cbr($RET['DOES_CLASS_RANK']).'</b><br>'._('Affects Class Rank').''. '</TD>';
        $header .= '<TD><b>' . cbr($RET['HALF_DAY']).'</b><br>'._('Half Day').''. '</TD>';
        $header .= '<TD><b>' . cbr($RET['DOES_BREAKOFF']).'</b><br>'._('Allow Teacher Gradescale').'' . '</TD>';
        $header .= "</TR><TR>";
        ...
        if($RET['PARENT_ID']!='')
        {
            ...
            if(count($children))
            $header .= '<TD colspan=2><b>' . $children[1]['TITLE'].'</b><br>'._('Parent Course').'' . '</TD>';
            else
            $header .= '<TD><b>' . ''._('No Parent Course Selected').''.'</b><br>'._('Parent Course').''. '</TD>';
            //print_r($children);
        }
        else
        $header .= '<TD><b>' .''._(' No Parent Course Selected').''.'</b><br>'._('Parent Course').''. '</TD>';
        $header .= '<TD><b>' . $periods_RET[1]['START_TIME'].'</b><br>'._('Start Time').''. '</TD>';
        $header .= '<TD><b>' . $periods_RET[1]['END_TIME'].'</b><br>'._('End Time').'' . '</TD><td></td>';
        $header .= '</TR>';
        $header .= '</TABLE>';
...
else
{
    ...
    DrawBC(""._('Courses')." > ".ProgramTitle());
    if($_REQUEST['print']!='list')
    {
        echo PopTable('header',_('Quick Search'));
    }
    ...
    if(count($mp_RET))
    {
        ...
        if($_REQUEST['print']!='list')
        echo CreateSelect($mp_RET, 'marking_period_id',_('All'), _('Select Marking Period: '), 'Modules.php?modname='.$_REQUEST['modname'].'&marking_period_id=');
        echo '</TD>';
        ...
        if($_REQUEST['marking_period_id'] && $_REQUEST['marking_period_id']!='')
        {
            ...
            if(count($subjects_RET))
            {
                ...
                $link['TITLE']['link'] .= "&modfunc=$_REQUEST[modfunc]";
                if($_REQUEST['print']!='list')
                echo CreateSelect($subjects_RET, 'subject_id', _('All'), _('Select Subject').':', 'Modules.php?modname='.$_REQUEST['modname'].'&marking_period_id='.$_REQUEST['marking_period_id'].'&subject_id=');
                echo '</TD>';
                ...
                if($_REQUEST['subject_id'] && $_REQUEST['subject_id']!='' )
                {
                    ...
                    if(count($courses_RET))
                    {
                        ...
                        if($_REQUEST['print']!='list')
                        echo CreateSelect($courses_RET, 'course_id', _('All'), _('Select Course').':' , 'Modules.php?modname='.$_REQUEST['modname'].'&marking_period_id='.$_REQUEST['marking_period_id'].'&subject_id='.$_REQUEST['subject_id'].'&course_id=');
                        echo '</TD></tr>';
    ...
    echo "<tr><td><br><br><b>"._('No Class List Found')."</b></td>";
    echo '</TR></TABLE></form>';
    echo "<br><br><br>";
...
function CreateExcel($dli='', $pli='', $sli='', $cli='', $mp='', $mp_name='')
{
    ...
    else
    {
        if($dli==''){
            ...
            $heading=""._( "All available classes for")." <font color='black'>".$mp_name."</font>";
        }
        else
        {
            if($pli==''){
                ...
                $heading =""._('All available classes for')." <font color='black'>".$mp_name." -> ".$d_ret[1]['TITLE']."</font>";
            }
            else
            {
                if($sli==''){
    ...
    $column_names = array('SUBJECT'=>_('Subject'),'COURSE'=>_('Course'),'SHORT_NAME'=>_('Class Name'), 'PERIOD'=>_('Period'),'TEACHER'=>_('Teacher'), 'LOCATION'=>_('Location'),'DAYS'=>_('Days') );
