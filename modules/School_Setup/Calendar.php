...
if(clean_param($_REQUEST['modfunc'],PARAM_ALPHAMOD)=='create')
{
	...
	//$message = '<TABLE cellspacing=0 cellpadding=0 border=0 ><TR><TD colspan=7 align=center>'._('Title').' <INPUT type=text name=title class=cell_floating id=title> <INPUT type=checkbox name=default value=Y>'._('Default Calendar for this School').'<BR><BR></TD></TR><TR><TD colspan=7 align=center>'._('From').' '.PrepareDate($fy_RET['START_DATE'],'_min').' '._('To').' '.PrepareDate($fy_RET['END_DATE'],'_max').'</TD></TR><tr><td class=clear></td></tr><TR><TD><INPUT type=checkbox value=Y name=weekdays[0]>'._('Sunday').'</TD><TD><INPUT type=checkbox value=Y name=weekdays[1] CHECKED>'._('Monday').'</TD><TD><INPUT type=checkbox value=Y name=weekdays[2] CHECKED>Tuesday</TD><TD><INPUT type=checkbox value=Y name=weekdays[3] CHECKED>'>_('Wednesday').'</TD><TD><INPUT type=checkbox value=Y name=weekdays[4] CHECKED>'._('Thursday').'</TD><TD><INPUT type=checkbox value=Y name=weekdays[5] CHECKED>'._('Friday').'</TD><TD><INPUT type=checkbox value=Y name=weekdays[6]>'._('Saturday').'</TD></TR></TABLE>';
          $message = '<TABLE cellspacing=0 cellpadding=0 border=0 ><TR><TD colspan=7 align=center>'._('Title').' <INPUT type=text name=title class=cell_floating id=title> <INPUT type=checkbox name=default value=Y> '._('Default Calendar for this School').'<BR><BR></TD></TR><TR><TD colspan=7 align=center>'._('From').' '.PrepareDate($fy_RET['START_DATE'],'_min').' '._('To').' '.PrepareDate($fy_RET['END_DATE'],'_max').'</TD></TR><tr><td class=clear></td></tr><TR><TD><INPUT type=checkbox value=Y name=weekdays[0]>'._('Sunday').'</TD><TD><INPUT type=checkbox value=Y name=weekdays[1] CHECKED>'._('Monday').'</TD><TD><INPUT type=checkbox value=Y name=weekdays[2] CHECKED>'._('Tuesday').'</TD><TD><INPUT type=checkbox value=Y name=weekdays[3] CHECKED>'._('Wednesday').'</TD><TD><INPUT type=checkbox value=Y name=weekdays[4] CHECKED>'._('Thursday').'</TD><TD><INPUT type=checkbox value=Y name=weekdays[5] CHECKED>'._('Friday').'</TD><TD><INPUT type=checkbox value=Y name=weekdays[6]>'._('Saturday').'</TD></TR></TABLE>';       
       
          $message .=calendarEventsVisibility();
	if(Prompt_Calender(''._('Create a new calendar').'','',$message))
	{
...
{   
	...
	if($has_assigned>0){
	UnableDeletePrompt(_('Cannot delete because students are enrolled in this calendar.'));
	}
  elseif($has_assigned_cp>0){
	UnableDeletePrompt(''._('Cannot delete because course periods are created on this calendar.').'');
	}
        else{
...
if(clean_param($_REQUEST['modfunc'],PARAM_ALPHAMOD)=='edit_calendar')
{
	...
	$ac_RET = $ac_RET[1];
        ...
        $message = '<TABLE cellspacing=0 cellpadding=0 border=0 ><TR><TD colspan=7 align=center>'._('Title').' <INPUT type=text name=title class=cell_floating id=title value="'.$acs_RET['TITLE'].'"> <INPUT type=checkbox name=default value=Y '. (($acs_RET['DEFAULT_CALENDAR']=='Y')? 'checked' : '').'>'._('Default Calendar for this School').'<BR><BR></TD></TR><TR><TD colspan=7 align=center>'._('From').'</TD></TR><TR><TD colspan=7 align=center> '.PrepareDate($ac_RET['START_DATE'],'_min',true,array('disabled'=>true),'N').'</TD></TR><TR><TD colspan=7 align=center> '._('To').' </TD></TR><TR><TD colspan=7 align=center >'.PrepareDate($ac_RET['END_DATE'],'_max',true,array('disabled'=>true),'N').'</TD></TR><TR><TD class=clear></TD></TR><TR><TD><INPUT type=checkbox value=Y name=weekdays[0] '.((in_array('Sunday', $weekdays)==true)? 'CHECKED' : '').' DISABLED>'._('Sunday').'</TD><TD><INPUT type=checkbox value=Y name=weekdays[1] '.((in_array('Monday', $weekdays)==true)? 'CHECKED' : '').' DISABLED>'._('Monday').'</TD><TD><INPUT type=checkbox value=Y name=weekdays[2] '.((in_array('Tuesday', $weekdays)==true)? 'CHECKED' : '').' DISABLED>'._('Tuesday').'</TD><TD><INPUT type=checkbox value=Y name=weekdays[3] '.((in_array('Wednesday', $weekdays)==true)? 'CHECKED' : '').' DISABLED>'._('Wednesday').'</TD><TD><INPUT type=checkbox value=Y name=weekdays[4] '.((in_array('Thursday', $weekdays)==true)? 'CHECKED' : '').' DISABLED>'._('Thursday').'</TD><TD><INPUT type=checkbox value=Y name=weekdays[5] '.((in_array('Friday', $weekdays)==true)? 'CHECKED' : '').' DISABLED>'._('Friday').'</TD><TD><INPUT type=checkbox value=Y name=weekdays[6] '.((in_array('Saturday', $weekdays)==true)? 'CHECKED' : '').' DISABLED>'._('Saturday').'</TD></TR></TABLE>';
        $message .=calendarEventsVisibility();
        if(Prompt_Calender(''._('Edit this calendar').'','',$message))
        {
            $col=Calender_Title;
            $cal_title=str_replace("'","''",paramlib_validation($col,$_REQUEST['title']));
...
if($_REQUEST['modfunc']=='detail')
{
                                                    ...
                                                    foreach($_REQUEST['values'] as $column=>$value)
                                                    {
                                                            if(trim($value))
                                                            {
                                                                    ...
                                                                    if($column=="SCHOOL_DATE")
                                                                        ...
                                                                        $values .= '\''._(str_replace("'","''",trim($value))).'\',';
                                                                    }
                                                                    $go = true;
                                                            }
                                                    }
	...
	else
	{
		...
		echo '<TABLE>';
		echo '<TR><TD>'._('Date').'</TD><TD>'.DateInput($RET[1]['SCHOOL_DATE'],'values[SCHOOL_DATE]','',true).'</TD></TR>';
		echo '<TR><TD>'._('Title').'</TD><TD>'.TextInput($RET[1]['TITLE'],'values[TITLE]','','style=width:380px;').'</TD></TR>';
		if($RET[1]['STAFF_ID'])
			echo '<TR><TD>'._('Teacher').'</TD><TD>'.TextAreaInput($RET[1]['STAFF_ID'],'values[STAFF_ID]').'</TD></TR>';
                if($RET[1]['ASSIGNED_DATE'])
                echo '<TR><TD>'._('Assigned Date').'</TD><TD>'.TextAreaInput($RET[1]['ASSIGNED_DATE'],'values[ASSIGNED_DATE]').'</TD></TR>';
                if($RET[1]['DUE_DATE'])
                echo '<TR><TD>'._('Due Date').' </TD><TD>'.TextAreaInput($RET[1]['DUE_DATE'],'values[DUE_DATE]').'</TD></TR>';
		echo '<TR><TD>'._('Notes').'</TD><TD>'.TextAreaInput($RET[1]['DESCRIPTION'],'values[DESCRIPTION]','','style=width:380px;height:200px;').'';
		if(AllowEdit())
		{
                        echo '<TR><TD>'._('Show Events System Wide').'</TD><TD align=center>'. _makeCheckBoxInput($RET[1]['CALENDAR_ID'],$_REQUEST['event_id']).'</TD></TR>';
			echo '<TR><TD colspan=2 align=center><INPUT type=submit class=btn_medium name=button value='._("Save").' onclick="formload_ajax(\'popform\');">';
			echo '&nbsp;';
			if($_REQUEST['event_id']!='new')
...
if(clean_param($_REQUEST['modfunc'],PARAM_ALPHAMOD)=='list_events')
{
	...
	DrawHeaderHome(PrepareDateSchedule($start_date,'_start').' <div style="float:left;">&nbsp;-&nbsp;</div> '.PrepareDateSchedule($end_date,'_end').' <div style="float:left; padding-left:5px; padding-top:2px;"><A HREF=Modules.php?modname='.$_REQUEST['modname'].'&calendar_id='.$_REQUEST['calendar_id'].'&month='.$_REQUEST['month'].'&year='.$_REQUEST['year'].'>'._('Back to Calendar').'</A></div>','<div style="float:left;"><INPUT type=submit class=btn_medium value='._('Go').'></div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <a href="for_export.php?modname=School_Setup/Calendar.php&modfunc=print&_openSIS_PDF=true" target=_blank ><img src="assets/print.png" alt="Print" title="Print" />'._('Print').'</a>');
...
//                                $get_date=  DBGet(DBQuery('SELECT COUNT(SCHOOL_DATE) AS SCHOOL_DATE FROM attendance_completed WHERE SCHOOL_DATE=\''.$date.'\''));
                            ...
                            if($_REQUEST['show_all'][$date]=='Y')
                            {
                                if($get_date[1]['SCHOOL_DATE']==0)
                                    ...
                                    echo '<font color=red><b>'._('Selected Day has association').'</b></font>';
                            }
                            else
                            {
                                if($get_date[1]['SCHOOL_DATE']==0)
	...
	if(AllowEdit())
	{
		...
		unset($tmp_REQUEST['calendar_id']);
                                    ...
                                    $link .="<td><a href='#' onclick='load_link(\"Modules.php?modname=$_REQUEST[modname]&modfunc=create\");'>".button('add')."</a></td><td><a href='#' onclick='load_link(\"Modules.php?modname=$_REQUEST[modname]&modfunc=create\");'>"._('Create a new calendar')."</a> </td>";
                                    if($_REQUEST['calendar_id'])
                                            $link .="<td>&nbsp;&nbsp;|&nbsp;&nbsp;</td><td><a href='#' onclick='load_link(\"Modules.php?modname=$_REQUEST[modname]&modfunc=delete_calendar&calendar_id=$_REQUEST[calendar_id]\");'>".button('remove')."</a></td><td><a href='#' onclick='load_link(\"Modules.php?modname=$_REQUEST[modname]&modfunc=delete_calendar&calendar_id=$_REQUEST[calendar_id]\");'>"._('Delete this calendar')."</a></td><td>&nbsp;&nbsp;|&nbsp;&nbsp;</td><td><a href='#' onclick='load_link(\"Modules.php?modname=$_REQUEST[modname]&modfunc=edit_calendar&calendar_id=$_REQUEST[calendar_id]\");'>".button('edit')."</a></td><td><a href='#' onclick='load_link(\"Modules.php?modname=$_REQUEST[modname]&modfunc=edit_calendar&calendar_id=$_REQUEST[calendar_id]\");'>"._('Edit this calendar')."</a></td>";
                                    $link .='</tr></table>';
	}
        if($_REQUEST['calendar_id'])
	DrawHeaderHome(PrepareDate(strtoupper(date("d-M-y",$time)),'',false,array('M'=>1,'Y'=>1,'submit'=>true)).' <A HREF=Modules.php?modname='.$_REQUEST['modname'].'&modfunc=list_events&calendar_id='.$_REQUEST['calendar_id'].'&month='.$_REQUEST['month'].'&year='.$_REQUEST['year'].'>'._('List').' '._('Events').'</A>',SubmitButton(''._('Save').'','','class=btn_medium'));
	...
	echo "<TD class=white>"._('Sunday')."</TD><TD class=white>"._('Monday')."</TD><TD class=white>"._('Tuesday')."</TD><TD class=white>"._('Wednesday')."</TD><TD class=white>"._('Thursday')."</TD><TD class=white>"._('Friday')."</TD><TD width=99 class=white>"._('Saturday')."</TD>";
	echo "</TR><TR>";
	...
	for($i=1;$i<=$last;$i++)
	{
		...
		if(AllowEdit())
		{
			if($calendar_RET[$date][1]['MINUTES']=='999')
				echo '<TABLE cellpadding=0 cellspacing=0 ><TR><TD><div id=syswide_holi_'.$i.' style=display:none><span>'._('System Wide').' </span><INPUT type=checkbox name=show_all['.$date.'] value=Y></div></TD><TD>'.CheckboxInput_Calendar($calendar_RET[$date],"all_day[$date]",'','',false,'<IMG SRC=assets/check.gif> ','',true,'id=all_day_'.$i.' onclick="return system_wide('.$i.');"').'</TD></TR></TABLE>';
			else
			{
				echo "<TABLE cellpadding=0 cellspacing=0 ><TR><TD><div id=syswide_holi_$i style=display:none><span>"._('System Wide')." </span><INPUT type=checkbox name=show_all[$date] value=Y></div></TD><TD><INPUT type=checkbox name=all_day[$date] value=Y id=all_day_$i onclick='return system_wide($i);'></TD>";
	...
	echo "</TD></TR></TABLE>";
	echo '<BR>'.SubmitButton(''._('Save').'','','class=btn_medium');
	echo "</CENTER>";
	echo '</FORM>';
}
...
function calendarEventsVisibility()
{
	...
	$profiles_RET = DBGet(DBQuery('SELECT ID,TITLE FROM user_profiles ORDER BY ID'));
                  ...
                  $return .= '<TABLE border=0 cellspacing=0 cellpadding=0 width=96% class=LO_field><TR><TD colspan=4><b>'._('Events').''._(' Visible ').''._('To').': </b></TD></TR>';
	foreach(array('admin'=>''._('Administrator w/Custom').'','teacher'=>''._('Teacher').''.''._(' w/Custom').'','parent'=>''._('Parent w/Custom').'') as $profile_id=>$profile)
		$return .= "<tr><TD colspan=4><INPUT type=checkbox name=profiles[$profile_id] value=Y".(in_array($profile_id,$visible_profile)?' CHECKED':'')."> "._($profile)."</TD></tr>";
	$i = 3;
	foreach($profiles_RET as $profile)
	{
		$i++;
		$return .= '<tr><TD colspan=4><INPUT type=checkbox name=profiles['.$profile['ID'].'] value=Y'.(in_array($profile[ID],$visible_profile)?' CHECKED':'')."> "._($profile[TITLE])."</TD></tr>";
