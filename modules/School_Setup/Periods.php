                ...
                ##############################################################################################################
                    if($id!='new')
                    {
                        ...
                        foreach($columns as $column=>$value)
                        {
                            ...
                            $checker=DBGet(DBQuery('SELECT COUNT(1) as TOTAL FROM schedule s,course_periods cp WHERE cp.COURSE_PERIOD_ID=s.COURSE_PERIOD_ID AND (s.END_DATE>\''.date('Y-m-d').'\' OR s.END_DATE IS NULL) AND cp.PERIOD_ID=\''.$id.'\' '));
                            ...
                            {
                            $err=_('Cannot modify start date or end date as period is associated');
                            $go=false;
                            }
                        }
                            else{
