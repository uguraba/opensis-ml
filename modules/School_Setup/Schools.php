...
//                                                                        echo '<script language=JavaScript>parent.side.location="'.$_SESSION['Side_PHP_SELF'].'?modcat="+parent.side.document.forms[0].modcat.value;</script>'; 
	...
	echo '<script language=JavaScript>parent.side.location="'.$_SESSION['Side_PHP_SELF'].'?modcat="+parent.side.document.forms[0].modcat.value;</script>';
	echo "<br><br>";
	DrawHeaderHome('<IMG SRC=assets/check.gif> &nbsp;'._(' A new school called').'<strong>'.  GetSchool(UserSchool()).'</strong>'._(' has been created. To finish the operation, click OK button.').'','<INPUT  type=submit value='._("OK").' class="btn_medium">');
...
echo '<br><strong>'._('School has been created successfully.').'</strong>';
}  else {
if(!$_REQUEST['modfunc'])
{
...
#	echo "<FIELDSET><TABLE>";
	...
	echo "<TR><TD class='label'><b>"._('School Name').":</b></td><td>".TextInput($schooldata['TITLE'],'values[TITLE]', '','class=cell_floating size=24')."</td></TR>";
	echo "<TR ALIGN=LEFT><TD class='label'><b>"._('Address').":</b></TD><td>".TextInput($schooldata['ADDRESS'],'values[ADDRESS]','','class=cell_floating maxlength=100 size=24')."</td></TR>";
	echo "<TR ALIGN=LEFT><TD class='label'><b>"._('City').":</b></TD><td>".TextInput($schooldata['CITY'],'values[CITY]','','maxlength=100, class=cell_floating size=24')."</td></TR>";
	echo "<TR ALIGN=LEFT><TD class='label'><b>"._('State').":</b></TD><td>".TextInput($schooldata['STATE'],'values[STATE]','','maxlength=10 class=cell_floating size=24')."</td></TR>";
	echo "<TR ALIGN=LEFT><TD class='label'><b>"._('Zip').''._('/').''._('Postal Code').":</b></TD><td>".TextInput($schooldata['ZIPCODE'],'values[ZIPCODE]','','maxlength=10 class=cell_floating size=24')."</td></TR>";

	echo "<TR ALIGN=LEFT><TD  class='label'><b>"._('Telephone').":</b></td><td>".TextInput($schooldata['PHONE'],'values[PHONE]','','class=cell_floating size=24')."</TD></TR>";
	echo "<TR ALIGN=LEFT><TD class='label'><b>"._('Principal').":</b></td><td>".TextInput($schooldata['PRINCIPAL'],'values[PRINCIPAL]','','class=cell_floating size=24')."</TD></TR>";
        echo "<TR ALIGN=LEFT><TD class='label'><b>"._('Base Grading Scale').":</b></td><td>".TextInput($schooldata['REPORTING_GP_SCALE'],'values[REPORTING_GP_SCALE]','','class=cell_floating maxlength=10 size=24')."</TD></TR>";
        echo "<TR ALIGN=LEFT><TD class='label'><b>"._('E-Mail').":</b></td><td>".TextInput($schooldata['E_MAIL'],'values[E_MAIL]','','class=cell_floating maxlength=100 size=24')."</TD></TR>";
        echo "<TR ALIGN=LEFT><TD class='label'><b>"._('CEEB').":</b></td><td>".TextInput($schooldata['CEEB'],'values[CEEB]','','class=cell_floating maxlength=100 size=24')."</TD></TR>";

	if(AllowEdit() || !$schooldata['WWW_ADDRESS'])
		echo "<TR ALIGN=LEFT><TD class='label'><b>"._('Website').":</b></td><td>".TextInput($schooldata['WWW_ADDRESS'],'values[WWW_ADDRESS]','','class=cell_floating size=24')."</TD></TR>";
	else
		...
		echo "<TR ALIGN=LEFT><TD class='label'><b>"._('Website').":</b></td><td><A HREF=http://$schooldata[WWW_ADDRESS] target=_blank>$schooldata[WWW_ADDRESS]</A></TD></TR>";
 ...
 echo "<TR ALIGN=LEFT><TD class='label'><b>"._('School Logo').": </b></td><td><table><tr><td><div align=center><a href ='Modules.php?modname=School_Setup/UploadLogo.php&modfunc=edit'><img src='".$uploaded_sql[1]['VALUE']."' width=70 class=pic /></a></div></td><td><a href ='Modules.php?modname=School_Setup/UploadLogo.php&modfunc=edit'>"._('Click here to')."<br />"._('change logo')."</a></td></tr></table></TD></TR>";
 else if(!$_REQUEST['new_school'])
 echo "<TR ALIGN=LEFT><TD class='label'><b>"._('School Logo').":</b></td><td><a href ='Modules.php?modname=School_Setup/UploadLogo.php'>"._('Click here to upload logo')."</a></TD></TR>";
...
#	echo "</TABLE></FIELDSET>";
	...
	if(User('PROFILE')=='admin' && AllowEdit())
	{
		...
		else
		{
			...
			DrawHeader('','',"<CENTER><INPUT TYPE=submit name=button id=button class=btn_medium VALUE="._('Update')." onclick='return formcheck_school_setup_school();'></CENTER>");
		}
	}
