...
$tables = array('school_periods'=>''._('School Periods').'','school_years'=>''._('Marking Periods').'','report_card_grades'=>''._('Report Card Grade Codes').'','report_card_comments'=>''._('Report Card Comment Codes').'','eligibility_activities'=>''._('Eligibility Activity Codes').'','attendance_codes'=>''._('Attendance Codes').'','school_gradelevels'=>''._('Grade Levels').'');

$table_list = '<TABLE align=center cellspacing="5" cellpadding="3" border=0><tr><td ><b>'._('New').''._(' ')._('School').''._(" ")._("'s").''._('Title').':&nbsp;&nbsp;</b><INPUT type=text name=title value='._('NewSchool').'  class="cell_medium_wide"></td></tr>';
...
echo '<strong>'._('School information has been copied successfully.').'</strong>';
}else{
DrawBC(_('School Setup')." > ".ProgramTitle());

if(Prompt_Copy_School(''._('Confirm Copy School').'',''._('Are you sure you want to copy the data for ').''.GetSchool(UserSchool()).''._('to a new school').'?',$table_list))
{
	...
	echo "<br><br>";
	DrawHeaderHome('<IMG SRC=assets/check.gif> &nbsp;'._('The data have been copied to a new school called').' '.paramlib_validation($col=TITLE,$_REQUEST['title']).' '._('To finish the operation, click OK button.').'','<INPUT  type=submit value='._("OK").' class="btn_medium">');
