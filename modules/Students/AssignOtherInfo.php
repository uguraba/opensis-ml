...
if(clean_param($_REQUEST['modfunc'],PARAM_ALPHAMOD)=='save')
{
	...
	if(count($_REQUEST['values']) && count($_REQUEST['student']))
	{
		...
		unset($_SESSION['_REQUEST_vars']['values']);
                ...
                {
                    ShowErr(''._('You must choose at least one field and one student').'');
                    for_error();
}
}
...
if(!$_REQUEST['modfunc'])
{
	...
	if($_REQUEST['search_modfunc']=='list')
	{
		...
		echo '<CENTER><SELECT name=category_id onchange="document.location.href=\''.PreparePHP_SELF($tmp_REQUEST).'&amp;category_id=\'+this.form.category_id.value;"><OPTION value="">'._('All').' '._('Categories').'</OPTION>';
		foreach($categories_RET as $category)
			echo '<OPTION value='.$category['ID'].($_REQUEST['category_id']==$category['ID']?' SELECTED':'').'>'._($category['TITLE']).'</OPTION>';
			...
			foreach($fields_RET['text'] as $field)
			 {
				...
				echo '<TR><TD class=lable>'._($field['TITLE']).'</TD><TD>'._makeTextInput($title).'</TD></TR>';
			 }	
		}
		if(count($fields_RET['numeric']))
		{
			foreach($fields_RET['numeric'] as $field)
				echo '<TR><TD class=lable>'._($field['TITLE']).'</TD><TD>'._makeTextInput('CUSTOM_'.$field['ID'],true).'</TD></TR>';
		}
		if(count($fields_RET['date']))
		{
			foreach($fields_RET['date'] as $field)
				echo '<TR><TD class=lable>'._($field['TITLE']).'</TD><TD>'._makeDateInput('CUSTOM_'.$field['ID']).'</TD></TR>';
		}
		if(count($fields_RET['select']))
		{
			foreach($fields_RET['select'] as $field)
			{
				...
				echo "<TR><TD class=lable_right valign=top>"._($field[TITLE])."</TD><TD>"._makeSelectInput($field[TITLE],$select_options).'</TD></TR>';
				echo "</TD></TR>";
                            }
                            else 
                            {
				...
				echo "<TR><TD class=lable_right valign=top>"._($field[TITLE])."</TD><TD>"._makeSelectInput('CUSTOM_'.$field['ID'],$select_options).'</TD></TR>';
				echo "</TD></TR>";
                            }
			}
		}
		if(count($fields_RET['textarea']))
		{
			...
			{
				echo '<TR><TD class=lable_right valign=top>'._($field['TITLE']).'</TD>';
				echo '<TD>';
		...
		{
			echo '<TR><TD class=lable valign=top>'._('Rolling Retention / Options').'</TD>';
			echo '<TD>';
			...
			echo '<TD class=lable_right valign=top>'._('Calendar').'</TD>';
			echo '<TD>';
