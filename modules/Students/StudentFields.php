...
if(clean_param($_REQUEST['modfunc'],PARAM_ALPHAMOD)=='delete')
{
	...
	$msg=_('Cannot delete because student fields are associated.');
	}else if($_REQUEST['category_id']==5){
	$has_assigned_RET=DBGet(DBQuery("SELECT COUNT(*) AS TOTAL_ASSIGNED FROM goal"));
	$has_assigned=$has_assigned_RET[1]['TOTAL_ASSIGNED'];
	$msg=_('Cannot delete because field categories are associated.');
	}
	if($has_assigned>0){
...
if(!$_REQUEST['modfunc'])
{
	...
	if(AllowEdit() && $_REQUEST['id']!='new' && $_REQUEST['category_id']!='new' && ($_REQUEST['id'] || $_REQUEST['category_id']>4))
		$delete_button = "<INPUT type=button value="._('Delete')." class=btn_medium onClick='javascript:window.location=\"Modules.php?modname=$_REQUEST[modname]&modfunc=delete&category_id=$_REQUEST[category_id]&id=$_REQUEST[id]\"'>"."&nbsp;";
	...
	if($_REQUEST['id'])
	{
		...
		DrawHeaderHome($title,$delete_button.SubmitButton(_('Save'),'','class=btn_medium onclick="formcheck_student_studentField_F1();"')); //'<INPUT type=submit value=Save>');
		...
		if($_REQUEST['id']!='new')
		{
			if($RET['TYPE']!='select' && $RET['TYPE']!='autos' && $RET['TYPE']!='edits' && $RET['TYPE']!='text')
			{
				...
				$type_options = array('select'=>''._('Pull-Down').'','autos'=>''._('Auto Pull-Down').'','edits'=>''._('Edit Pull-Down').'','text'=>''._('Text').'','radio'=>''._('Checkbox').'','codeds'=>''._('Coded Pull-Down').'','numeric'=>''._('Number').'','multiple'=>''._('Select Multiple from Options').'','date'=>''._('Date').'','textarea'=>''._('Long Text').'');
			}
			else
				$type_options = array('select'=>''._('Pull-Down').'','autos'=>''._('Auto Pull-down').'','edits'=>''._('Edit Pull-Down').'','text'=>''._('Text').'');
		}
		else
			$type_options = array('select'=>''._('Pull-Down').'','autos'=>''._('Auto Pull-down').'','edits'=>''._('Edit Pull-Down').'','text'=>''._('Text').'','radio'=>''._('Checkbox').'','codeds'=>''._('Coded Pull-Down').'','numeric'=>''._('Number').'','multiple'=>''._('Select Multiple from Options').'','date'=>''._('Date').'','textarea'=>''._('Long Text').'');

		$header .= '<TD>' . SelectInput($RET['TYPE'],'tables['.$_REQUEST['id'].'][TYPE]',''._('Data Type').'',$type_options,false,'id=type onchange="formcheck_student_studentField_F1_defalut();"') . '</TD>';
		...
		$header .= '<TD>' . SelectInput($RET['CATEGORY_ID']?$RET['CATEGORY_ID']:$_REQUEST['category_id'],'tables['.$_REQUEST['id'].'][CATEGORY_ID]',''._('Student Field Category').'',$categories_options,false) . '</TD>';
                                    if($_REQUEST['id']=='new' || $RET['SORT_ORDER']=='')
                                        $header .= '<TD>' . TextInput($RET['SORT_ORDER'],'tables['.$_REQUEST['id'].'][SORT_ORDER]',''._('Sort Order').'','maxlength=5 onkeydown="return numberOnly(event);"') . '</TD>';
                                    else
                                        $header .= '<TD>' . TextInput($RET['SORT_ORDER'],'tables['.$_REQUEST['id'].'][SORT_ORDER]',''._('Sort Order').'','maxlength=5 onkeydown=\"return numberOnly(event);\"') . '</TD>';
		$header .= '</TR><TR>';
		$colspan = 2;
		...
		{
			$header .= '<TD colspan=2>'.TextAreaInput($RET['SELECT_OPTIONS'],'tables['.$_REQUEST['id'].'][SELECT_OPTIONS]',''._('Pull-Down/Auto Pull-Down/Coded Pull-Down/Select Multiple Choices').'<BR>'._('* one per line').'','rows=7 cols=40') . '</TD>';
			$colspan = 1;
		}
		$header .= '<TD valign=bottom colspan='.$colspan.'>'.TextInput_mod_a($RET['DEFAULT_SELECTION'],'tables['.$_REQUEST['id'].'][DEFAULT_SELECTION]',''._('Default').'').'<small><BR>'._('* for dates').': YYYY-MM-DD,<BR> '._('for checkboxes').': Y <BR> '.('for long text it will be ignored').'</small></TD>';

		$new = ($_REQUEST['id']=='new');
		$header .= '<TD>' . CheckboxInput($RET['REQUIRED'],'tables['.$_REQUEST['id'].'][REQUIRED]',''._('Required').'','',$new) . '</TD>';
                                    $header .= '<TD>' . CheckboxInput($RET['HIDE'],'tables['.$_REQUEST['id'].'][HIDE]',''._('Hide').'','',$new) . '</TD>';
		$header .= '</TR>';
		$header .= '</TABLE>';
	}
	elseif($_REQUEST['category_id'])
	{
		...
		echo " method=POST>";
		DrawHeaderHome($title,$delete_button.SubmitButton(_('Save'),'','class=btn_medium onclick="formcheck_student_studentField_F2();"'));
		$header .= '<TABLE cellpadding=3 width=100%>';
		$header .= '<TR>';

		$header .= '<TD>' . TextInput($RET['TITLE'],'tables['.$_REQUEST['category_id'].'][TITLE]',''._('Title').'') . '</TD>';
		if($_REQUEST['category_id']=='new' || $RET['SORT_ORDER']=='')
                                        $header .= '<TD>' . TextInput($RET['SORT_ORDER'],'tables['.$_REQUEST['category_id'].'][SORT_ORDER]',''._('Sort Order').'','maxlength=5 onkeydown="return numberOnly(event);"') . '</TD>';
                                    else
                                       $header .= '<TD>' . TextInput($RET['SORT_ORDER'],'tables['.$_REQUEST['category_id'].'][SORT_ORDER]',''._('Sort Order').'','maxlength=5 onkeydown=\"return numberOnly(event);\"') . '</TD>';
	...
	echo '<TD valign=top>';
	$columns = array('TITLE'=>''._('Category').'','SORT_ORDER'=>''.('Order').'');
	$link = array();
...
//	$link['add']['link'] = "Modules.php?modname=$_REQUEST[modname]&category_id=new";
	...
	if($_REQUEST['category_id'] && $_REQUEST['category_id']!='new' && count($categories_RET))
	{
		...
		echo '<TD valign=top>';
		$columns = array('TITLE'=>_('Student Field'),'FEILD'=>_('Field Type'));
		$link = array();
	...
	//	$link['add']['link'] = "Modules.php?modname=$_REQUEST[modname]&category_id=$_REQUEST[category_id]&id=new";
                ...
                switch ($_REQUEST[category_id]) {
                    case 1:
                             $arr=array(''._('Name').'',''._('Estimated Grad. Date').'',''._('Gender').'',''._('Ethnicity').'',''._('Common Name').'',''._('Birthdate').'',''._('Primary Language').'',''._('Email').'',''._('Phone').''
                                 ,''._('Student ID').'',''._('Alternate ID').'',''._('Grade').'',''._('Calendar').'',''._('Rolling/Retention Options').'',''._('Username').'',''._('Password').'',''._('Last Login').'',''._('Disable Student').''
                                 ,''._('Start Date').'',''._('Enrollment Code').'',''._('Drop Date').'',''._('Drop Code').'',''._('School').'');
                        ...
                        break;
                    case 2:
                             $arr=array(''._('Primary Care Physician').'',''._("Physician's Phone").'',''._('Preferred Medical Facility').'',''._('Date(Medical Notes)').'',''._('Medical Notes').'',''._('Type(Immunizations)').''
                                 ,''._('Date(Immunizations)').'',''._('Comments(Immunizations)').'',''._('Alert Date').'',''._('Medical Alert').'',''._('Date(Nurse Visits)').'',''._('Time In').'',''._('Time Out').'',''._('Reason').'',''._('Result').'',''._('Comments(Nurse Visits)').'');
                             $link['TITLE']['link'] = "Modules.php?modname=$_REQUEST[modname]&category_id=$_REQUEST[category_id]";
                             $link['add']['link'] = "#"." onclick='check_content(\"ajax.php?modname=$_REQUEST[modname]&category_id=$_REQUEST[category_id]&id=new\");'";
                             $link['TITLE']['variables'] = array('id'=>'ID');
                        ...
                        break;
                     case 3:
                             $arr=array(''._('Address Line 1').'',''._('Address Line 2').'',''._('City').'',''._('State').'',''._('Zip/Postal Code').'',''._('School Bus Pick-up').'',''._('School Bus Drop-off').'',''._('Bus No').'',''._('Relationship to Student').''
                                 ,''._('First Name').'',''._('Last Name').'',''._('Home Phone').'',''._('Work Phone').'',''._('Cell/Mobile Phone').'',''._('Email').'',''._('Custody of Student').'');

                        break;
                     case 4:
                             $arr=array(''._('Entered By').'',''._('Date').'',''._('Comments').'');

                        break;
                    case 5:
                             $arr=array(''._('Goal Title').'',''._('Begin Date').'',''._('End Date').'',''._('Goal Description').'');
                ...
                foreach ($arr as $key => $value) {
                    $fields_RET1[$count]=array('ID'=>'','TITLE'=>$value,'FEILD'=>'<span style="color:#ea8828;">'._('Default').'</span>');
                    $count++;
                }
                $count2=1;
		...
		ListOutput($dd,$columns,_('Student Field'),_('Student Fields'),$link,array(),$LO_options);
...
{
	$options = array('radio'=>''._('Checkbox').'','text'=>''._('Text').'','autos'=>''._('Auto Pull-Down').'','edits'=>''._('Edit Pull-Down').'','select'=>''._('Pull-Down').'','codeds'=>''._('Coded Pull-Down').'','date'=>''._('Date').'','numeric'=>''._('Number').'','textarea'=>''._('Long Text').'','multiple'=>''._('Select Multiple').'');
