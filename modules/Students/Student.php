...
if($_REQUEST['action'] == 'delete_goal' || $_REQUEST['action']=='delete_goal_can' || $_REQUEST['action']=='delete_goal_ok')
{
	...
	if(!isset($_REQUEST['ans']))
	{
		...
		PopTable('header',''._('Delete Confirmation').'');
		echo "<div class=clear></div><b>"._('Are you sure want to delete this Goal').('?')."</b><div class=clear></div><div class=clear></div><center><a href='Modules.php?modname=Students/Student.php&include=Goal&category_id=5&action=delete_goal_ok&gid=".$goal_id."&ans=yes' style='text-decoration:none; padding:6px 24px 6px 25px;' class=btn_medium><strong>"._('OK')."</strong></a> 
		
		<a href='Modules.php?modname=Students/Student.php&include=Goal&category_id=5&action=delete_goal_can&gid=".$goal_id."&ans=no' style='text-decoration:none; padding:6px 15px 6px 15px;' class=btn_medium><strong>"._('Cancel')."</strong></a></center>";
	...
	elseif(isset($_REQUEST['ans']) && $_REQUEST['ans']=='yes')
	{
		...
		else
		{
			...
			echo '<div align="center"><font color="red"><b>'._('Unable to delete Goal. Please delete Progresses first.').'</b></div>';
			unset($_REQUEST['modfunc']);
		}
	}
	else
...
if($_REQUEST['action']=='delete' || $_REQUEST['action']=='delete_can' || $_REQUEST['action']=='delete_ok')
{
	...
	if(!isset($_REQUEST['ans']))
	{
		...
		echo "<div class=clear></div><b>"._('Are you sure want to delete this Progress?')."</b><div class=clear></div><div class=clear></div><center><a href='Modules.php?modname=Students/Student.php&include=Goal&category_id=5&action=delete_ok&gid=".$goal_id."&pid=".$progress_id."&ans=yes' style='text-decoration:none; padding:6px 24px 6px 25px;' class=btn_medium><strong>"._('OK')."</strong></a> 
		
		<a href='Modules.php?modname=Students/Student.php&include=Goal&category_id=5&action=delete_can&gid=".$goal_id."&pid=".$progress_id."&ans=no' style='text-decoration:none; padding:6px 15px 6px 15px;' class=btn_medium><strong>"._('Cancel')."</strong></a></center>";
...
if($_REQUEST['err_msg']==true)
    echo "<center><font color=red><b>"._('Birthdate is invalid, data could not be saved.')."</b><font></center>";
if($_REQUEST['modfunc']=='detail' && $_REQUEST['student_id'] && $_REQUEST['student_id']!='new')
{
	...
	else
	{
		...
		echo '<TABLE>';
		echo '<TR><TD>'._('Current school drop date').'</TD><TD>'.'  '.DateInput_for_EndInput('','TRANSFER[STUDENT_ENROLLMENT_END_DATE]','',$div,true).'</TD></TR>';
		echo '<TR><TD>'._('Transferring to').'</TD><TD>'.SelectInput('','TRANSFER[SCHOOL]','',$options,false,'class=cell_medium').'</TD></TR>';
		echo '<TR><TD>'._('New school\'s enrollment date').'</TD><TD>'.'  '.DateInput_for_EndInput('','TRANSFER[STUDENT_ENROLLMENT_START]','',$div,true).'</TD></TR>';
...
//            AND LAST_NAME='".$stud_rec[1]['LAST_NAME']."' AND STUDENT_ID!='".UserStudentID()."'";
    ...
    if((count($_REQUEST['students']) || count($_REQUEST['values'])) && AllowEdit())
    {
		if($_REQUEST['student_id'] && $_REQUEST['student_id']!='new')
        {
			...
			if(count($_REQUEST['students']))
            {
                ...
                foreach($_REQUEST['students'] as $column_name=>$value)
                {
                                                                                ...
                                                                                if($custom['NULL']=='NO' && trim($value)=='' && $custom['DEFAULT']){
                                                                                    ...
                                                                                    $custom_TITLE=$custom_RET[1]['TITLE'];
                                                                                    echo "<font color=red><b>"._('Unable to save data, because')." ".$custom_TITLE.' '._('is Numeric type.').'</b></font><br/>';
                                                                                    $error=true;
                                                                                }else{
            ...
            if($msg)
                echo '<font style="color:red"><b>'._($msg).'</b></font>';
        }
        else
        {
						...
						if($column=='START_DATE' || $column=='END_DATE') 
                                                {
                                                    ...
                                                    {
                                                        $err=''._("Invalid Enrollment date could not be saved").'';
                                                        continue;
                                                    }
            ...
            if(!$error_new_student){
                ...
                else
                {
                    $_REQUEST['student_id'] = "new";
                    unset($value);
                    echo "<font color=red><b>"._('User name already exist. Please try with a different user name.')."</b></font>";
                }
            }
            $new_student = true;
        }
...
{
if($err)
    echo '<center><font color=red><b>'._($err).'</b></font></center>';
if($_REQUEST['student_id']=='new')
DrawBC(''.''._('Students').''.'' > ''.''._('Add a Student').''.'');
else
...
schedule ss
 WHERE s.STUDENT_ID=ssm.STUDENT_ID AND ssm.STUDENT_ID=ss.STUDENT_ID AND ssm.SYEAR=cp.SYEAR AND ssm.SYEAR=ss.SYEAR AND cp.COURSE_ID=ss.COURSE_ID AND cp.COURSE_PERIOD_ID=ss.COURSE_PERIOD_ID AND ss.MARKING_PERIOD_ID IN (".GetAllMP('',$queryMP).")
		 ...
		 if(User('PROFILE')=='admin' || User('PROFILE')=='teacher')
		  {
				...
				echo "<div style='margin-right:15px; font-weight:bold; font-size:14px;'>"._('Showing')." ".$count." "._('of')." ".$_SESSION['total_stu']."</div>";
				...
				if($total[1]['STUDENT_ID']>1)
				{
				 ...
				 {
					echo "<span class='pg-prev' style='margin-right:10px; font-size:14px; font-weight:normal;'><A HREF=Modules.php?modname=Students/Student.php&v=1&student_id=".UserStudentID()." >&laquo; "._('First')."</A></span>";
					//echo '&nbsp;&nbsp;&nbsp;';
					echo "<span class='pg-prev' style='margin-right:10px; font-size:14px; font-weight:normal;'><A HREF=Modules.php?modname=Students/Student.php&v=2&student_id=".UserStudentID()." >&lsaquo; "._('Previous')."</A></span>";
				 }
				 if(UserStudentID()!=$s2_id[1]['STUDENT_ID'])
				 {
					//echo '&nbsp;&nbsp;&nbsp;';
					echo "<span class='pg-nxt' style='margin-left:10px; font-size:14px; font-weight:normal;'><A HREF=Modules.php?modname=Students/Student.php&v=3&student_id=".UserStudentID()." >"._('Next')." &rsaquo;</A></span>";
					//echo '&nbsp;&nbsp;&nbsp;';
					echo "<span class='pg-nxt' style='margin-left:10px; font-size:14px; font-weight:normal;'><A HREF=Modules.php?modname=Students/Student.php&v=4&student_id=".UserStudentID()." >"._('Last')." &raquo;</A></span>";
				 }
			}
    ...
    if($_REQUEST['modfunc']!='delete' || $_REQUEST['delete_ok']=='1')
    {
      ...
      echo "<input type='hidden' value="._('Addresses & Contacts').">";
   ...
   {
            echo '<CENTER>'.SubmitButton(''._('Save').'','','class=btn_medium onclick="formcheck_student_student();"').'</CENTER>';
   }
        echo '</FORM>';
    }
    else
