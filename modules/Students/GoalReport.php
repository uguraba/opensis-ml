...
$res_goal = DBGet(DBQuery($sql_goal),array('START_DATE'=>'ProperDate','END_DATE'=>'ProperDate'));
	...
	#echo "<tr><td valign=top width=300><table width=100% ><tr><td colspan=2 style=\"border-bottom:1px solid #333;  font-weight:bold;\">Personal Information</td></tr>";
		...
		if(count($res_goal) != 0)
		{
			...
			echo "<meta charset='UTF-8'><tr><td width=105>".DrawLogo()."</td><td  style=\"font-size:15px; font-weight:bold; padding-top:20px;\">". GetSchool(UserSchool())."</font></td><td align=right style=\"padding-top:20px;\">". ProperDate(DBDate()) ."<br />"._('Powered by openSIS')."</td></tr><tr><td colspan=3 style=\"border-top:1px solid #333;\">&nbsp;</td></tr></table>";
			echo "<meta charset='UTF-8'><table width=100% cellspacing=0 style=\"border-collapse:collapse\">";
		
		
			echo "<tr><td width=15%>"._('Student Name').":</td>";
			echo "<td>" .$student['FULL_NAME']. "</td></tr>";
			/*
			echo "<tr><td>OSH ID:</td>";
			echo "<td>". $student['OSH_ID'] ." </td></tr>";
			*/
			echo "<tr><td>"._('Grade').":</td>";
			echo "<td>". $student['GRADE_ID'] ." </td></tr>";
			echo "<tr><td>"._('Gender').":</td>";
			echo "<td>".$sql_student['GENDER'] ."</td></tr>";
			echo "<tr><td>"._('Ethnicity').":</td>";
			echo "<td>".$sql_student['ETHNICITY'] ."</td></tr>";
			if($sql_student['COM_NAME'] !='')
			{
			echo "<tr><td>"._('Common Name').":</td>";
			echo "<td>".$sql_student['COM_NAME'] ."</td></tr>";
			}
			if($sql_student['SOCIAL_SEC'] !='')
			{
			echo "<tr><td>"._('Social Security').":</td>";
			...
			if($sql_student['LANG'] !='')
			{
			echo "<tr><td>"._('Language Spoken').":</td>";
			echo "<td>".$sql_student['LANG'] ."</td></tr>";
			echo "<tr><td colspan=2 style=\"height:18px\"></td></tr>";
			}
			
			echo '<tr><td>&nbsp;</td><td>&nbsp;</td></tr>';
			echo '<tr><td><b><u>'._('Goal Details').'</u></b></td><td>&nbsp;</td></tr>';
			echo "<tr><td colspan=2 style=\"height:18px\"></td></tr>";
			foreach($res_goal as $row_goal)
			{               
				echo '<tr><td><b>'._('Goal Title').': </b></td><td>'.$row_goal['GOAL_TITLE'].'</td></tr>';
				echo '<tr><td><b>'._('Begin Date').': </b></td><td>'.$row_goal['START_DATE'].'</td></tr>';
				echo '<tr><td><b>'._('End Date').': </b></td><td>'.$row_goal['END_DATE'].'</td></tr>';
				echo '<tr><td valign=top><b>'._('Goal Description:').' </b></td><td>'.$row_goal['GOAL_DESCRIPTION'].'</td></tr>';
				echo "<tr><td colspan=2 style=\"height:18px\"></td></tr>";
				...
				{               $goal_id=$row_goal['GOAL_ID'];
					...
					echo '<tr><td><b><u>'._('Progress Details').'</u></b></td><td>&nbsp;</td></tr>';
					echo "<tr><td colspan=2 style=\"height:18px\"></td></tr>";
					foreach($res_pro as $row_pro)
					{
						echo '<tr><td><b>'._('Date of Entry').': </b></td><td>'.$row_pro['START_DATE'].'</td></tr>';
					# ----------------------------- CP ------------------------------------------------- #	
						echo '<tr><td><b>'._('Course Period').': </b></td><td>'.$row_pro['CP_TITLE'].'</td></tr>';
					# ----------------------------- CP ------------------------------------------------- #		
						echo '<tr><td><b>'._('Progress Period Name').': </b></td><td>'.$row_pro['PROGRESS_NAME'].'</td></tr>';
						echo '<tr><td><b>'._('Proficiency').': </b></td><td>'.$row_pro['PROFICIENCY'].'</td></tr>';
						echo '<tr><td><b>'._('Progress Assessment').': </b></td><td>'.$row_pro['PROGRESS_DESCRIPTION'].'</td></tr>';
						echo "<tr><td colspan=2 style=\"height:18px\"></td></tr>";
					}
				}
				
				echo "<meta charset='UTF-8'><tr><td colspan=2 style=\"height:18px; border-top:1px solid #333;\"></td></tr>";
			}
...
#	DrawHeaderHome('<table><tr><td>'.PrepareDate($start_date,'_start').'</td><td> - </td><td>'.PrepareDate($end_date,'_end').'</td><td> - </td><td>'.$advanced_link.'</td><td> : <INPUT type=submit value=Go class=btn_medium></td></tr></table>');
	...
	echo '<TABLE border=0 width=98% align=center><tr><td style=padding-top:25px;>'._('Please select the date range :').'</td><TD valign=middle style=padding-top:25px;>';
	#$date=date("Y-m-d");
	$date=''; // 2009-04-08
	echo ''._('From :').' </TD><TD valign=middle>';
	...
	echo '</TD><TD valign=middle style=padding-top:25px;>'._('To :').' </TD><TD valign=middle>';
	#DrawHeader(PrepareDateGoal_End($date,'_date',false,array('submit'=>true)));
	DrawHeader(PrepareDate($end_date,'_end'));
	echo '</TD><TD valign=middle style=padding-top:22px;><input type="checkbox" name="chk_pro" id="chk_pro" value="Y" /> '._('With Progress').'';
	echo '</TD></TR></TABLE>';
