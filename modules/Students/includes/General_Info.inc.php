...
if(($_REQUEST['month_values'] && ($_POST['month_values'] || $_REQUEST['ajax'])) || ($_REQUEST['values']['student_enrollment'] && ($_POST['values']['student_enrollment'] || $_REQUEST['ajax'])))
{
	...
	else
	{
		...
		if(count($found_RET))
		{
			...
			unset($_REQUEST['month_values']['student_enrollment']['new']);
			unset($_REQUEST['year_values']['student_enrollment']['new']);
			echo ErrorMessage(array(''._('The student is already enrolled on that date, and could not be enrolled a second time on the date you specified.  Please fix, and try enrolling the student again.').''));
		}
	}
        ...
        if($_REQUEST['values']['student_enrollment'])
        {
            ...
            else 
            {
                if($_REQUEST['values'][$table][$id][$column]!='' && strtotime($_REQUEST['values'][$table][$id][$column])>=strtotime($start_date))
                {
                    ...
                    {
                     $error=date('F',strtotime($e_date)).' '._('has').' '.$num_days.' '._('days').'';  
                    }
                    else 
                    {
                      unset($error);  
                ...
                {
                    $sql='SELECT ID,COURSE_ID,COURSE_PERIOD_ID,MARKING_PERIOD_ID FROM schedule WHERE STUDENT_ID=\''.UserStudentID().'\' AND SYEAR=\''.UserSyear().'\' AND SCHOOL_ID=\''.UserSchool().'\' AND DROPPED=\'N\'';
                    ...
                    { 
                        ...
                        {
                            echo "<p align='center'><b style='color:red'>"._('Student cannot be dropped because student has got attendance till')." ".date('m-d-Y',strtotime($max_at_dt))."</b></p>";
                        }
                    ...
                    else 
                    {
                        ...
                        if(strtotime($get_details[1]['START_DATE'])>=strtotime($_REQUEST['values'][$table][$id][$column]))
                            echo "<p align='center'><b style='color:red'>"._('Student drop date cannot be before student enrollment date')." </b></p>";
                        else
                        SaveData($iu_extra,'',$field_names);
                    }
                    }
                ...
                {
                     echo "<p align='center'><b style='color:red'>"._('Please enter proper drop date.Drop date must be greater than student enrollment date.')."</b></p>";
                }
            }
        }
...
$columns = array('START_DATE'=>_('Start Date'),'ENROLLMENT_CODE'=>_('Enrollment Code'),'END_DATE'=>_('Drop Date'),'DROP_CODE'=>_('Drop Code'),'SCHOOL_ID'=>_('School'));

$schools_RET = DBGet(DBQuery('SELECT ID,TITLE FROM schools WHERE ID!=\''.UserSchool().'\''));
$next_school_options = array(UserSchool()=>_('Next grade at current school'),'0'=>'Retain','-1'=>_('Do not enroll after this school year'));
...
$ethnic_option = array('White, Non-Hispanic'=>''._('White, Non-Hispanic').'','Black, Non-Hispanic'=>''._('Black, Non-Hispanic').'','Hispanic'=>''._('Hispanic').'','American Indian or Native Alaskan'=>''._('American Indian or Native Alaskan').'','Pacific Islander'=>''._('Pacific Islander').'','Asian'=>''._('Asian').'','Indian'=>''._('Indian').'','Middle Eastern'=>''._('Middle Eastern').'','African'=>''._('African').'','Mixed Race'=>''._('Mixed Race').'','Other'=>''._('Other').'');

$language_option = array('English'=>''._('English').'','Arabic'=>''._('Arabic').'','Bengali'=>''._('Bengali').'','Chinese'=>''._('Chinese').'','French'=>''._('French').'','German'=>''._('German').'','Haitian Creole'=>''._('Haitian Creole').'','Hindi'=>''._('Hindi').'','Italian'=>''._('Italian').'','Japanese'=>''._('Japanese').'','Korean'=>''._('Korean').'','Malay'=>''._('Malay').'','Polish'=>''._('Polish').'','Portuguese'=>''._('Portuguese').'','Russian'=>''._('Russian').'','Somali'=>''._('Somali').'','Spanish'=>''._('Spanish').'','Thai'=>''._('Thai').'','Turkish'=>''._('Turkish').'','Urdu'=>''._('Urdu').'','Vietnamese'=>''._('Vietnamese').'');


echo '<TABLE width=100% border=0 cellpadding=3>';
echo '<TR><td height="30px" colspan=2 class=hseparator><b>'._('Demographic Information').'</b></td></tr>';
echo '<TR><td valign="top">';
echo '<TABLE border=0>';
echo '<tr><td style=width:120px>'._('Name').'</td><td>:</td><td>';
if($_REQUEST['student_id']=='new')
...
echo'</td></tr>';
echo '<tr><td>'._('Estimated Grad. Date').' </td><td>:</td><td>'.DateInput($student['ESTIMATED_GRAD_DATE'],'students[ESTIMATED_GRAD_DATE]','').'</td></tr>';
echo '<tr><td>'._('Gender').'<font color=red>*</font></td><td>:</td><td>'.SelectInput($student['GENDER'],'students[GENDER]','',array('Male'=>'Male','Female'=>'Female'),'N/A','').'</td></tr>';
echo '<tr><td>'._('Ethnicity').'<font color=red>*</font></td><td>:</td><td>'.SelectInput($student['ETHNICITY'],'students[ETHNICITY]','',$ethnic_option,'N/A','').'</td></tr>';
echo '<tr><td>'._('Common Name').'</td><td>:</td><td>'.TextInput($student['COMMON_NAME'],'students[COMMON_NAME]','','size=10 class=cell_medium maxlength=10').'</td></tr>';
echo '<tr><td>'._('Birthdate').'<font color=red>*</font></td><td>:</td><td>'.DateInput($student['BIRTHDATE'],'students[BIRTHDATE]','').'</td></tr>';
echo '<tr><td>'._('Primary Language').'</td><td>:</td><td>'.SelectInput($student['LANGUAGE'],'students[LANGUAGE]','',$language_option,'N/A','').'</td></tr>';

echo '<tr><td>'._('Email').'</td><td>:</td><td>'.TextInput($student['EMAIL'],'students[EMAIL]','','size=100 class=cell_medium maxlength=100').'</td></tr>';
echo '<tr><td>'._('Phone').'</td><td>:</td><td>'.TextInput($student['PHONE'],'students[PHONE]','','size=100 class=cell_medium maxlength=100').'</td></tr>';
...
if($_REQUEST['student_id']!='new' && $StudentPicturesPath && (($file = @fopen($picture_path=$StudentPicturesPath.'/'.UserStudentID().'.JPG','r')) || ($file = @fopen($picture_path=$StudentPicturesPath.'/'.UserStudentID().'.JPG','r'))))
{
	...
	echo '<br><a href=Modules.php?modname=Students/Upload.php?modfunc=edit style="text-decoration:none"><b>'._("Update Student's Photo").'</b></a></div>';
	else
	echo '';
}
else
{
	...
	if(User('PROFILE')=='admin' && User('PROFILE')!='student' && User('PROFILE')!='parent')
	echo '<br><a href=Modules.php?modname=Students/Upload.php style="text-decoration:none"><b>'._("Upload Student's Photo").'</b></a></div>';
	}
	else
	echo '';
}
...
echo '<TABLE border=0>';
echo '<tr><td>'._('Student ID').'</td><td>:</td><td>';
if($_REQUEST['student_id']=='new')
{
echo NoInput(''._('Will automatically be assigned').'','');
...
echo '<tr><td>'._('Alternate ID').'</td><td>:</td><td>';
...
echo'</td></tr>';
echo '<tr><td>'._('Grade').'<font color=red>*</font></td><td>:</td><td>';
...
echo'</td></tr>';
echo '<tr><td>'._('Calendar').'</td><td>:</td><td>'.SelectInput($calendar,"values[student_enrollment][$id][CALENDAR_ID]",(!$calendar||!$div?'':'').''.(!$calendar||!$div?'':''),$calendar_options,false,'',$div).'</td></tr>';
echo '<tr><td>'._("Rolling").'/'._("Retention Options").'</td><td>:</td><td>'.SelectInput($next_school,"values[student_enrollment][$id][NEXT_SCHOOL]",(!$next_school||!$div?'':'').''.(!$next_school||!$div?'':''),$next_school_options,false,'',$div).'</td></tr>';
echo'</table>';
echo '</td></TR>';
echo '<TR><td height="30px" colspan=2 class=hseparator><b>'._('Access Information').'</b></td></tr><tr><td colspan="2">';
echo '<TABLE border=0>';
echo '<tr><td style=width:120px>'._('Username').'</td><td>:</td><td>';
...
echo'</td></tr>';
echo '<tr><td>'._('Password').'</td><td>:</td><td>';
...
echo'</table>';
echo '</td></TR>';
echo '<TR><td height="30px" colspan=2 class=hseparator><b>'._('Enrollment Information').'</b></td></tr><tr><td colspan="2">';
echo '</td></tr></table>';
...
if($_REQUEST['student_id']!='new')
{
	if(count($RET))
		...
		$id = 'new';
		ListOutput($RET,$columns,_('Enrollment Record'),_('Enrollment Records'),$link);
		if($id!='new')
		$next_school = $RET[count($RET)]['NEXT_SCHOOL'];
	if($id!='new')
		$calendar = $RET[count($RET)]['CALENDAR_ID'];
...
{
 	$id = 'new';
	ListOutputMod($RET,$columns,_('Enrollment Record'),_('Enrollment Records'),$link,array(),array('count'=>false));
	$next_school = UserSchool();
	$calendar = $calendars_RET[1]['CALENDAR_ID'];
	$div = false;
}
