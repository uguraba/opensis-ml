...
$next_school_options = array(UserSchool()=>''._('Next grade at current school').'','0'=>''._('Retain'),'-1'=>''.('Do not enroll after this school year').'');
...
echo '<CENTER><TABLE><TR><TD>'.SelectInput($calendar,"values[student_enrollment][$id][CALENDAR_ID]",(!$calendar||!$div?'<FONT color=red>':'').''._('Calendar').''.(!$calendar||!$div?'</FONT>':''),$calendar_options,false,'',$div).'</TD><TD width=30></TD><TD>'.SelectInput($next_school,"values[student_enrollment][$id][NEXT_SCHOOL]",(!$next_school||!$div?'<FONT color=red>':'').''._('Rolling / Retention Options').''.(!$next_school||!$div?'</FONT>':''),$next_school_options,false,'',$div).'</TD></TR></TABLE></CENTER>';
