...
if((!clean_param($_REQUEST['modfunc'],PARAM_NOTAGS) || clean_param($_REQUEST['modfunc'],PARAM_NOTAGS)=='choose_course') && !clean_param($_REQUEST['course_modfunc'],PARAM_NOTAGS))
{
	...
	{
		if(AllowEdit())
			$delete_button = "<INPUT type=button class=btn_medium value="._('Delete')." onClick='javascript:window.location=\"Modules.php?modname=Students/Student.php&include=Goal&modfunc=delete&goal_id=$_REQUEST[goal_id]&progress_id=$_REQUEST[progress_id]&course_period_id=$_REQUEST[course_period_id]\"'> ";
		// ADDING & EDITING FORM
...
#echo isset($edit_per_prnt[1]['CAN_EDIT']);
	...
	{	
		...
		{
			$header .= '<TR>';
			$header .= "<TD>&nbsp;</TD><TD align=right><a href='Modules.php?modname=Students/Student.php&include=Goal&category_id=5&action=delete&gid=".$row_gid[0]."&pid=".$_REQUEST[progress_id]."'>"._('Delete This Progress')."</a></TD>"; // DateInput is copied from School_Setup/MarkingPeriods.php line 295
			$header .= '</TR>';
		}
	}
			...
			$header .= '<TR>';
			$header .= '<TD>'._('Goal Title').'</TD><TD>' . SelectInput($RET['GOAL_ID']?$RET['GOAL_ID']:$_REQUEST['goal_id'],'tables[progress]['.$_REQUEST['progress_id'].'][GOAL_ID]','',$options,false) . '</TD>'; // DateInput is copied from School_Setup/MarkingPeriods.php line 295
			$header .= '</TR>';
			
			
			$header .= '<TR>';
			$header .= '<TD>'._('Course Period').'</TD><TD>' . SelectInput($RET_sel['COURSE_PERIOD_ID'],'tables[progress]['.$_REQUEST['progress_id'].'][COURSE_PERIOD_ID]','',$options_cp) . '</TD>'; // DateInput is copied from School_Setup/MarkingPeriods.php line 295
			$header .= '</TR>';
			...
			$header .= '<TR>';
			$header .= '<TD>'._('Date of Entry').'<input type="hidden" name="hgoal" value="'.$_REQUEST[goal_id].'" /></TD><TD>' . DateInput($RET['START_DATE'],'tables['.$_REQUEST['progress_id'].'][START_DATE]','') . '</TD>'; // DateInput is copied from School_Setup/MarkingPeriods.php line 295
			$header .= '</TR>';
			$header .= '<TR>';
			$header .= '<TD>'._('Progress Period').' '._('Name').'</TD><TD>' . TextInput($RET['PROGRESS_NAME'],'tables[progress]['.$_REQUEST['progress_id'].'][PROGRESS_NAME]','','size=60 maxlength=50') . '</TD>';
			$header .= '</TR>';
			$header .= '<TR>';
			...
			$header .= '<TD>'._('Proficiency Scale').'</TD><TD>' . SelectInput($RET['PROFICIENCY'],'tables[progress]['.$_REQUEST['progress_id'].'][PROFICIENCY]','',$options) . '</TD>';
			$header .= '</TR>';
			$header .= '<TR>';
			$header .= '<TD valign=top>'._('Progress Assessment').'</TD><TD>' . TextAreaInput($RET['PROGRESS_DESCRIPTION'],'tables[progress]['.$_REQUEST['progress_id'].'][PROGRESS_DESCRIPTION]','', 'rows=10 cols=57', 'true', '200px') . '<input type="hidden" name="tabl" id="tabl" value="progress"></TD>';
			$header .= '</TR>';
			$header .= '</TABLE>';
			DrawHeader($header);
...
$edit_per_prnt = DBGet(DBQuery('SELECT CAN_EDIT FROM profile_exceptions WHERE MODNAME=\'Students/Student.php&category_id=5\' AND PROFILE_ID=3'));
	...
	{	
		if(((User('PROFILE') == 'admin') && isset($edit_per_adm[1]['CAN_EDIT'])) || ((User('PROFILE') == 'teacher') && isset($edit_per_teach[1]['CAN_EDIT'])) || ((User('PROFILE') == 'student') && isset($edit_per_stu[1]['CAN_EDIT'])) || ((User('PROFILE') == 'parent') && isset($edit_per_prnt[1]['CAN_EDIT'])))
		{
			...
			$header .= "<TD>&nbsp;</TD><TD align=right><a href='Modules.php?modname=Students/Student.php&include=Goal&category_id=5&action=delete_goal&gid=".$_REQUEST['goal_id']."'>"._('Delete')." "._('This Goal')."</a></TD>"; // DateInput is copied from School_Setup/MarkingPeriods.php line 295
			$header .= '</TR>';
		}
	}
			...
			$header .= '<TR>';
			$header .= '<TD>'._('Goal Title').' </TD><TD>' . TextInput($RET['GOAL_TITLE'],'tables[goal]['.$_REQUEST['goal_id'].'][GOAL_TITLE]','','size=75 maxlength=50') . '</TD>';
			$header .= '</TR>';
			$header .= '<TR>';
			$header .= '<TD>'._('Begin Date').'</TD><TD>' . DateInput($RET['START_DATE'],'tables['.$_REQUEST['goal_id'].'][START_DATE]','') . '</TD>'; // DateInput is copied from School_Setup/MarkingPeriods.php line 295
			$header .= '</TR>';
			$header .= '<TR>';
			$header .= '<TD>'._('End Date').'</TD><TD>' . DateInput($RET['END_DATE'],'tables['.$_REQUEST['goal_id'].'][END_DATE]','') . '</TD>'; // DateInput is copied from School_Setup/MarkingPeriods.php line 296
			$header .= '</TR>';
			$header .= '<TR>';
			$header .= '<TD valign=top>'._('Goal Description').'</TD><TD>' . TextAreaInput($RET['GOAL_DESCRIPTION'],'tables[goal]['.$_REQUEST['goal_id'].'][GOAL_DESCRIPTION]','', 'rows=10 cols=70', 'true', '200px') . '<input type="hidden" name="tabl" id="tabl" value="goal"></TD>';
			$header .= '</TR>';
			$header .= '</TABLE>';
			DrawHeader($header);
		}
	...
	DrawHeaderHome(''._('Goals').'',"<A HREF=for_window.php?modname=Students/Student.php&include=Goal&modfunc=$_REQUEST[modfunc]&course_modfunc=search>"._('Search')."</A>");
	...
	echo '<TD valign=top>';
	$columns = array('GOAL_TITLE'=>''._('Goals').'');
	$link = array();
	...
	ListOutput($subjects_RET,$columns,''._('Goal').'',''._('Goals').'',$link,array(),$LO_options);
	echo '</TD>';
	...
	{       
		...
		echo '<TD valign=top>';
		$columns = array('PROGRESS_NAME'=>''._('Progresses').'');
		$link = array();
		...
		*/
		...
		ListOutput($courses_RET,$columns,''._('Progress').'',''._('Progresses').'',$link,array(),$LO_options);
		echo '</TD>';
	}
