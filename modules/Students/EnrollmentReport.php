...
if($_REQUEST['modfunc']=='save')
{
 ...
 else
 {
	...
	$st_list = '\''.implode('\',\'',$_REQUEST['st_arr']).'\'';
        ...
        if(count($RET))
	{
            $columns = array('START_DATE'=>''._('Start Date').'','ENROLLMENT_CODE'=>''._('Enrollment Code').'','END_DATE'=>''._('Drop Date').'','DROP_CODE'=>''._('Drop Code').'','SCHOOL_ID'=>''._('Last')._(' ')._('School').'');		$handle = PDFStart();
		foreach($RET as $student_id=>$value)
		{
			echo "<meta charset='UTF-8'><table width=100%  style=\" font-family:Arial; font-size:12px;\" >";
			echo "<meta charset='UTF-8'><tr><td width=105>".DrawLogo()."</td><td  style=\"font-size:15px; font-weight:bold; padding-top:20px;\">". GetSchool(UserSchool()).' ('.$cur_session.')'."<div style=\"font-size:12px;\">"._('Student Enroll Report')."</div></td><td align=right style=\"padding-top:20px\">". ProperDate(DBDate()) ."<br \>"._('Powered by openSIS')."</td></tr><tr><td colspan=3 style=\"border-top:1px solid #333;\">&nbsp;</td></tr></table>";
...
if(!$_REQUEST['modfunc'])
{
	...
	if($_REQUEST['search_modfunc']=='list')
	{
		...
		echo '<BR><CENTER><INPUT type=submit class=btn_xxlarge value=\''._('Create Enrollment Report for Selected Students').'\'></CENTER>';
		echo "</FORM>";
	}
}
...
{
	return '<INPUT type=checkbox name=st_arr[] value='.''._($value).''.' checked>';
}
