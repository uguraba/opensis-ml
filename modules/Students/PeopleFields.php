...
if(!$_REQUEST['modfunc'])
{
	...
	if($_REQUEST['id'])
	{
		...
		DrawHeaderHome($title,$delete_button.SubmitButton(_('Save'),'','class=btn_medium onclick="formcheck_student_contactField_F1();"')); //'<INPUT type=submit value=Save>');
		...
		$header .= '<TD>' . SelectInput($RET['CATEGORY_ID']?$RET['CATEGORY_ID']:$_REQUEST['category_id'],'tables['.$_REQUEST['id'].'][CATEGORY_ID]',''._('Contact Field Category').'',$categories_options,false) . '</TD>';

		$header .= '<TD>' . TextInput($RET['SORT_ORDER'],'tables['.$_REQUEST['id'].'][SORT_ORDER]',''._('Sort Order').'') . '</TD>';
	...
	elseif($_REQUEST['category_id'])
	{
		...
		echo " method=POST>";
		DrawHeaderHome($title,$delete_button.SubmitButton(_('Save'),'','class=btn_medium onclick="formcheck_student_contactField_F2();"')); //'<INPUT type=submit value=Save>');
		$header .= '<TABLE cellpadding=3 width=100%>';
		$header .= '<TR>';
		...
		if($_REQUEST['category_id']=='new')
			$new = true;
		$header .= '<TD><TABLE><TR>';
		$header .= '<TD>' . CheckboxInput($RET['CUSTODY'],'tables['.$_REQUEST['category_id'].'][CUSTODY]',_('Custody'),'',$new,'<IMG SRC=assets/check.gif height=15 vspace=0 hspace=0 border=0>','<IMG SRC=assets/x.gif height=15 vspace=0 hspace=0 border=0>') . '</TD>';
		$header .= '<TD>' . CheckboxInput($RET['EMERGENCY'],'tables['.$_REQUEST['category_id'].'][EMERGENCY]',_('Emergency'),'',$new,'<IMG SRC=assets/check.gif height=15 vspace=0 hspace=0 border=0>','<IMG SRC=assets/x.gif height=15 vspace=0 hspace=0 border=0>') . '</TD>';
		$header .= '</TR><TR>';
...
//	$link['add']['link'] = "Modules.php?modname=$_REQUEST[modname]&category_id=new";
	...
	ListOutput($categories_RET,$columns,_('Contact Field Category'),_('Contact Field Categories'),$link,array(),$LO_options);
	echo '</TD>';
	...
	if($_REQUEST['category_id'] && $_REQUEST['category_id']!='new' && count($categories_RET))
	{
		...
		echo '<TD valign=top>';
		$columns = array('TITLE'=>_('Contact Field'),'SORT_ORDER'=>_('Order'),'TYPE'=>_('Data Type'));
		$link = array();
	...
	//	$link['add']['link'] = "Modules.php?modname=$_REQUEST[modname]&category_id=$_REQUEST[category_id]&id=new";
		...
		ListOutput($fields_RET,$columns,_('Contact Field'),_('Contact Fields'),$link,array(),$LO_options);
...
{
	$options = array('radio'=>_('Checkbox'),'text'=>_('Text'),'autos'=>_('Auto Pull-Down'),'edits'=>_('Edit Pull-Down'),'select'=>_('Pull-Down'),'codeds'=>_('Coded Pull-Down'),'date'=>_('Date'),'numeric'=>_('Number'),'textarea'=>_('Long Text'),'multiple'=>_('Select Multiple'));	return $options[$value];
}
