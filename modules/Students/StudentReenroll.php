...
if(clean_param($_REQUEST['modfunc'],PARAM_ALPHA)=='save')
{
    ...
    if($_REQUEST['student'])
    {
        ...
        $enroll_msg=''._("Selected students are successfully re enrolled").'';
    }
    else
    {
        $err="<b><font color=red>"._('No students are selected.')."</font></b>";    
    }
unset($_REQUEST['modfunc']);
	...
	if($_REQUEST['search_modfunc']=='list')
	{
		...
		echo '<TABLE><TR><TD>'._('Start Date').' <font color="red">*</font> </TD><TD>: </TD><TD>'.PrepareDate(DBDate(),'').'</TD></TR>';
		...
		echo '<SELECT name=grade_id id=grade_id><OPTION value="">'._('Select')._(' ')._('Grade').'</OPTION>';
		foreach($sel_grade as $g_id)
			echo "<OPTION value=$g_id[ID]>".$g_id['TITLE'].'</OPTION>';
		echo '</SELECT>';
		echo '</TD></TR>';
                
                echo '<TR><TD>'._('Enrollment Code').' <font color="red">*</font> </TD><TD>: </TD><TD>';
		$enroll_code = DBGet(DBQuery('SELECT TITLE,ID FROM student_enrollment_codes WHERE SYEAR=\''.UserSyear().'\' AND TYPE IN (\''.Add.'\',\''.TrnE.'\',\''.Roll.'\')'));
		echo '<SELECT name=en_code id=en_code><OPTION value="">'._('Select Enroll Code').'</OPTION>';
		foreach($enroll_code as $enr_code)
			echo "<OPTION value=$enr_code[ID]>"._($enr_code['TITLE']).'</OPTION>';
		echo '</SELECT>';
		echo '</TD></TR>';
		echo '</TABLE>';
		PopTable ('footer');
...
if(!$_REQUEST['modfunc'])
{
	...
	if($_REQUEST['search_modfunc']=='list')
	{
		...
		echo '<BR><CENTER>'.SubmitButton(""._('Re Enroll Selected Students')."",'','class=btn_re_enroll onclick=\'return reenroll();\'').'</CENTER>';
		echo "</FORM>";
	}
