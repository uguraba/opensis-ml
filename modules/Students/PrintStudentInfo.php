		...
		foreach($RET as $student)
		{
			$_SESSION['student_id'] = $student['STUDENT_ID'];
                                                        echo "<meta charset='UTF-8'><table width=100% style=\" font-family:Arial; font-size:12px;\" >";
                                                                                echo "<tr><td width=105>".DrawLogo()."</td><td  style=\"font-size:15px; font-weight:bold; padding-top:20px;\">". GetSchool(UserSchool())."<div style=\"font-size:12px;\">"._('Student Information Report')."</div></td><td align=right style=\"padding-top:20px;\">". ProperDate(DBDate()) ."<br />"._('Powered by openSIS')."</td></tr><tr><td colspan=3 style=\"border-top:1px solid #333;\">&nbsp;</td></tr></table>";

                                                        echo "<meta charset='UTF-8'><table cellspacing=0  border=\"0\" style=\"border-collapse:collapse\">";
                                                        ...
                                                        echo "<table width=100% ><tr><td colspan=2 style=\"border-bottom:1px solid #333;  font-size:14px;  font-weight:bold;\">"._('Personal Information')."</td></tr>";

                                                    if($_REQUEST['category']['1'])
                                                    {
			 
			//----------------------------------------------
			echo "<tr><td width=45% style='font-weight:bold'>"._('Student Name').":</td>";
			echo "<td width=55%>" .$student['FULL_NAME']. "</td></tr>";
			echo "<tr><td style='font-weight:bold'>"._('ID').":</td>";
			echo "<td>". $student['STUDENT_ID'] ." </td></tr>";
			if($student['ALT_ID']!='')
			{
				echo "<tr><td style='font-weight:bold'>"._('Alt ID').":</td>";
				echo "<td>". $student['ALT_ID'] ." </td></tr>";
			}
			echo "<tr><td style='font-weight:bold'>"._('Grade').":</td>";
			echo "<td>". $student['GRADE_ID'] ." </td></tr>";
			echo "<tr><td style='font-weight:bold'>"._('Gender').":</td>";
			echo "<td>".$sql['GENDER'] ."</td></tr>";
			echo "<tr><td style='font-weight:bold'>"._('Ethnicity').":</td>";
			echo "<td>".$sql['ETHNICITY'] ."</td></tr>";
			if($sql['COMMON_NAME'] !='')
			{
			echo "<tr><td style='font-weight:bold'>"._('Common Name')."</td>";
			echo "<td>".$sql['COMMON_NAME'] ."</td></tr>";
			}
			if($sql['SOCIAL_SEC_NO'] !='')
			{
			echo "<tr><td style='font-weight:bold'>"._('Social Security').":</td>";
			echo "<td>".$sql['SOCIAL_SEC_NO'] ."</td></tr>";
			}
			echo "<tr><td style='font-weight:bold'>"._('Birth Date').":</td>";
			echo "<td>".$sql['BIRTHDAY'] ."</td></tr>";
			if($sql['LANGUAGE'] !='')
			{
			echo "<tr><td style='font-weight:bold'>"._('Language Spoken').":</td>";
			echo "<td>".$sql['LANGUAGE'] ."</td></tr>";
			
			}
                        if($sql['EMAIL'] !='')
			{
			echo "<tr><td style='font-weight:bold'>"._('Email ID').":</td>";
			echo "<td>".$sql['EMAIL'] ."</td></tr>";
			
			}
                        if($sql['PHONE'] !='')
			{
			echo "<tr><td style='font-weight:bold'>"._('Phone').":</td>";
			...
			$rolling = _('Do not enroll after this school year');
  ...
  if($_REQUEST['category']['3'])
   {
      $addresses_RET = DBGet(DBQuery('SELECT a.ADDRESS_ID,             sjp.STUDENT_RELATION,a.ADDRESS,a.STREET,a.CITY,a.STATE,a.ZIPCODE,a.PHONE,a.MAIL_ADDRESS,a.MAIL_STREET,a.MAIL_CITY,a.MAIL_STATE,a.MAIL_ZIPCODE,    sjp.CUSTODY,sja.MAILING,sja.RESIDENCE\''.$address_custom.'\' FROM address a,students_join_address sja,students_join_people sjp WHERE a.ADDRESS_ID=sja.ADDRESS_ID AND sja.STUDENT_ID=\''.UserStudentID().'\' AND a.ADDRESS_ID=sjp.ADDRESS_ID AND sjp.STUDENT_ID=sja.STUDENT_ID
			...
			foreach($addresses_RET as $address)
			{
				...
				{
				echo "<table width=100%><tr><td colspan=2 style=\"border-bottom:1px solid #333;  font-size:14px; font-weight:bold;\">"._('Home Address')."</td></tr>";
				echo "<tr><td width=45% style='font-weight:bold'>"._('Address1').":</td>";
				echo "<td width=55%>".$address['ADDRESS']."</td></tr>";
				if($address['STREET']!='')
     			{
				echo "<tr><td width=35% style='font-weight:bold'>"._('Address2').":</td>";
				echo "<td width=65%>".$address['STREET']."</td></tr>";
				}
				echo "<tr><td style='font-weight:bold'>"._('City').":</td>";
				echo"<td>".($address['CITY']?$address['CITY'].'':'')."</td></tr>";
				echo "<tr><td style='font-weight:bold'>"._('State').":</td>";
				echo"<td>".$address['STATE']."</td></tr>";
				echo "<tr><td style='font-weight:bold'>"._('Zipcode').":</td>";
				echo"<td>".($address['ZIPCODE']?$address['ZIPCODE'].'':'')."</td></tr>";
				echo "</table>";

				echo "</td><td></td><td valign=top width=300>";
				echo "<table width=100%><tr><td colspan=2 style=\"border-bottom:1px solid #333;  font-size:14px;  font-weight:bold;\">"._('Mailing Address')."</td></tr>";
				echo "<tr><td width=45% style='font-weight:bold'>"._('Address1').":</td>";
	...
	foreach($address_categories_RET as $categories)
			{
			   if(!$categories[1]['RESIDENCE']&&!$categories[1]['MAILING']&&!$categories[1]['BUS'] || $categories[1]['RESIDENCE']=='Y'&&$address['RESIDENCE']=='Y' || $categories[1]['MAILING']=='Y'&&$address['MAILING']=='Y' || $categories[1]['BUS']=='Y'&&($address['BUS_PICKUP']=='Y'||$address['BUS_DROPOFF']=='Y'))
				...
				echo "<table width=100% border=0><tr><td colspan=2 style=\"border-bottom:1px solid #333;  font-size:14px;  font-weight:bold;\">"._('Primary Emergency Contact')."</td></tr>";
					...
					foreach($contacts_RET as $contact)
					{
					  	...
					  	echo "<tr><td width=45% style='font-weight:bold'>"._('Relation').":</td><td width=55%>".$contact['PRIM_STUDENT_RELATION']."</td></tr>";

						echo "<tr><td style='font-weight:bold'>"._('First Name').":</td><td>".$contact['PRI_FIRST_NAME']."</td></tr>";
						echo "<tr><td style='font-weight:bold'>"._('Last Name').":</td><td>".$contact['PRI_LAST_NAME']."</td></tr>";
					   if($contact['HOME_PHONE']!='')
						{
						     echo "<tr><td style='font-weight:bold'>"._('Home Phone').":</td><td>".$contact['HOME_PHONE']."</td></tr>";
						 }
						 if($contact['WORK_PHONE']!='')
						{
						     echo "<tr><td style='font-weight:bold'>"._('Work Phone').":</td><td>".$contact['WORK_PHONE']."</td></tr>";
						 }
						 if($contact['MOBILE_PHONE']!='')
						{
						     echo "<tr><td style='font-weight:bold'>"._('Mobile Phone').":</td><td>".$contact['MOBILE_PHONE']."</td></tr>";
						 }
						 if($contact['EMAIL']!='')
						{
						     echo "<tr><td style='font-weight:bold'>"._('Email').":</td><td>".$contact['EMAIL']."</td></tr>";
						 }

						 if($contact['PRIM_ADDRESS']!='')
						{
						     echo "<tr><td style='font-weight:bold'>"._('Address1').":</td><td>".$contact['PRIM_ADDRESS']."</td></tr>";
						 }

						 if($contact['PRIM_STREET']!='')
						{
						     echo "<tr><td style='font-weight:bold'>"._('Address2').":</td><td>".$contact['PRIM_STREET']."</td></tr>";
						 }

						 if($contact['PRIM_CITY']!='')
						{
						     echo "<tr><td style='font-weight:bold'>"._('City').":</td><td>".$contact['PRIM_CITY']."</td></tr>";
						 }

						 if($contact['PRIM_STATE']!='')
						{
						     echo "<tr><td style='font-weight:bold'>"._('State').":</td><td>".$contact['PRIM_STATE']."</td></tr>";
						 }

						 if($contact['PRIM_ZIPCODE']!='')
						{
						     echo "<tr><td style='font-weight:bold'>"._('Zipcode').":</td><td>".$contact['PRIM_ZIPCODE']."</td></tr>";
						 }
						echo "</table>";
...
echo "</td><td></td><td valign=top>";
						...
						echo "<table width=100% border=0><tr><td colspan=2 style=\"border-bottom:1px solid #333;  font-size:14px;  font-weight:bold;\">"._('Secondary Emergency Contact')."</td></tr>";

					  	echo "<tr><td width=45% style='font-weight:bold'>"._('Relation').":</td><td width=55%>".$contact['SEC_STUDENT_RELATION']."</td></tr>";	     			echo "<tr><td style='font-weight:bold'>First Name :</td><td>".$contact['SEC_FIRST_NAME']."</td></tr>";
						echo "<tr><td style='font-weight:bold'>"._('Last Name').":</td><td>".$contact['SEC_LAST_NAME']."</td></tr>";
					   if($contact['SEC_HOME_PHONE']!='')
						{
						     echo "<tr><td style='font-weight:bold'>"._('Home Phone').":</td><td>".$contact['SEC_HOME_PHONE']."</td></tr>";
						 }
						 if($contact['SEC_WORK_PHONE']!='')
						{
						     echo "<tr><td style='font-weight:bold'>"._('Work Phone').":</td><td>".$contact['SEC_WORK_PHONE']."</td></tr>";
						 }
						 if($contact['SEC_MOBILE_PHONE']!='')
						{
						     echo "<tr><td style='font-weight:bold'>"._('Mobile Phone').":</td><td>".$contact['SEC_MOBILE_PHONE']."</td></tr>";
						 }
						 if($contact['SEC_EMAIL']!='')
						{
						     echo "<tr><td style='font-weight:bold'>"._('Email').":</td><td>".$contact['SEC_EMAIL']."</td></tr>";
						 }

						 if($contact['SEC_ADDRESS']!='')
						 {
						     echo "<tr><td style='font-weight:bold'>"._('Address1').":</td><td>".$contact['SEC_ADDRESS']."</td></tr>";
						  }

						  if($contact['SEC_STREET']!='')
						  {
						     echo "<tr><td style='font-weight:bold'>"._('Address2').":</td><td>".$contact['SEC_STREET']."</td></tr>";
						   }

						  if($contact['SEC_CITY']!='')
						  {
						     echo "<tr><td style='font-weight:bold'>"._('City').":</td><td>".$contact['SEC_CITY']."</td></tr>";
						   }

						    if($contact['SEC_STATE']!='')
						   {
						     echo "<tr><td style='font-weight:bold'>"._('State').":</td><td>".$contact['SEC_STATE']."</td></tr>";
						    }

						   if($contact['SEC_ZIPCODE']!='')
						   {
						     echo "<tr><td style='font-weight:bold'>"._('Zipcode').":</td><td>".$contact['SEC_ZIPCODE']."</td></tr>";
						    }
...
$info_RET = DBGet(DBQuery("SELECT p.FIRST_NAME,p.MIDDLE_NAME,p.LAST_NAME,sjp.STUDENT_RELATION,sjp.ADDN_HOME_PHONE,sjp.ADDN_WORK_PHONE,sjp.ADDN_MOBILE_PHONE,sjp.ADDN_EMAIL,sjp.ADDN_ADDRESS,sjp.ADDN_STREET,sjp.ADDN_CITY,sjp.ADDN_STATE,sjp.ADDN_ZIPCODE FROM people p,students_join_people sjp WHERE p.PERSON_ID=sjp.PERSON_ID AND sjp.STUDENT_ID='".UserStudentID()."'"));
					...
					if($info_RET[1]['STUDENT_RELATION']!='')
					{
					  ...
					  echo "<tr><td colspan=2 style=\"border-bottom:1px solid #333;  font-size:14px; font-weight:bold;\">"._('Additional Contact')."</td></tr>";
						foreach($info_RET as $info)
						{

						 echo "<tr><td width=45% style='font-weight:bold'>"._('Relation').":</td><td width=55%>".$info['STUDENT_RELATION']."</td></tr>";
						 echo "<tr><td style='font-weight:bold'>"._('First Name').":</td><td>".$info['FIRST_NAME']."</td></tr>";
						 if($info['MIDDLE_NAME']!='')
						 {
					   		 echo "<tr><td style='font-weight:bold'>"._('Middle Name').":</td><td>".$info['MIDDLE_NAME']."</td></tr>";
					   	 }
					  	echo "<tr><td style='font-weight:bold'>"._('Last Name').":</td><td>".$info['LAST_NAME']."</td></tr>";
					    if($info['ADDN_HOME_PHONE']!='')
						 {
						     echo "<tr><td style='font-weight:bold'>"._('Home Phone').":</td><td>".$info['ADDN_HOME_PHONE']."</td></tr>";
						 }
						 if($info['ADDN_WORK_PHONE']!='')
						 {
						     echo "<tr><td style='font-weight:bold'>"._('Work Phone').":</td><td>".$info['ADDN_WORK_PHONE']."</td></tr>";
						 }
						 if($info['ADDN_MOBILE_PHONE']!='')
						 {
						    echo "<tr><td style='font-weight:bold'>"._('Mobile Phone').":</td><td>".$info['ADDN_MOBILE_PHONE']."</td></tr>";
						 }
						 if($info['ADDN_EMAIL']!='')
						 {
						     echo "<tr><td style='font-weight:bold'>"._('Email').":</td><td>".$info['ADDN_EMAIL']."</td></tr>";
						 }

						 if($info['ADDN_ADDRESS']!='')
						 {
						     echo "<tr><td style='font-weight:bold'>"._('Address1').":</td><td>".$info['ADDN_ADDRESS']."</td></tr>";
						 }

						if($info['ADDN_STREET']!='')
						 {
						     echo "<tr><td style='font-weight:bold'>"._('Address2').":</td><td>".$info['ADDN_STREET']."</td></tr>";
						 }

						if($info['ADDN_CITY']!='')
						 {
						     echo "<tr><td style='font-weight:bold'>"._('City').":</td><td>".$info['ADDN_CITY']."</td></tr>";
						 }

						if($info['ADDN_STATE']!='')
						 {
						     echo "<tr><td style='font-weight:bold'>"._('State:').":</td><td>".$info['ADDN_STATE']."</td></tr>";
						 }

						 if($info['ADDN_ZIPCODE']!='')
						 {
						     echo "<tr><td style='font-weight:bold'>"._('Zipcode').":</td><td>".$info['ADDN_ZIPCODE']."</td></tr>";
						 }
...
echo "<tr><td valign=top colspan=3>";
 ...
 #echo '</td><td></td><td></td></tr></table></TABLE><div style="page-break-before: always;">&nbsp;</div>';
			...
			if($_REQUEST['category']['2'] && ($sql['PHYSICIAN_NAME'] !='' || $sql['PHYSICIAN_PHONO'] !='' || $sql['HOSPITAL'] !=''))
			{
				...
				echo "<table width='100%'><tr><td style=\"border-bottom:1px solid #333;  font-size:14px; font-weight:bold;\">"._('Medical Information')."</td></tr></table>";
				echo "</td><td></td><td valign=top>";
				echo "</td></tr>";
				echo "<tr><td valign=top colspan=3>";
				echo "<table width='100%'><tr><td colspan=\"2\" style=\"border-bottom:1px solid #9a9a9a; font-weight:bold; color:4a4a4a; font-size:12px;\">"._('General Information')."</td></tr>
				<tr><td colspan=2 style=\"height:5px;\"></td></tr>";
				if($sql['PHYSICIAN_NAME'] !='')
				{
				echo "<tr><td width=21% style='font-weight:bold'>"._('Physician Name').":</td>";
				echo "<td width=79%>".$sql['PHYSICIAN_NAME'] ."</td></tr>";
				}
				if($sql['PHYSICIAN_PHONO'] !='')
				{
				echo "<tr><td style='font-weight:bold'>"._('Physicians Phone').":</td>";
				echo "<td>".$sql['PHYSICIAN_PHONO'] ."</td></tr>";
				}
				if($sql['HOSPITAL'] !='')
				{
				echo "<tr><td style='font-weight:bold'>"._('Hospital Name').":</td>";
				...
				if($medical['MCOMNT'] !='')
				{
				echo "<tr><td valign='top' style='font-weight:bold'>"._('Date').":</td>";
				echo "<td align='justify'>".$medical['MCOMNT']."</td></tr>";
				}
				if($medical['DNOTE'] !='')
				{
				echo "<tr><td valign='top' style='font-weight:bold'>"._("Doctor's Note").":</td>";
 ...
 #############################################################################
			...
			if($_REQUEST['category']['2'] && count($res_immunization) >= 1)
			{
				...
				echo "<table width=100%>
				<tr><td colspan=2 style=\"border-bottom:1px solid #9a9a9a; font-weight:bold; color:4a4a4a; font-size:12px;\">"._('Immunization / Physical Record')."</td></tr>
				<tr><td colspan=2 style=\"height:5px;\"></td></tr>";
				...
				foreach($res_immunization as $row_immunization)
					...
					{
					echo "<tr><td width=21% style='font-weight:bold'>"._('Type').":</td>";
					echo "<td width=79%>".$row_immunization['TYPE'] ."</td></tr>";
					}
					if($row_immunization['MEDICAL_DATE'] !='')
					{
					echo "<tr><td style='font-weight:bold'>"._('Date').":</td>";
					echo "<td>".$row_immunization['MEDICAL_DATE'] ."</td></tr>";
					}
					if($row_immunization['COMMENTS'] !='')
					{
					echo "<tr><td valign='top' style='font-weight:bold'>"._('Comments').":</td>";
...
# ---------------------------------- Medical Alert ---------------- #
			...
			if($_REQUEST['category']['2'] && count($res_alert) >= 1)
			{
				...
				#echo "<br>";
				#echo $_REQUEST['category']['2']!='Y'
				echo "<table width=100%><tr><td colspan=2 style=\"border-bottom:1px solid #9a9a9a; font-weight:bold; color:4a4a4a; font-size:12px;\">"._('Medical Alert')."</td></tr>
				<tr><td colspan=2 style=\"height:5px;\"></td></tr>";
				...
				foreach($res_alert as $row_alert)
				{
					if($row_alert['TITLE'] !='')
					{
					echo "<tr><td width=21% style='font-weight:bold'>"._('Medical Alert').":</td>";
					echo "<td width=79% align='justify'>".$row_alert['TITLE'] ."</td></tr>";
					}
                                                                                        if($row_alert['ALERT_DATE'] !='')
					{
					echo "<tr><td width=21% style='font-weight:bold'>"._('Date').":</td>";
...
# ---------------------------------- Nurse Visit Record ---------------- #
			...
			if($_REQUEST['category']['2'] && count($res_visit) >= 1)
			{
				...
				#echo "<br>";
				#echo $_REQUEST['category']['2']!='Y'
				echo "<table width=100%><tr><td colspan=2 style=\"border-bottom:1px solid #9a9a9a; font-weight:bold; color:4a4a4a; font-size:12px;\">"._('Nurse Visit Record')."</td></tr>
				<tr><td colspan=2 style=\"height:5px;\"></td></tr>";
				...
				foreach($res_visit as $row_visit)
				{
					...
					{
					echo "<tr><td style='font-weight:bold'>"._('Time In').":</td>";
					echo "<td>".$row_visit['TIME_IN'] ."</td></tr>";
					}
					if($row_visit['TIME_OUT'] !='')
					{
					echo "<tr><td style='font-weight:bold'>"._('Time Out').":</td>";
					echo "<td>".$row_visit['TIME_OUT'] ."</td></tr>";
					}
					if($row_visit['REASON'] !='')
					{
					echo "<tr><td style='font-weight:bold'>"._('Reason').":</td>";
					echo "<td>".$row_visit['REASON'] ."</td></tr>";
					}
					if($row_visit['RESULT'] !='')
					{
					echo "<tr><td style='font-weight:bold'>"._('Result').":</td>";
			...
			//$sql_comment = "select * from student_mp_comments where student_id='".$_SESSION['student_id']."'";
                            foreach($res_comment as $row_comment)
                            {
                                if($_REQUEST['category']['4'] && $row_comment['COMMENT'] != '')
                                {
                                        ...
                                        echo "<table width=100%><tr><td colspan=2 style=\"border-bottom:1px solid #333;  font-size:14px; font-weight:bold;\">"._('Comment')."</td></tr>";
                                         if($row_comment['USER_NAME'] !='')
                                        {
                                        echo "<tr><td width=21% valign='top' style='font-weight:bold'>"._('Entered by').":</td>";
                                        echo "<td width=79% align=justify>".$row_comment['USER_NAME'] ."</td></tr>";
                                        }
                                        if($row_comment['COMMENT_DATE'] !='')
                                        {
                                        echo "<tr><td width=21% valign='top' style='font-weight:bold'>"._('Date').":</td>";
                                        echo "<td width=79% align=justify>".$row_comment['COMMENT_DATE'] ."</td></tr>";
                                        }
                                        if($row_comment['COMMENT'] !='')
                                        {
                                        echo "<tr><td width=21% valign='top' style='font-weight:bold'>"._('Comment').":</td>";
				...
				echo "<tr><td colspan=3 valign=top>";
                                echo '<strong>'._('Goals').'</strong>';
                                $res_goal = DBGet(DBQuery("SELECT goal_id AS GOAL,GOAL_TITLE,START_DATE,END_DATE,GOAL_DESCRIPTION FROM goal WHERE student_id='".$_SESSION['student_id']."'"),array('START_DATE'=>'ProperDate','END_DATE'=>'ProperDate'));
                                if($res_goal)
                                {
                                        echo '<table><tr><td>'._('Title').'</td><td>'._('Start date').'</td><td>'._('End date').'</td><td>'._('Description').'</td></tr>';
...
if(!$_REQUEST['modfunc'])
{
	...
	if($_REQUEST['search_modfunc']=='list')
	{
		...
		foreach($categories_RET as $category)
			...
			{
			$extra['extra_header_left'] .= '<TR><TD align="right" style="white-space:nowrap">'._($category['TITLE']).'</td>';
	...
	if($_REQUEST['search_modfunc']=='list')
	{
		echo '<BR><CENTER><INPUT type=submit class=btn_xxlarge value=\''._("Print Info for Selected Students").'\'></CENTER>';
		echo "</FORM>";
	}
}
