...
if($_REQUEST['search_modfunc']=='search_fnc' || !$_REQUEST['search_modfunc'])
{
	...
	switch(User('PROFILE'))
	{
		...
		unset($_SESSION['s']);
			...
			echo '<div id="addiv">';
			echo '<div><label onclick="show_search_div();" style="cursor:pointer; font-weight:bold; font-size:12px;  text-decoration:underline;">'._('Advanced').' '._('Search').'</label></div>';
			echo '</div>';
			
			echo '<div id="searchdiv" style="display:none; width:410px;">';
			echo '<div>&laquo;&nbsp;<label onclick="hide_search_div();" style="cursor:pointer; font-weight:bold; font-size:12px;  text-decoration:underline;">'._('Back to Basic Search').'</label></div>';
			echo '<div style="height:14px;"></div>';
			echo '<div style=text-align:left;padding-left:74px;padding-bottom:14px;>'._('Comments').'&nbsp;<input type=text name="mp_comment" size=30 class="cell_floating"></div>';
			echo '<div style="margin-left:22px; padding-bottom:5px;"><b>'._('Birthday').'</b></div>';
			echo '<div style=text-align:left;padding-left:98px;padding-bottom:4px;>'._('From').': '.SearchDateInput('day_from_birthdate','month_from_birthdate','','Y','Y','').'</div>';
			echo '<div style=text-align:left;padding-left:110px;padding-bottom:20px;>'._('To').': '.SearchDateInput('day_to_birthdate','month_to_birthdate','','Y','Y','').'</div>';
			echo '<div style="margin-left:22px; padding-bottom:5px;"><b>'._('Goal and Progress').'</b></div>';
			echo '<div style=text-align:left;padding-left:80px;padding-bottom:4px;>'._('Goal Title').' <input type=text name="goal_title" size=30 class="cell_floating"></div>';
			echo '<div style=text-align:left;padding-left:45px;padding-bottom:4px;>'._('Goal Description').' <input type=text name="goal_description" size=30 class="cell_floating"></div>';
			echo '<div style=text-align:left;padding-left:47px;padding-bottom:4px;>'._('Progress Period').' <input type=text name="progress_name" size=30 class="cell_floating"></div>';
			echo '<div style=text-align:left;padding-left:16px;padding-bottom:14px;>'._('Progress Assessment').' <input type=text name="progress_description" size=30 class="cell_floating"></div>';
			
			echo '<div style="margin-left:22px; padding-bottom:5px;"><b>'._('Medical').'</b></div>';
			echo '<div style=text-align:left;padding-left:103px;padding-bottom:4px;>'._('Date').' '.SearchDateInput('med_day','med_month','med_year','Y','Y','Y').'</div>';
			echo '<div style=text-align:left;padding-left:60px;padding-bottom:14px;>'._('Doctor\'s Note').' <input type=text name="doctors_note_comments" size=30 class="cell_floating"></div>';
			
			echo '<div style="margin-left:22px; padding-bottom:5px;"><b>'._('Immunization').'</b></div>';
			echo '<div style=text-align:left;padding-left:102px;padding-bottom:4px;>'._('Type').' <input type=text name="type" size=30 class="cell_floating"></div>';
			echo '<div style=text-align:left;padding-left:104px;padding-bottom:4px;>'._('Date').' '.SearchDateInput('imm_day','imm_month','imm_year','Y','Y','Y').'</div>';
			echo '<div style=text-align:left;padding-left:76px;padding-bottom:14px;>'._('Comments').' <input type=text name="imm_comments" size=30 class="cell_floating"></div>';
			
			echo '<div style="margin-left:22px; padding-bottom:5px;"><b>'._('Medical Alert').'</b></div>';
			echo '<div style=text-align:left;padding-left:104px;padding-bottom:4px;>'._('Date').' '.SearchDateInput('ma_day','ma_month','ma_year','Y','Y','Y').'</div>';
			echo '<div style=text-align:left;padding-left:102px;padding-bottom:14px;>'._('Alert').' <input type=text name="med_alrt_title" size=30 class="cell_floating"></div>';
			
			echo '<div style="margin-left:22px; padding-bottom:5px;"><b>'._('Nurse Visit').'</b></div>';
			echo '<div style=text-align:left;padding-left:104px;padding-bottom:4px;>'._('Date').' '.SearchDateInput('nv_day','nv_month','nv_year','Y','Y','Y').'</div>';
			echo '<div style=text-align:left;padding-left:89px;padding-bottom:4px;>'._('Reason').' <input type=text name="reason" size=30 class="cell_floating"></div>';
			echo '<div style=text-align:left;padding-left:96px;padding-bottom:4px;>'._('Result').' <input type=text name="result" size=30 class="cell_floating"></div>';
			echo '<div style=text-align:left;padding-left:76px;padding-bottom:4px;>'._('Comments').' <input type=text name="med_vist_comments" size=30 class="cell_floating"></div>';
			...
			{
				echo '<INPUT type=checkbox name=address_group value=Y'.(Preferences('DEFAULT_FAMILIES')=='Y'?' CHECKED':'').'>'._('Group by Family').'<BR>';
			...
			echo '<INPUT type=checkbox name=include_inactive value=Y>'._('Include Inactive Students').'<BR>';
			echo '<BR>';
			//echo Buttons('Submit','Reset');
			if($extra['pdf']!=true)
			echo "<INPUT type=SUBMIT class=btn_medium value="._('Submit')." onclick='return formcheck_student_advnc_srch();formload_ajax(\"search\");'>&nbsp<INPUT type=RESET class=btn_medium value="._('Reset').">";
			else
			echo "<INPUT type=SUBMIT class=btn_medium value="._('Submit')." onclick='return formcheck_student_advnc_srch();'>&nbsp<INPUT type=RESET class=btn_medium value="._('Reset').">";
...
//		else
	...
	$LO_columns = array('FULL_NAME'=>''._('Student'),'STUDENT_ID'=>''._('Student ID'),'ALT_ID'=>''._('Alternate ID'),'GRADE_ID'=>''._('Grade').'','PHONE'=>''._('Phone').'');
	...
	if(count($students_RET) > 1 || $link['add'] || !$link['FULL_NAME'] || $extra['columns_before'] || $extra['columns_after'] || ($extra['BackPrompt']==false && count($students_RET)==0) || ($extra['Redirect']===false && count($students_RET)==1))
                  {
                        ...
                        {
                            DrawHeader("<div><A HREF=".PreparePHP_SELF($tmp_REQUEST) . "&expanded_view=true class=big_font ><img src=\"themes/Blue/expanded_view.png\" />"._('Expanded View')."</A></div><div class=break ></div>",$extra['header_right']);
                            DrawHeader(str_replace('<BR>','<BR> &nbsp;',substr($_openSIS['SearchTerms'],0,-4)));
                        }
                        elseif(!UserStudentID() && count($students_RET)!=0)
                        {
                            DrawHeader("<div><A HREF=".PreparePHP_SELF($tmp_REQUEST) . "&expanded_view=false class=big_font><img src=\"themes/Blue/expanded_view.png\" />"._('Original View')."</A></div><div class=break ></div>",$extra['header_right']);
