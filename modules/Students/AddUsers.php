...
if(isset($_REQUEST['student_id']) && $_REQUEST['student_id']!='new' || UserStudentID())
{
	...
	DrawHeaderHome( ''._('Selected Student')._(':').''.$RET[1]['FIRST_NAME'].'&nbsp;'.($RET[1]['MIDDLE_NAME']?$RET[1]['MIDDLE_NAME'].' ':'').$RET[1]['LAST_NAME'].'&nbsp;'.$RET[1]['NAME_SUFFIX'].' (<A HREF=Side.php?student_id=new&modcat='.$_REQUEST['modcat'].'><font color=red>'._('Deselect').'</font></A>) | <A HREF=Modules.php?modname='.$_REQUEST['modname'].'&search_modfunc=list&next_modname=Students/Student.php&ajax=true&bottom_back=true&return_session=true target=body>'._('Back to Student List').'</A>');
        }else if($count_student_RET[1]['NUM']==1){
        DrawHeaderHome( ''._('Selected Student')._(':').''.$RET[1]['FIRST_NAME'].'&nbsp;'.($RET[1]['MIDDLE_NAME']?$RET[1]['MIDDLE_NAME'].' ':'').$RET[1]['LAST_NAME'].'&nbsp;'.$RET[1]['NAME_SUFFIX'].' (<A HREF=Side.php?student_id=new&modcat='.$_REQUEST['modcat'].'><font color=red>'._('Deselect').'</font></A>) ');
        }
}
...
if($_REQUEST['modfunc']!='delete')
{
	...
	if(UserStudentID())
	{
		...
		if($_REQUEST['modfunc']=='list' && $_SESSION['count_stf'])
			echo "<BR><CENTER>".SubmitButton(''._('Add Selected Parents').'','','class=btn_large')."</CENTER></FORM>";
	}
}
