...
if(!$_REQUEST['modfunc'])
{
	...
	if($_REQUEST['id'])
	{
		...
		DrawHeaderHome($title,$delete_button.SubmitButton(_('Save'),'','class=btn_medium onclick="formcheck_student_addressField_F1();"')); //'<INPUT type=submit value=Save>');
		...
		$header .= '<TD>' . SelectInput($RET['CATEGORY_ID']?$RET['CATEGORY_ID']:$_REQUEST['category_id'],'tables['.$_REQUEST['id'].'][CATEGORY_ID]',_('Address Field Category'),$categories_options,false) . '</TD>';
		...
		{
			$header .= '<TD colspan=2>'.TextAreaInput($RET['SELECT_OPTIONS'],'tables['.$_REQUEST['id'].'][SELECT_OPTIONS]',_('Pull-Down/Auto Pull-Down/Coded Pull-Down/Select Multiple Choices<BR>* one per line'),'rows=7 cols=40') . '</TD>';
			$colspan = 1;
		}
	...
	elseif($_REQUEST['category_id'])
	{
		...
		echo " method=POST>";
		DrawHeaderHome($title,$delete_button.SubmitButton(_('Save'),'','class=btn_medium onclick="formcheck_student_addressField_F2();"')); //'<INPUT type=submit value=Save>');
		#PopTable_wo_header('header');
		$header .= '<TABLE cellpadding=3 width=100%>';
		$header .= '<TR>';
...
//	$link['add']['link'] = "Modules.php?modname=$_REQUEST[modname]&category_id=new";
	...
	ListOutput($categories_RET,$columns,_('Address Field Category'),_('Address Field Categories'),$link,array(),$LO_options);
	echo '</TD>';
	...
	if($_REQUEST['category_id'] && $_REQUEST['category_id']!='new' && count($categories_RET))
	{
		...
		echo '<TD valign=top>';
		$columns = array('TITLE'=>_('Address Field'),'SORT_ORDER'=>_('Order'),'TYPE'=>_('Data Type'));
		$link = array();
	...
	//	$link['TITLE']['link'] = "#"." onclick='check_content(\"ajax.php?modname=$_REQUEST[modname]&modfunc=$_REQUEST[modfunc]&mp_term=FY\");'";	
		...
		ListOutput($fields_RET,$columns,_('Address Field'),_('Address Fields'),$link,array(),$LO_options);
