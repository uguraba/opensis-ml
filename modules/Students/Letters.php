...
$extra['search'] .= '<TR><TD valign=top align=right>'._('Letter Text').'</TD><TD><TEXTAREA name=letter_text rows=5 cols=40></TEXTAREA>';
...
else
{
	...
	if(count($RET))
	{
		...
		foreach($RET as $student)
		{
			...
			{
			echo "<meta charset='UTF-8'><tr><td colspan=2 style=\"height:18px\"></td></tr>";
			echo "<meta charset='UTF-8'><table width=100%  style=\" font-family:Arial; font-size:12px;\" >";
			echo "<tr><td width=105>".DrawLogo()."</td><td  style=\"font-size:15px; font-weight:bold; padding-top:20px;\">". GetSchool(UserSchool())."<div style=\"font-size:12px;\">"._('Student Letter')."</div></td><td align=right style=\"padding-top:20px;\">". ProperDate(DBDate()) ."<br />"._('Powered by openSIS')."</td></tr><tr><td colspan=3 style=\"border-top:1px solid #333;\">&nbsp;</td></tr></table>";			echo '<table border=0 style=\" font-family:Arial; font-size:12px;\">';
			echo '<tr>';
			echo '<td>'.$student['FULL_NAME'].', #'.$student['STUDENT_ID'].'</td></tr>';
			echo '<tr>';
			echo '<td>'.$student['GRADE_ID'].''._('Grade').'</td></tr>';
			echo '<tr>';
			echo '<td>'._('Course').': '._($course_title) . "". GetMP($current_mp).'</td></tr>';
			...
			foreach($student as $column=>$value)
				...
				echo "<meta charset='UTF-8'><tr><td style=\"height:18px\"></td></tr>";
		...
		unset($_openSIS['DrawHeader']);
	        ...
	        echo "<tr><td colspan=2 style=\"height:18px\"></td></tr>";
			echo "<meta charset='UTF-8'><table width=100%  style=\" font-family:Arial; font-size:12px;\" >";
			echo "<tr><td width=105>".DrawLogo()."</td><td  style=\"font-size:15px; font-weight:bold; padding-top:20px;\">". GetSchool(UserSchool())."<div style=\"font-size:12px;\">"._('Student Letter')."</div></td><td align=right style=\"padding-top:20px;\">". ProperDate(DBDate()) ."<br \>"._('Powered by openSIS')."</td></tr><tr><td colspan=3 style=\"border-top:1px solid #333;\">&nbsp;</td></tr></table>";
			...
			echo '<td>'.$student['GRADE_ID'].' '._('Grade').'</td></tr>';
			echo '<tr>';
			echo '<td>'._('Course').': '.$course_title . "". GetMP($current_mp).'</td></tr>';
			echo '</table>';
			echo '<br>';
			$letter_text = $_REQUEST['letter_text'];
			foreach($student as $column=>$value)
