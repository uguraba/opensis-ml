...
if($_SESSION['LAST_LOGIN'])
    ...
    $welcome .= ''._('User').' : '.User('NAME').' | '._('Last').' '._('login').': '.ProperDate(substr($_SESSION['LAST_LOGIN'],0,10)).' '._('at').' ' .substr($_SESSION['LAST_LOGIN'],10);
//if($_REQUEST['failed_login'])
if($_SESSION['FAILED_LOGIN'])
$welcome .= ' | <span class=red >'.$_SESSION['FAILED_LOGIN'].'</b> '._('failed login attempts').'</span>';

//----------------------------------------Update Missing Attendance_________________________________-

echo '<div id="calculating" style="display: none; padding-top:20px; padding-bottom:15px;"><img src="assets/missing_attn_loader.gif" /><br/><br/><br/><span style="color:#c90000;"><span style=" font-size:15px; font-weight:bold;">'._('Please wait.').'</span><br /><span style=" font-size:12px;">'._('Compiling missing attendance data. Do not click anywhere.').'</span></span></div>
<div id="resp" style="font-size:14px"></div>';

//-----------------------------------------Update missing attendance ends--------------------------------------------------
//$user_event_visibility=DBGet(DBQuery('SELECT COUNT(*) AS VISIBLE FROM calendar_events_visibility WHERE CALENDAR_ID IN (SELECT CALENDAR_ID FROM attendance_calendars WHERE SCHOOL_ID='.UserSchool().' AND SYEAR='.UserSyear().') AND PROFILE_ID='.$_SESSION['PROFILE_ID'].''));
//$user_event_visibility=$user_event_visibility[1]['VISIBLE'];
switch (User('PROFILE'))
{
    case 'admin':
        DrawBC (_($welcome).' |'._('Role').':'._('Administrator').'');
...
//            }
        ...
        {
            echo '<p>';
            ListOutput($events_RET,array('SCHOOL_DATE'=>''._('Date').'','TITLE'=>''._('Event').'','DESCRIPTION'=>''._('Description').'','SCHOOL'=>''._('School').''),''._('Upcoming Event').'',''._('Upcoming Events').'',array(),array(),array('save'=>false,'search'=>false));
            echo '</p>';
        }
	...
	$RET=DBGet(DBQuery('SELECT SCHOOL_ID,SCHOOL_DATE,COURSE_PERIOD_ID,TEACHER_ID,SECONDARY_TEACHER_ID FROM missing_attendance WHERE SCHOOL_ID=\''.UserSchool().'\' AND SYEAR=\''.  UserSyear().'\' AND SCHOOL_DATE<\''.date('Y-m-d').'\' LIMIT 0,1 '));
          if (count($RET))
          {
                echo '<p><font color=#FF0000><b>'._('Warning!! - Teachers have missing attendance. Go to : Users -> Teacher Programs -> Missing Attendance').'</b></font></p>';
          }
        }
        echo '<div id="attn_alert" style="display: none" ><p><font color=#FF0000><b>'._('Warning!! - Teachers have missing attendance. Go to : Users -> Teacher Programs -> Missing Attendance').'</b></font></p></div>';
       ...
       $notice_date=DBGet(DBQuery('SELECT END_DATE FROM school_years WHERE SYEAR=\''.UserSyear().'\' AND SCHOOL_ID=\''.UserSchool().'\''));
        ...
        {
            echo '<p><font color=#FF0000><b>'._('School year is ending or has ended. Rollover required.').'</b></font></p>';
        }
...
*/
    ...
    case 'teacher':
        DrawBC (_($welcome).' |'._('Role').':'._('Teacher').'');
...
{
    echo '<p><center><font color=#FF0000><b>'._('Warning!!').'</b></font>'._(' - Teachers have missing attendance data').':</center>';
   ...
   ListOutput_missing_attn_teach_port($RET,array('SCHOOL_DATE'=>''._('Date').'','TITLE'=>''._('Period -Teacher').'','SCHOOL'=>''._('School').''),''._('Period').'',''._('Periods').'',$link,array(),array('save'=>false,'search'=>false));
...
*/
    ...
    case 'parent':
        DrawBC (_($welcome).' | '._('Role').' : '._('Parent'));
...
foreach($courses_RET as $course)
	{
               ...
               if(count($assignments_RET))
		{
			...
			$LO_columns = array('TITLE'=>''._('Title').'','CATEGORY'=>''._('Category').'','ASSIGNED_DATE'=>''._('Assigned Date').'','DUE_DATE'=>''._('Due Date').'','COMMENT'=>''._('Description').'');
                        ...
                        foreach($assignments_RET as $assignment)
			{
                        ...
                        DrawHeader(''._('Subject').' - '._(substr($course['TITLE']),strrpos(str_replace(' - ',' ^ ',_($course['TITLE'])),'^')));
 ...
 break;
    ...
    case 'student':
        DrawBC (_($welcome).' | '._('Role').' : '._('Student').'');
        ...
        {
            echo '<p>';
            ListOutput($events_RET,array('TITLE'=>''._('Event').'','SCHOOL_DATE'=>''._('Date').'','DESCRIPTION'=>''._('Description').''),''._('Upcoming Event').'',''._('Upcoming Events').'',array(),array(),array('save'=>false,'search'=>false));
            echo '</p>';
        }
        ...
        else
		 {
          $assignments_RET = DBGet(DBQuery( 'SELECT ga.ASSIGNMENT_ID,ga.TITLE,ga.DESCRIPTION as COMMENT,ga.ASSIGNED_DATE,ga.DUE_DATE,ga.POINTS AS POINTS_POSSIBLE,at.TITLE AS CATEGORY
			...
			if(count($assignments_RET))
			{
				...
				DrawHeader(''._('Subject').' - '.substr($course['TITLE'],strrpos(str_replace(' - ',' ^ ',$course['TITLE']),'^')));
