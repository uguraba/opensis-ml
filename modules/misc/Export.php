...
if($_REQUEST['search_modfunc']=='list')
{
	if(!$fields_list)
	{
		$fields_list = array('FULL_NAME'=>(Preferences('NAME')=='Common'?''._('Last, Common').'':''._('Last, First M').''),'FIRST_NAME'=>''._('First').'','FIRST_INIT'=>''._('First Initial').'','LAST_NAME'=>''._('Last').'','MIDDLE_NAME'=>''._('Middle').'','NAME_SUFFIX'=>''._('Suffix').'','STUDENT_ID'=>''._('Student ID').'','GRADE_ID'=>''._('Grade').'','SCHOOL_ID'=>''._('School').'','NEXT_SCHOOL'=>''._('Rolling / Retention Options').'','CALENDAR_ID'=>''._('Calendar').'','USERNAME'=>''._('Username').'','PASSWORD'=>''._('Password').'','ALT_ID'=>''._('Alternate ID').'','BIRTHDATE'=>''._('DOB').'','EMAIL'=>''._('Email ID').'','ADDRESS'=>''._('Address').'','CITY'=>''._('City').'','STATE'=>''._('State').'','ZIPCODE'=>''._('Zip Code').'','PHONE'=>''._('Phone').'','MAIL_ADDRESS'=>''._('Mailing Address').'','MAIL_CITY'=>''._('Mailing City').'','MAIL_STATE'=>''._('Mailing State').'','MAIL_ZIPCODE'=>''._('Mailing Zipcode').'','PARENTS'=>''._('Contacts').'');
		...
		foreach($periods_RET as $period)
			$fields_list['PERIOD_'.$period['PERIOD_ID']] = $period['TITLE']._('Teacher')._('-')._('Room');
	}
	...
	if($openSISModules['Food_Service'] && ($_REQUEST['fields']['FS_ACCOUNT_ID']=='Y' || $_REQUEST['fields']['FS_DISCOUNT']=='Y' || $_REQUEST['fields']['FS_STATUS']=='Y' || $_REQUEST['fields']['FS_BARCODE']=='Y' || $_REQUEST['fields']['FS_BALANCE']=='Y'))
	{
		...
		$fields_list += array('FS_ACCOUNT_ID'=>''._('F/S Account ID').'','FS_DISCOUNT'=>''._('F/S Discount').'','FS_STATUS'=>''._('F/S Status').'','FS_BARCODE'=>''._('F/S Barcode').'','FS_BALANCE'=>''._('F/S Balance').'',);
	}
                ...
                if($_REQUEST['fields'])
                {
                    ...
                    ListOutputPrint_Report($RET,$columns,$extra['singular']?$extra['singular']:''._('Student').'',$extra['plural']?$extra['plural']:''._('Students').'',array(),$extra['LO_group'],$extra['LO_options']);
                    echo "</body></html>";
                }
}
else
{
	if(!$fields_list)
	{
		if(AllowUse('Students/Student.php&category_id=1'))
			$fields_list['General'] = array('FULL_NAME'=>(Preferences('NAME')=='Common'?''._('Last, Common').'':''._('Last, First M').''),'FIRST_NAME'=>''._('First').'','FIRST_INIT'=>''._('First Initial').'','LAST_NAME'=>''._('Last').'','MIDDLE_NAME'=>''._('Middle').'','NAME_SUFFIX'=>''._('Suffix').'','STUDENT_ID'=>''._('Student ID').'','GRADE_ID'=>''._('Grade').'','SCHOOL_ID'=>''._('School').'','NEXT_SCHOOL'=>''._('Rolling / Retention Options').'','CALENDAR_ID'=>''._('Calendar').'','USERNAME'=>''._('Username').'','ALT_ID'=>''._('Alternate ID').'','BIRTHDATE'=>''._('DOB').'','EMAIL'=>''._('Email ID').'','PHONE'=>''._('Phone').'');
		if(AllowUse('Students/Student.php&category_id=3'))
		{
			$fields_list['Address'] = array('ADDRESS'=>''._('Address').'','CITY'=>''._('City').'','STATE'=>''._('State').'','ZIPCODE'=>''._('Zip Code').'','MAIL_ADDRESS'=>''._('Mailing Address').'','MAIL_CITY'=>''._('Mailing City').'','MAIL_STATE'=>''._('Mailing State').'','MAIL_ZIPCODE'=>''._('Mailing Zipcode').'');
...
/*******************************************************************************/
	...
	DrawHeader("<div><a class=big_font><img src=\"themes/Blue/expanded_view.png\" />"._('Select Fields To Generate Report')."</a></div>",$extra['header_right']);
	PopTable_wo_header('header');
	echo '<TABLE><TR>';
	foreach($fields_list as $category=>$fields)
	{
		
		echo '<TD colspan=2 class=break_headers ><b>'._($category).'<BR></b></TD></TR><TR>';
		foreach($fields as $field=>$title)
		{
			$i++;
			echo '<TD><INPUT type=checkbox onclick="addHTML(\'<LI>'._($title).'</LI>\',\'names_div\',false);addHTML(\'<INPUT type=hidden name=fields['.$field.'] value=Y>\',\'fields_div\',false);addHTML(\'\',\'names_div_none\',true);this.disabled=true">'._($title).($field=='PARENTS'?'<BR>(<small>'._('Relation').': </small><input type=text id=relation name=relation size=8>)':'').'</TD>';
			if($i%2==0)
				echo '</TR><TR>';
		}
		if($i%2!=0)
	...
	DrawHeader("<div><a class=big_font><img src=\"themes/Blue/expanded_view.png\" />"._('Selected Fields')."</a></div>",$extra['header_right']);
	echo '<div id="names_div_none" class="error_msg" style="padding:6px 0px 0px 6px;">'._('no fields selected').'</div><ol id=names_div class="selected_report_list"></ol>';
