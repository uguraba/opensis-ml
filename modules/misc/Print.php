...
if($_REQUEST['search_modfunc']=='list')
{
	if(!$fields_list)
	{
		$fields_list = array('FULL_NAME'=>(Preferences('NAME')=='Common'?''._('Last, Common').'':'Last, First M'),'FIRST_NAME'=>''._('First').'','FIRST_INIT'=>''._('First Initial').'','LAST_NAME'=>''._('Last').'','MIDDLE_NAME'=>''._('Middle').'','NAME_SUFFIX'=>''._('Suffix').'','STUDENT_ID'=>''._('Student ID').'','GRADE_ID'=>''._('Grade').'','SCHOOL_ID'=>''._('School').'','NEXT_SCHOOL'=>''._('Rolling / Retention Options').'','CALENDAR_ID'=>''._('Calendar').'','USERNAME'=>''._('Username').'','PASSWORD'=>''._('Password').'','ADDRESS'=>''._('Address').'','CITY'=>''._('City').'','STATE'=>''._('State').'','ZIPCODE'=>''._('Zip Code').'','PHONE'=>''._('Home Phone').'','MAIL_ADDRESS'=>''._('Mailing Address').'','MAIL_CITY'=>''._('Mailing City').'','MAIL_STATE'=>''._('Mailing State').'','MAIL_ZIPCODE'=>''._('Mailing Zipcode').'','PARENTS'=>''._('Contacts').'');
	...
	$custom_RET = DBGet(DBQuery('SELECT TITLE,ID,TYPE FROM address_fields ORDER BY SORT_ORDER'));
...
else
{
	if(!$fields_list)
	{
		if(AllowUse('Students/Student.php&category_id=1'))
			$fields_list['General'] = array('FULL_NAME'=>(Preferences('NAME')=='Common'?''._('Last, Common').'':'Last, First M'),'FIRST_NAME'=>''._('First').'','FIRST_INIT'=>''._('First Initial').'','LAST_NAME'=>''._('Last').'','MIDDLE_NAME'=>''._('Middle').'','NAME_SUFFIX'=>''._('Suffix').'','STUDENT_ID'=>''._('Student ID').'','GRADE_ID'=>''._('Grade').'','SCHOOL_ID'=>''._('School').'','NEXT_SCHOOL'=>''._('Rolling / Retention Options').'','CALENDAR_ID'=>''._('Calendar').'','USERNAME'=>''._('Username').'','PASSWORD'=>''._('Password').'');
		if(AllowUse('Students/Student.php&category_id=3'))
		{
			$fields_list['Address'] = array('ADDRESS'=>''._('Address').'','CITY'=>''._('City').'','STATE'=>''._('State').'','ZIPCODE'=>''._('Zip Code').'','PHONE'=>''._('Home Phone').'','MAIL_ADDRESS'=>''._('Mailing Address').'','MAIL_CITY'=>''._('Mailing City').'','MAIL_STATE'=>''._('Mailing State').'','MAIL_ZIPCODE'=>''._('Mailing Zipcode').'','PARENTS'=>''._('Contacts').'');
	...
	$periods_RET = DBGet(DBQuery('SELECT TITLE,PERIOD_ID FROM school_periods WHERE SYEAR=\''.UserSyear().'\' AND SCHOOL_ID=\''.UserSchool().'\' ORDER BY SORT_ORDER'));
	foreach($periods_RET as $period)
		$fields_list['Schedule']['PERIOD_'.$period['PERIOD_ID']] = $period['TITLE']._(' Teacher - Room');
	...
	DrawHeader("<div><a class=big_font><img src=\"themes/Blue/expanded_view.png\" />"._('Fields')."</a></div><div class=break ></div>",$extra['header_right']);
	...
	foreach($fields_list as $category=>$fields)
	{
		...
		{
			$i++;
			echo '<TD><INPUT type=checkbox onclick="addHTML(\'<LI>'.$title.'</LI>\',\'names_div\',false);addHTML(\'<INPUT type=hidden name=fields['.$field.'] value=Y>\',\'fields_div\',false);this.disabled=true">'.$title.($field=='PARENTS'?'<BR>(<small>'._('Relation').': </small><input type=text id=relation name=relation size=8>)':'').'</TD>';
			if($i%2==0)
				echo '</TR><TR>';
		}
		if($i%2!=0)
