...
if($_REQUEST['modfunc']=='save')
{
 ...
 else
 {
	...
	$extra['FROM'] .= ',student_report_card_grades sg1 LEFT OUTER JOIN report_card_grades rpg ON (rpg.ID=sg1.REPORT_CARD_GRADE_ID),
					course_periods rc_cp,courses c,school_periods sp,student_mp_stats stat,schools sc ';
	$extra['WHERE'] .= ' AND sg1.MARKING_PERIOD_ID IN ('.$mp_list.')
					AND rc_cp.COURSE_PERIOD_ID=sg1.COURSE_PERIOD_ID AND c.COURSE_ID = rc_cp.COURSE_ID AND sg1.STUDENT_ID=ssm.STUDENT_ID AND sp.PERIOD_ID=rc_cp.PERIOD_ID
                                                                                           AND sc.ID=sg1.SCHOOL_ID and stat.student_id=ssm.student_id';
...
//	}
	...
	if(count($RET))
	{
		...
		if($_REQUEST['elements']['period_absences']=='Y')
			$columns += array('ABSENCES'=>''._('Abs').'<BR>'._('YTD / MP').'');
		...
		foreach($RET as $student_id=>$course_periods)
		{
			...
			echo "<meta charset='UTF-8'><tr><td width=105>".DrawLogo()."</td><td  style=\"font-size:15px; font-weight:bold; padding-top:20px;\">". GetSchool(UserSchool()).' ('.$cur_session.')'."<div style=\"font-size:12px;\">"._('Student Report Card')."</div></td><td align=right style=\"padding-top:20px\">". ProperDate(DBDate()) ."<br \>"._('Powered by openSIS')."</td></tr><tr><td colspan=3 style=\"border-top:1px solid #333;\">&nbsp;</td></tr></table>";
...
//					echo '<BR><BR><BR>';
				...
				if($_REQUEST['elements']['mp_absences']=='Y')
				{
					...
					$mp_absences = ''._('Daily Absences this').' '._(GetMP($last_mp,'TITLE')).': '.$count;
				}
				if($_REQUEST['elements']['ytd_absences']=='Y')
				{
					...
					foreach($attendance_day_RET[$student_id] as $mp_abs)
						foreach($mp_abs as $abs)
							$count += 1-$abs['STATE_VALUE'];
					DrawHeader(''._('Year-to-Date Daily Absences').': '.$count,$mp_absences);
					$count_lines++;
				}
...
//				}
                ...
                if($_REQUEST['elements']['signature']=='Y')
                {
                    ...
                    echo '<tr><td align="left" width=190><span style="font-size:13px; font-weight:bold; height:30px;">'._('Teacher').'&rsquo;'._('s Signature').'</span><br/><br/><br/><br/></td><td width="5" valign=top>:</td><td align=left  valign=top>______________________________________</td><td valign=top align=left><span style="font-size:13px; font-weight:bold; height:30px;">'._('Date ').': ______________</span></td></tr>';
                    echo '<tr><td align=left><span style="font-size:13px; font-weight:bold; height:30px;">'._('Principal').'&rsquo;'._('s Signature').'</span><br/><br/><br/><br/></td><td  valign=top>:</td><td  valign=top>______________________________________</td>';
                    echo '<td valign=top align=left><span style="font-size:13px; font-weight:bold; height:30px;">'._('Date').' : ______________</span></td></tr>';
                    echo '<tr><td align="left"><span style="font-size:13px; font-weight:bold; height:30px;">'._('Parent/Guardian').'&rsquo;'._('s Signature').'</span></td><td  valign=top>:</td><td valign=top>______________________________________</td><td valign=top align=left><span style="font-size:13px; font-weight:bold; height:30px;">'._('Date').' : ______________</span></td></tr>';
                    echo '</table>';
		...
		BackPrompt(_('Missing grades or No Students were found.'));
	}
	else
...
if(!$_REQUEST['modfunc'])
{
	...
	{
		echo "<meta charset='UTF-8'><FORM action=for_export.php?modname=$_REQUEST[modname]&modfunc=save&include_inactive=$_REQUEST[include_inactive]&_openSIS_PDF=true&head_html=Student+Report+Card method=POST target=_blank>";
		...
		$extra['extra_header_left'] .= '<TD><INPUT type=checkbox name=elements[signature] value=Y>'._('Include Signature Line').'</TD>';
		...
		$extra['extra_header_left'] .= '<TD><INPUT type=checkbox name=elements[mp_absences] value=Y'.(GetMP(UserMP(),'SORT_ORDER')!=1?' CHECKED':'').'>'._('Daily')._(' ')._('Absences')._(' ')._('this')._(' ')._('Marking')._(' ')._('Period').'</TD>';
		$extra['extra_header_left'] .= '</TR><TR>';
		$extra['extra_header_left'] .= '<TD><INPUT type=checkbox name=elements[ytd_tardies] value=Y>'._('Other')._(' ')._('Attendance')._(' ')._('Year-to-Date').': <SELECT name="ytd_tardies_code">';
		...
		$extra['extra_header_left'] .= '<TD><INPUT type=checkbox name=elements[mp_tardies] value=Y>'._('Other')._(' ')._('Attendance')._(' ')._('this')._(' ')._('Marking')._(' ')._('Period').': <SELECT name="mp_tardies_code">';
		foreach($attendance_codes as $code)
			$extra['extra_header_left'] .= '<OPTION value='.$code['ID'].'>'.$code['SHORT_NAME'].'</OPTION>';
		$extra['extra_header_left'] .= '</SELECT></TD>';
		$extra['extra_header_left'] .= '</TR><TR>';
		$extra['extra_header_left'] .= '<TD><INPUT type=checkbox name=elements[period_absences] value=Y>'._('Period-by-Period')._(' ')._('Absences').'</TD>';
		$extra['extra_header_left'] .= '<TD><INPUT type=checkbox name=elements[gpa] value=Y>'._('GPA').'</TD>';
		$extra['extra_header_left'] .= '</TR>';
...
//				$extra['extra_header_left'] .= '<TD><INPUT type=checkbox name=mp_arr[] value='.$sem.'>'.GetMP($sem,'SHORT_NAME').'</TD>';
			if(GetMP($sem,'DOES_EXAM')=='Y')
				$extra['extra_header_left'] .= '<TD><INPUT type=checkbox name=mp_arr[] value=E'.$sem.'>'.GetMP($sem,'SHORT_NAME').' '._('Exam').'</TD>';
		...
		if($sem)
		{
			...
			if(GetMP($fy,'DOES_EXAM')=='Y')
				$extra['extra_header_left'] .= '<TD><INPUT type=checkbox name=mp_arr[] value=E'.$fy.'>'.GetMP($fy,'SHORT_NAME').' '._('Exam').'</TD>';
...
//		Widgets('mailing_labels',true);
	...
	if($_REQUEST['search_modfunc']=='list')
	{
		...
		echo '<BR><CENTER><INPUT type=submit class=btn_xxlarge value=\''._('Create Report Cards for Selected Students').'\'></CENTER>';
		echo "</FORM>";
	}
}
