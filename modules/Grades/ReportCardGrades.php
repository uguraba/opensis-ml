...
if(clean_param($_REQUEST['modfunc'],PARAM_ALPHAMOD)=='remove')
{
 if($_REQUEST['tab_id']!='new'){
	...
	if($has_assigned>0){
	UnableDeletePromptX(_('Cannot delete because student grades are associated.'));
	}else{
	if($_REQUEST['tab_id']!='new')
	{
...
if(!$_REQUEST['modfunc'])
{
	...
	else
	{
		...
		$functions = array('TITLE'=>'makeTextInput','GP_SCALE'=>'makeTextInput', 'COMMENT'=>'makeTextInput','GPA_CAL'=>'makeCheckInput','SORT_ORDER'=>'makeTextInput');  //gcu customization
		$LO_columns = array('TITLE'=>''._('Gradescale').'','GP_SCALE'=>''._('Scale Value').'', 'COMMENT'=>''._('Comment').'','GPA_CAL'=>''._('Calculate GPA').'','SORT_ORDER'=>''._('Sort Order').'');  //gcu customization

		$link['add']['html'] = array('TITLE'=>makeTextInput('','TITLE'),'GP_SCALE'=>makeTextInput('', 'GP_SCALE'),'COMMENT'=>makeTextInput('','COMMENT'),'GPA_CAL'=>makeCheckInput('','GPA_CAL'),'SORT_ORDER'=>makeTextInput('','SORT_ORDER'));   //gcu customization
	...
	echo "<meta charset='UTF-8'><FORM name=F1 id=F1 action=Modules.php?modname=$_REQUEST[modname]&modfunc=update&tab_id=$_REQUEST[tab_id] method=POST>";
	...
	echo '<BR>';
	echo '<CENTER>'.SubmitButton(''._('Save').'','','class=btn_medium onclick="formcheck_grade_grade();"').'</CENTER>';
	PopTable ('footer');
	echo '</div>';
	echo '</FORM>';
}
