...
if(clean_param($_REQUEST['student_id'],PARAM_INT))
{
	...
	DrawHeaderHome( ''._('Selected Student').': '.$RET[1]['FIRST_NAME'].'&nbsp;'.($RET[1]['MIDDLE_NAME']?$RET[1]['MIDDLE_NAME'].' ':'').$RET[1]['LAST_NAME'].'&nbsp;'.$RET[1]['NAME_SUFFIX'].'  | <A HREF=Modules.php?modname='.$_REQUEST['modname'].'&search_modfunc=list&next_modname=Students/Student.php&ajax=true&bottom_back=true&return_session=true target=body>Back to Student List</A>');
...
//DrawHeaderHome( 'Selected Student: '.$RET[1]['FIRST_NAME'].'&nbsp;'.($RET[1]['MIDDLE_NAME']?$RET[1]['MIDDLE_NAME'].' ':'').$RET[1]['LAST_NAME'].'&nbsp;'.$RET[1]['NAME_SUFFIX'].' (<A HREF=Side.php?student_id=new&modcat='.$_REQUEST['modcat'].'><font color=red>Remove</font></A>) | <A HREF=Modules.php?modname='.$_REQUEST['modname'].'&search_modfunc=list&next_modname='.$_REQUEST['modname'].'&ajax=true&bottom_back=true&return_session=true target=body>Back to Student List</A>');
        ...
        DrawHeaderHome( ''._('Selected Student').': '.$RET[1]['FIRST_NAME'].'&nbsp;'.($RET[1]['MIDDLE_NAME']?$RET[1]['MIDDLE_NAME'].' ':'').$RET[1]['LAST_NAME'].'&nbsp;'.$RET[1]['NAME_SUFFIX'].' (<A HREF=Side.php?student_id=new&modcat='.$_REQUEST['modcat'].'><font color=red>Deselect</font></A>) ');
        }
...
if(clean_param($_REQUEST['student_id'],PARAM_INT))
{
	...
	$LO_columns = array('TYPE_TITLE'=>_('Category'),'TITLE'=>_('Assignment'),'POINTS'=>_('Points'),'LETTER_GRADE'=>_('Grade'),'COMMENT'=>_('Comment'));
	$item = 'Assignment';
	$items = 'Assignments';
...
else
{
	...
	if($_REQUEST['assignment_id']!='all')
		$LO_columns += array('STUDENT_ID'=>_('Student ID'));
	if($_REQUEST['include_inactive']=='Y')
		$LO_columns += array('ACTIVE'=>'School Status','ACTIVE_SCHEDULE'=>_('Course Status'));
	$item = 'Student';
	$items = 'Students';
	...
	elseif(clean_param($_REQUEST['assignment_id'],PARAM_INT))
	{
		...
		$LO_columns += array('POINTS'=>_('Points'),'LETTER_GRADE'=>_('Grade'),'COMMENT'=>_('Comment'));
...
$assignment_select = '<SELECT name=assignment_id onchange="document.location.href=\'Modules.php?modname='.$_REQUEST['modname'].'&include_inactive='.$_REQUEST['include_inactive'].'&assignment_id=\'+this.options[selectedIndex].value"><OPTION value="">Totals</OPTION><OPTION value="all"'.(($_REQUEST['assignment_id']=='all' && !$_REQUEST['student_id'])?' SELECTED':'').'>'._('All').'</OPTION>';
if($_REQUEST['student_id'])
...
if( count($stu_RET) == 0)
DrawHeaderHome($assignment_select,'','<INPUT type=checkbox name=include_inactive value=Y'.($_REQUEST['include_inactive']=='Y'?" CHECKED onclick='document.location.href=\"".PreparePHP_SELF($tmp_REQUEST)."&include_inactive=\";'":" onclick='document.location.href=\"".PreparePHP_SELF($tmp_REQUEST)."&include_inactive=Y\";'").'>'._('Include Inactive Students').'');
else
DrawHeaderHome($assignment_select,$_REQUEST['assignment_id']?SubmitButton('Save','','class=btn_medium'):'','<INPUT type=checkbox name=include_inactive value=Y'.($_REQUEST['include_inactive']=='Y'?" CHECKED onclick='document.location.href=\"".PreparePHP_SELF($tmp_REQUEST)."&include_inactive=\";'":" onclick='document.location.href=\"".PreparePHP_SELF($tmp_REQUEST)."&include_inactive=Y\";'").'>'._('Include Inactive Students').'');
...
echo $_REQUEST['assignment_id']?'<CENTER>'.SubmitButton(''._('Save'),'','class=btn_medium').'</CENTER>':'';
echo '</FORM>';
			...
			if($assignment_id=='' && !$_REQUEST['student_id'])
			{
				if(count($points_RET[$THIS_RET['STUDENT_ID']]))
				{
					...
					$total = 0;
                                ...
                                if($_SESSION['ROUNDING']=='UP')
                                        $total_m = ceil(($total*100));
                                elseif($_SESSION['ROUNDING']=='DOWN')
                                        $total_m = floor(($total*100));
                                elseif($_SESSION['ROUNDING']=='NORMAL')
                                        $total_m = round(($total*100));
                                else
                                    $total_m=($total*100);
//                                $ppercent= _makeLetterGrade($total,"",User('STAFF_ID'),"%");
                                $ppercent=$total_m;
                                $round_ppercent = round($ppercent, 2);
                                $total_m=$total_m/100;
                                if($points_RET[$THIS_RET['STUDENT_ID']][1]['PARTIAL_POINTS']!='')
                                    return ($total>$max_allowed?'<FONT color=red>':'').$round_ppercent.($total>$max_allowed?'</FONT>':'').'%&nbsp;<B>'._makeLetterGrade($total_m,"",User('STAFF_ID')).'</B>';
                                else
					return _('Not Graded');
			}
			else
					...
					if($points!='-1')
                                        {
                                            if($points!='')
                                            {
                                                ...
                                                elseif($rounding[1]['ROUNDING']=='NORMAL')
                                                 ...
                                                 #return ($THIS_RET['DUE']||$points!=''?($points>$total_points[$assignment_id]*$max_allowed?'<FONT color=red>':''):'<FONT color=gray>').$points_m.'%'.($THIS_RET['DUE']||$points!=''?($points>$total_points[$assignment_id]*$max_allowed?'</FONT>':''):'').'&nbsp;<B>'. _makeLetterGrade($make_grade_points).'</B>'.($THIS_RET['DUE']||$points!=''?'':'</FONT>');
		...
		if($points!='*')
                {
                    if($points!='')
                    {
                       ...
                       #  return '<TABLE border=0 cellspacing=0 cellpadding=0 class=LO_field><TR align=center><TD>'.TextInput($points,'values['.$THIS_RET['STUDENT_ID'].']['.$assignment_id.'][POINTS]','',' size=2 maxlength=7 tabindex='.$tabindex).'<HR>'.$total_points[$assignment_id].'</TD><TD>&nbsp;'.($THIS_RET['D'.$assignment_id]||$points!=''?($points>$total_points[$assignment_id]*$max_allowed?'<FONT color=red>':''):'<FONT color=gray>').$points_m.'%'.($THIS_RET['D'.$assignment_id]||$points!=''?($points>$total_points[$assignment_id]*$max_allowed?'</FONT>':''):'').'<BR>&nbsp;<B>'. _makeLetterGrade($make_letter_points).'</B>'.($THIS_RET['D'.$assignment_id]||$points!=''?'':'</FONT>').'</TD></TR></TABLE>';
//			return '<TABLE border=0 cellspacing=0 cellpadding=0 class=LO_field><TR align=center><TD>'.TextInput($points,'values['.$THIS_RET['STUDENT_ID'].']['.$assignment_id.'][POINTS]','',' size=2 maxlength=7 tabindex='.$tabindex).'<HR>'.$total_points[$assignment_id].'</TD><TD>&nbsp;'.($THIS_RET['D'.$assignment_id]||$points!=''?($points>$total_points[$assignment_id]*$max_allowed?'<FONT color=red>':''):'<FONT color=gray>').Percent($points/$total_points[$assignment_id],0).($THIS_RET['D'.$assignment_id]||$points!=''?($points>$total_points[$assignment_id]*$max_allowed?'</FONT>':''):'').'<BR>&nbsp;<B>'. _makeLetterGrade($points/$total_points[$assignment_id]).'</B>'.($THIS_RET['D'.$assignment_id]||$points!=''?'':'</FONT>').'</TD></TR></TABLE>';
                        ...
                        return '<TABLE border=0 cellspacing=0 cellpadding=0 class=LO_field><TR align=center><TD>'.TextInput($points,'values['.$THIS_RET['STUDENT_ID'].']['.$assignment_id.'][POINTS]','',' size=2 maxlength=7 tabindex='.$tabindex).'<HR>'.$total_points[$assignment_id].'</TD><TD>&nbsp;'.($THIS_RET['D'.$assignment_id]||$points!=''?($points>$total_points[$assignment_id]*$max_allowed?'<FONT color=red>':''):'<FONT color=gray>').'&nbsp;&nbsp;'._('Not Graded').'</TD></TR></TABLE>';
                }
		else
