...
echo '<TD><fieldset>';
echo '<legend><b>'._('General').'</b></legend>';
echo '<TABLE>';
if(count($grades)>0)
{
	...
	echo '<TR><TD colspan=3><TABLE><TR><TD colspan=3><B>'._('Score Rounding').'</B></TD></TR><TR><TD><INPUT type=radio name=values[ROUNDING] value=UP'.(($programconfig['ROUNDING']=='UP')?' CHECKED':'').'>'._('Up').'</TD><TD><INPUT type=radio name=values[ROUNDING] value=DOWN'.(($programconfig['ROUNDING']=='DOWN')?' CHECKED':'').'>'._('Down').'</TD><TD><INPUT type=radio name=values[ROUNDING] value=NORMAL'.(($programconfig['ROUNDING']=='NORMAL')?' CHECKED':'').'>'._('Normal').'</TD><TD><INPUT type=radio name=values[ROUNDING] value=\'\''.(($programconfig['ROUNDING']=='')?' CHECKED':'').'>'._('None').'</TD></TR></TABLE></TD></TR>';
}
if(!$programconfig['ASSIGNMENT_SORTING'])
	$programconfig['ASSIGNMENT_SORTING'] = 'ASSIGNMENT_ID';
echo '<TR><TD colspan=3><TABLE><TR><TD colspan=3><b>'._('Assignment Sorting').'</b></td></tr><tr><td><input type=radio name=values[ASSIGNMENT_SORTING] value=ASSIGNMENT_ID'.(($programconfig['ASSIGNMENT_SORTING']=='ASSIGNMENT_ID')?' CHECKED':'').'>'._('Newest First').'</TD><TD><INPUT type=radio name=values[ASSIGNMENT_SORTING] value=DUE_DATE'.(($programconfig['ASSIGNMENT_SORTING']=='DUE_DATE')?' CHECKED':'').'>'._('Due Date').'</TD><TD><INPUT type=radio name=values[ASSIGNMENT_SORTING] value=ASSIGNED_DATE'.(($programconfig['ASSIGNMENT_SORTING']=='ASSIGNED_DATE')?' CHECKED':'').'>'._('Assigned Date').'</TD><TD><INPUT type=radio name=values[ASSIGNMENT_SORTING] value=UNGRADED'.(($programconfig['ASSIGNMENT_SORTING']=='UNGRADED')?' CHECKED':'').'>'._('Ungraded').'</TD></TR></TABLE></TD></TR>';
...
echo '<TR><TD valign=top width=30></TD><TD align=right><INPUT type=checkbox name=values[ELIGIBILITY_CUMULITIVE] value=Y'.(($programconfig['ELIGIBILITY_CUMULITIVE']=='Y')?' CHECKED':'').'></TD><TD align=left>'._('Calulate Eligibility using Cumulative Semester Grades').'</TD></TR>';
...
if($commentsA_select)
{
	...
	echo '</SELECT></TD><TD align=left>'._('Default comment code').'</TD></TR>';
}
...
echo '<TD><fieldset>';
echo '<legend><b>'._('Final Grading Percentages').'</b></legend><div style="width:780px; padding-bottom:8px; overflow:auto; overflow-x:scroll;">';
if($quarters)
{
foreach($semesters as $sem)
	if($sem['DOES_GRADES']=='Y')
	{
		...
		if($total !=100)
			$table .= '<TD><FONT color=red>'._('Total not 100%!').'</TD>';
...
if($year[1]['DOES_GRADES']=='Y')
{
	...
	foreach($semesters as $sem)
	{
		...
		if($sem['DOES_EXAM']=='Y')
			$table .= '<TD style="white-space:nowrap">'.$sem['TITLE'].''._(" Exam").'</TD>';
	}
	if($year[1]['DOES_EXAM']=='Y')
		$table .= '<TD>'.$year[1]['TITLE'].''._(' Exam').'</TD>';
	$table .= '</TR><TR>';
	$total = 0;
	foreach($semesters as $sem)
	{
	...
	if($total !=100)
		$table .= '<TD style="white-space:nowrap"><FONT color=red>'._('Total not 100%!').'</TD>';
	$table .= '</TR>';
	$table .= '</TABLE>';
	echo $table;
}
...
echo '</TR><TR></TABLE>';
PopTable('footer');
echo '<CENTER><INPUT type=submit value='._("Save").' class="btn_medium"></CENTER>';
echo '</FORM>';
?>
