...
//       AND mp.school_id = '".UserSchool()."' order by mp.post_end_date";
       ...
       AND mp.school_id = \''.UserSchool().'\' order by mp.post_end_date';
        ...
        echo '</FORM>';
           ...
           echo '<BR><table align=center ><tr><td align=center colspan=2><div class=student>'.$displayname.'</div><br><b>'._('MARKING PERIOD').'</b><br>'._('WEIGHTED GPA').':'.sprintf('%0.2f',$gmp[$mp_id]['weighted_gpa']).'<br>'._('UNWEIGHTED GPA').':'.sprintf('%0.2f',$gmp[$mp_id]['unweighted_gpa']).'</td></tr>';
            
            
            $sms_grade_level = TextInput($gmp[$mp_id]['grade_level'],"SMS_GRADE_LEVEL",""._('Grade Level')."",'size=15 maxlength=3 class=cell_floating');
            ...
            if ($mp_id=="0"){
                ...
                echo '<tr><td align=right width=50% valign=top>'._('Grade').':</td><td width=50% valign=top>'.$sms_grade_level.'</td></tr><tr><td class=clear></td></tr></table>';
                $tabs = array();
                $tabs[] = array('title'=>_('Grades'),'link'=>"Modules.php?modname=$_REQUEST[modname]&tab_id=grades&mp_id=$mp_id");
                $tabs[] = array('title'=>_('Credits'),'link'=>"Modules.php?modname=$_REQUEST[modname]&tab_id=credits&mp_id=$mp_id");
            ...
            echo '<CENTER>';
            if (!$LO_ret){
                echo SubmitButton(_('Remove Marking Period'), 'removemp','class=btn_large');
				echo '&nbsp;';
            }
            echo SubmitButton(_('Save'),'','class=btn_medium').'</CENTER>';
            echo '</FORM>';
    }
}
