...
if(clean_param($_REQUEST['modfunc'],PARAM_ALPHAMOD)=='save')
{
	...
	//	BackPrompt('No Students were found.');
            ...
            ShowErr(_('No grade found.'));
            else 
            ShowErr(_('No Students were found.'));
	for_error();
	}
	}
	else
	...
	ShowErr(_('You must choose at least one student and marking period'));
	for_error();
	}
}
...
if(!$_REQUEST['modfunc'])
{
	...
	if($_REQUEST['search_modfunc']=='list')
	{
		...
		$extra['extra_header_left'] .= '</TR><TR>';
		$extra['extra_header_left'] .= '<TD><INPUT type=checkbox name=elements[ytd_tardies] value=Y>'._('Other')._(' ')._('Attendance')._(' ')._('Year').('-')._('to')._('-')._('date').':<SELECT name="ytd_tardies_code">';
		...
		$extra['extra_header_left'] .= '<TD><INPUT type=checkbox name=elements[mp_tardies] value=Y>'._('Other Attendance this quarter:').' <SELECT name="mp_tardies_code">';
		foreach($attendance_codes as $code)
			$extra['extra_header_left'] .= '<OPTION value='.$code['ID'].'>'.$code['SHORT_NAME'].'</OPTION>';
		$extra['extra_header_left'] .= '</SELECT></TD>';
		$extra['extra_header_left'] .= '</TR><TR>';
...
//				$extra['extra_header_left'] .= '<TD><INPUT type=checkbox name=mp_arr[] value='.$sem.'>'.GetMP($sem,'SHORT_NAME').'</TD>';
	...
	if($_REQUEST['search_modfunc']=='list')
	{
		PopTable ('footer');
		    ...
		    echo '<BR><CENTER>'.SubmitButton(_('Create Grade Lists for Selected Students'),'','class=btn_xxlarge').'</CENTER>';
                                    }
		echo "</FORM>";
	}
}
