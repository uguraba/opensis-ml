...
if(!$_REQUEST['id'])
{
	DrawHeader(''._('Totals').'',"<A HREF=Modules.php?modname=$_REQUEST[modname]&id=all>"._('Expand All')."</A>");
	$courses_RET = DBGet(DBQuery('SELECT c.TITLE AS COURSE_TITLE,cp.TITLE,cp.COURSE_PERIOD_ID,cp.COURSE_ID,cp.TEACHER_ID AS STAFF_ID FROM schedule s,course_periods cp,courses c WHERE s.SYEAR=\''.UserSyear().'\' AND cp.COURSE_PERIOD_ID=s.COURSE_PERIOD_ID AND s.MARKING_PERIOD_ID IN ('.GetAllMP($MP_TYPE,UserMP()).') AND (\''.date('Y-m-d',strtotime(DBDate())).'\' BETWEEN s.START_DATE AND s.END_DATE OR \''.date('Y-m-d',strtotime(DBDate())).'\'>=s.START_DATE AND s.END_DATE IS NULL) AND s.STUDENT_ID=\''.UserStudentID().'\' AND cp.GRADE_SCALE_ID IS NOT NULL'.(User('PROFILE')=='teacher'?' AND (cp.TEACHER_ID=\''.User('STAFF_ID').'\' OR cp.SECONDARY_TEACHER_ID=\''.User('STAFF_ID').'\')':'').' AND c.COURSE_ID=cp.COURSE_ID ORDER BY cp.COURSE_ID'),array(),array('COURSE_PERIOD_ID'));
        if($display_rank=='Y')
        $LO_columns = array('TITLE'=>''._('Course Title').'','TEACHER'=>''._('Teacher').'','PERCENT'=>''._('Percent').'','GRADE'=>''._('Letter').'','UNGRADED'=>''._('Ungraded').'')+($do_stats?array('BAR1'=>''._('Grade').' '._('Range').'(%)','BAR2'=>''._('Class Rank').''):array());
        else
            $LO_columns = array('TITLE'=>''._('Course Title').'','TEACHER'=>''._('Teacher').'','PERCENT'=>''._('Percent').'','GRADE'=>''._('Letter').'','UNGRADED'=>''._('Ungraded').'')+($do_stats?array('BAR1'=>''._('Grade').' '._('Range').'(%)'):array());
...
else
{
 if($_REQUEST['modfun']=='assgn_detail')
    ...
    # $val1 .=  '<table width="100%" cellpadding="2" cellspacing="2" border="0" align="center"><tr><td><fieldset style="border:1px solid;">';
     $val1 .= '<center > <strong>'._('Assignment')._(' ')._('Details').'</center><br><br>';
    $val1 .= '<table width="95%" cellpadding="2" cellspacing="2" border="0" align="center">';
     $val1 .= '<tr><td valign=top > <strong>'._('Title').'</strong></td><td valign=top>:</td><td valign=top >'.$assignments_RET[1]['TITLE'].'</td>';
     $val1 .= '<td valign=top > <strong>'._('Description').'</strong></td><td valign=top >:</td><td valign=top width=55%>'.$assignments_RET[1]['DESCRIPTION'].'</td></tr>';
     $val1 .= '<tr><td valign=top > <strong>'._('Assignement Type').'</strong></td><td valign=top >:</td><td valign=top>'.$assignments_RET[1]['ASSIGNMENT_TYPE'].'</td>';
     $val1 .= '<td valign=top> <strong>'._('Points').'</strong></td><td valign=top>:</td><td valign=top>'.$assignments_RET[1]['POINTS'].'</td></tr>';
     $val1 .= '<tr><td valign=top>';
     $val1 .= '<strong>'._('Assigned Date').'</strong>';
     $val1 .= '</td><td valign=top>:</td><td valign=top>'.$assignments_RET[1]['ASSIGNED_DATE'].'</td>';
     $val1 .= '<td valign=top> <strong>'._('Due Date').'</strong></td><td valign=top>:</td><td valign=top>'.$assignments_RET[1]['DUE_DATE'].'</td></tr>';
...
if($_REQUEST['id']=='all')
	...
	else
	{
		...
		DrawHeaderhome($val1);
                DrawHeader('<br><B>'.$courses_RET[1]['COURSE_TITLE'].'</B> - '.substr($courses_RET[1]['TITLE'],strrpos(str_replace(' - ',' ^ ',$courses_RET[1]['TITLE']),'^')+2),"<A HREF=Modules.php?modname=$_REQUEST[modname]>"._('Back to Totals')."</A>");
	}
...
//           ECHO "SELECT * FROM student_enrollment ssm WHERE STUDENT_ID='".UserStudentID()."'  AND ssm.SYEAR='".UserSyear()."' AND ((ssm.START_DATE IS NOT NULL AND '".date('Y-m-d')."'>=ssm.START_DATE) AND ('".date('Y-m-d')."'<=ssm.END_DATE OR ssm.END_DATE IS NULL)) ";
               ...
               if(count($assignments_RET))
		{
		      if($_REQUEST['id']=='all')
                        {
                          # echo '<BR>';
                          DrawHeader('<br><B>'.$course['COURSE_TITLE'].'</B> - '.substr($course['TITLE'],strrpos(str_replace(' - ',' ^ ',$course['TITLE']),'^')+2),"<A HREF=Modules.php?modname=$_REQUEST[modname]>"._('Back to Totals')."</A>");
                        }
                       if($do_stats)
			...
			//echo '<pre>'; var_dump($all_RET); echo '</pre>';
                                                    if($display_rank=='Y')
			$LO_columns = array('TITLE'=>''._('Title').'','CATEGORY'=>''._('Category').'','POINTS'=>''._('Points').''._(' / ').''._('Possible').'','PERCENT'=>''._('Percent').'','LETTER'=>''._('Letter').'','ASSIGNED_DATE'=>''._('Assigned Date').'','DUE_DATE'=>''._('Due Date').'')+($do_stats?array('BAR1'=>''._('Grade')._(' ')._('Range').'','BAR2'=>''._('Class Rank').''):array());
                                                    else
                                                            $LO_columns = array('TITLE'=>''._('Title').'','CATEGORY'=>''._('Category').'','POINTS'=>''._('Points').''._(' / ').''._('Possible').'','PERCENT'=>''._('Percent').'','LETTER'=>''._('Letter').'','ASSIGNED_DATE'=>''._('Assigned Date').'','DUE_DATE'=>''._('Due Date').'')+($do_stats?array('BAR1'=>''._('Grade')._(' ')._('Range')):array());
