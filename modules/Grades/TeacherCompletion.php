...
// $sql = 'SELECT DISTINCT s.STAFF_ID,CONCAT(s.LAST_NAME,\', \',s.FIRST_NAME) AS FULL_NAME,sp.TITLE,cpv.PERIOD_ID FROM staff s,school_periods sp,'.actual_cp_table('course_periods').' cp,'.actual_cp_table('course_period_var').' cpv
//
//WHERE sp.PERIOD_ID = cpv.PERIOD_ID AND cp.GRADE_SCALE_ID IS NOT NULL AND cp.TEACHER_ID=s.STAFF_ID  AND cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID
//
...
//
//AND cp.MARKING_PERIOD_ID IN ('.GetAllMP('QTR',$cur_mp).') AND cp.SYEAR=\''.UserSyear().'\' AND cp.SCHOOL_ID=\''.UserSchool().'\' AND s.PROFILE=\'teacher\'
//			'.(($_REQUEST['period'])?' AND cp.PERIOD_ID=\''.$_REQUEST[period].'\'':'').'
//			
//		';	
...
// $sql_gradecompleted = 'SELECT DISTINCT s.STAFF_ID,cpv.PERIOD_ID FROM staff s,school_periods sp,'.actual_cp_table('course_periods').' cp,'.actual_cp_table('course_period_var').' cpv
//
