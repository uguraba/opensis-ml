...
if($_REQUEST['modfunc']=='save')
{
	...
	if(count($RET))
	{
		...
		foreach($RET as $student)
		{
			...
			echo "<meta charset='UTF-8'><td>". $student['GRADE_ID'] ." </td></tr>";
			...
			if($sum_percent>0)
				...
				$sum_points = 0;
                        if($flag)
                            $link['add']['html'] = array('TITLE'=>'<B>'._('Total').'</B>','LETTER_GRADE'=>'( '.$total_stpoints.' / '.$total_asgnpoints.' ) '._makeLetterGrade($sum_points,"",User('STAFF_ID'),"%").'&nbsp;'._makeLetterGrade($sum_points,"",User('STAFF_ID')));
			else
                            $link['add']['html'] = array('TITLE'=>'<B>Total</B>','LETTER_GRADE'=>'Not Graded');
...
if(!$_REQUEST['modfunc'])
{
	...
	{
		echo "<FORM action=for_export.php?modname=$_REQUEST[modname]&modfunc=save&include_inactive=$_REQUEST[include_inactive]&_openSIS_PDF=true&head_html=Student+Progress+Report method=POST target=_blank>";
