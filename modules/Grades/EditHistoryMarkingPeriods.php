...
if(!$_REQUEST['modfunc']){
                echo "<FORM action=Modules.php?modname=$_REQUEST[modname]&modfunc=update&tab_id=$_REQUEST[tab_id]&mp_id=$mp_id method=POST>";
                            DrawHeader('',SubmitButton(_('Save'),'','class=btn_medium'));
                            echo '<BR>';
...
//                }
                ...
                ListOutput($LO_ret,$LO_columns,''._('History Marking Period').'',''._('History Marking Periods').'',$link,array(),array('count'=>true,'download'=>false,'search'=>false));
                echo '<CENTER>';
