   ...
   $extra['moreland_cust'] = 'assignment_grade';
	...
	if(count($RET))
	{
		...
		foreach($RET as $student)
		{
			...
			{
			  echo '<table border=0  style=\"font-size:12px;\" >';
                          echo '<tr><td>'._(' Total').':</td><td>'.Percent($sum_points).'&nbsp;'._makeLetterGrade($sum_points,$course_period_id).'</td> </tr>';
                          echo '<tr><td></td></tr>';
                          echo '</table>';
                        }
                          else
...
//
		echo '<TABLE>';
		echo '<TR><TD align=right width=120>'._('Assigned Date').'</TD><TD><INPUT type=checkbox value=Y name=assigned_date></TD>';
		echo '<TD align=right> '._('Exclude Ungraded E/C Assignments').'</TD><TD><INPUT type=checkbox value=Y name=exclude_ec checked></TD></TR>';
		echo '<TR><TD align=right width=120>'._('Due Date').'</TD><TD><INPUT type=checkbox value=Y name=due_date checked></TD>';
		echo '<TD align=right>'._('Exclude Ungraded Assignments Not Due').'</TD><TD><INPUT type=checkbox value=Y name=exclude_notdue></TD></TR>';
		//Widgets('mailing_labels');
                echo '<TD align=right>'._('With Assignment Details').' </TD><TD><INPUT type=radio value=detail name=list_type checked=true></TD><TD> Totals Only<INPUT type=radio value=total name=list_type></TD></TR>';
...
//            if($_SESSION['count_stu']!=0)
		echo '<BR><CENTER><INPUT type=submit value='._('\'Create Progress Reports').'\')  class=btn_large></CENTER>';
		echo "</FORM>";
//	}
}
