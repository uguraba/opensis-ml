...
if(clean_param($_REQUEST['modfunc'],PARAM_ALPHAMOD)=='remove')
{
	...
	if($has_assigned>0){
	UnableDeletePromptX(_('Cannot delete because report card comments are associated.'));
	}else{
...
if(!$_REQUEST['modfunc'])
{
	...
	$tabs = array(1=>array('title'=>$course_RET[1]['TITLE'],'link'=>"Modules.php?modname=$_REQUEST[modname]&course_id=$_REQUEST[course_id]&tab_id=$_REQUEST[course_id]"),
		2=>array('title'=>''._('All Courses').'','link'=>"Modules.php?modname=$_REQUEST[modname]&course_id=$_REQUEST[course_id]&tab_id=0"),
		3=>array('title'=>''._('General').'','link'=>"Modules.php?modname=$_REQUEST[modname]&course_id=$_REQUEST[course_id]&tab_id=new"));
	...
	else
	{
		...
		$LO_columns = array('SORT_ORDER'=>''._('ID').'','TITLE'=>''._('Comment').'');
	...
	echo "<meta charset='UTF-8'><FORM name=F1 id=F1 action=Modules.php?modname=$_REQUEST[modname]&modfunc=update&course_id=$_REQUEST[course_id]&tab_id=$_REQUEST[tab_id] method=POST>";
	DrawHeaderHome($course_select,SubmitButton(''._('Save').'','','class=btn_medium onclick="formcheck_grade_comment();"')); 
	#echo '<BR>';
	...
	echo "<meta charset='UTF-8'><table width=300px height=120px><tr><td>"; //hack for an empty poptable 
	ListOutputMod($LO_ret,$LO_columns,'','',$link,array(),array('count'=>false,'download'=>false,'search'=>false));
	echo "</td></tr></table>";
	PopTable ('footer');
	echo '</div>';
	echo '<CENTER>'.SubmitButton(''._('Save').'','','class=btn_medium onclick="formcheck_grade_comment();"').'</CENTER>';
	echo '</FORM>';
}
