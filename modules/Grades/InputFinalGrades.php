...
if(!$_REQUEST['_openSIS_PDF'])
{
	...
	unset($tmp_REQUEST['include_inactive']);
                $grade_start_date=DBGet(DBQuery('SELECT `POST_START_DATE` FROM `marking_periods` WHERE `marking_period_id`='.$tmp_REQUEST['mp']));
		$grade_end_date=DBGet(DBQuery('SELECT `POST_END_DATE` FROM `marking_periods` WHERE `marking_period_id`='.$tmp_REQUEST['mp']));
		...
		$grade_status='not open yet';
                ...
                DrawHeaderHome('','<INPUT type=checkbox name=include_inactive value=Y'.($_REQUEST['include_inactive']=='Y'?" CHECKED onclick='document.location.href=\"".PreparePHP_SELF($tmp_REQUEST)."&include_inactive=\";'":" onclick='document.location.href=\"".PreparePHP_SELF($tmp_REQUEST)."&include_inactive=Y\";'").'>'._('Include Inactive Students').'','');
        if(count($stu_RET)!=0)
	DrawHeaderHome($mps_select,SubmitButton('Save','submit[save]','class=btn_medium'),'');
	if(AllowEdit()){
	DrawHeader(($current_completed?'<FONT COLOR=green>'._('These grades are complete').'</FONT>':'<FONT COLOR=green>'._('Grade reporting is open for this marking period').'</FONT>').(AllowEdit()?' | <FONT COLOR=green>'._('You can edit these grades').'</FONT>':' | <FONT COLOR=red>'._('Grade reporting begins on').' : '.date("M d, Y ",strtotime( $grade_start_date[1]['POST_START_DATE'])).'</FONT>'));
	}else if($grade_status =='not open yet'){
			DrawHeader(($current_completed?'<FONT COLOR=green>'._('These grades are complete').'</FONT>':'<FONT COLOR=red>'._('Grade reporting is not open for this marking period').'</FONT>').(AllowEdit()?' | <FONT COLOR=green>'>_('You can edit these grades').'</FONT>':' | <FONT COLOR=red>'._('Grade reporting starts on').': '.date("M d, Y ",strtotime( $grade_start_date[1]['POST_START_DATE'])).' and ends on : '.date("M d, Y ",strtotime( $grade_end_date[1]['POST_END_DATE'])).'</FONT>'));
		}else if($grade_status =='closed'){
			DrawHeader(($current_completed?'<FONT COLOR=green>'._('These grades are complete').'</FONT>':'<FONT COLOR=red>'._('These grades are complete').'</FONT>').(AllowEdit()?' | <FONT COLOR=green>'._('You can edit these grades').'</FONT>':' | <FONT COLOR=red>'._('Grade reporting ended for this marking period on').' : '.date("M d, Y ",strtotime( $grade_end_date[1]['POST_END_DATE'])).'</FONT>'));
		}else if($grade_status=='not set yet'){
                    	DrawHeader('<FONT COLOR=red>'._('Grade reporting date has not set for this marking period').'</FONT>');
                }
if(AllowEdit())
	{
	...
	if($_REQUEST['use_percents']!='true')
		$gb_header = "<A HREF=Modules.php?modname=$_REQUEST[modname]&include_inactive=$_REQUEST[include_inactive]&mp=$_REQUEST[mp]&use_percents=true>"._('Assign Percents')."</A>";
	elseif($not_graded!=true)
		$gb_header = "<A HREF=Modules.php?modname=$_REQUEST[modname]&include_inactive=$_REQUEST[include_inactive]&mp=$_REQUEST[mp]&use_percents=false>"._('Assign Letters')."</A>";
	
		if(substr($_REQUEST['mp'],0,1)!='E')
		{
			#echo $_REQUEST[modname].'<br>'.$_REQUEST[include_inactive];
			$gb_header .= ($not_graded? '':' | ')."<A HREF=Modules.php?modname=$_REQUEST[modname]&include_inactive=$_REQUEST[include_inactive]&modfunc=gradebook&mp=$_REQUEST[mp]&use_percents=true>"._('Get Gradebook Grades')."</A>";
			...
			if($prev_mp)
			{
				$gb_header .= " | <A HREF=Modules.php?modname=$_REQUEST[modname]&include_inactive=$_REQUEST[include_inactive]&modfunc=grades&mp=$_REQUEST[mp]&prev_mp=$prev_mp[MARKING_PERIOD_ID]&use_percents=false>"._('Get')." $prev_mp[TITLE] "._('Grades')."</A>";
				$gb_header .= " | <A HREF=Modules.php?modname=$_REQUEST[modname]&include_inactive=$_REQUEST[include_inactive]&modfunc=comments&mp=$_REQUEST[mp]&prev_mp=$prev_mp[MARKING_PERIOD_ID]&use_percents=false>"._('Get')." $prev_mp[TITLE]"._('Comments')."</A>";
			}
		}
                                    $bar=' | ';
                                    if(substr($_REQUEST['mp'],0,1)=='E' && $not_graded==true)
                                        $bar='';
                                            $gb_header .= "$bar<A HREF=Modules.php?modname=$_REQUEST[modname]&include_inactive=$_REQUEST[include_inactive]&modfunc=clearall&mp=$_REQUEST[mp]&use_percents=$_REQUEST[use_percents]>"._('Clear All')."</A>";
	}
...
$columns = array('FULL_NAME'=>_('Student'),'STUDENT_ID'=>_('Student ID'));
if($_REQUEST['include_inactive']=='Y')
	$columns += array('ACTIVE'=>_('School Status'),'ACTIVE_SCHEDULE'=>_('Course Status'));
if($_REQUEST['use_percents']!='true')
	$columns += array('GRADE_PERCENT'=>_('Percent'),'REPORT_CARD_GRADE'=>_('Assign Grade'));
elseif($not_graded)
                  $columns += array('GRADE_PERCENT'=>_('Assign Percent'));
else
	$columns += array('REPORT_CARD_GRADE'=>_('Grade'),'GRADE_PERCENT'=>_('Assign Percent'));
...
if(substr($_REQUEST['mp'],0,1)!='E'&& GetMP($_REQUEST['mp'],'DOES_COMMENTS')=='Y')
{
	...
	$columns += array('COMMENT'=>_('Comment'));
}
...
echo "</center>";
 if(count($stu_RET)!=0)
echo '<CENTER>'.SubmitButton(_('Save'),'submit[save]','class=btn_medium').'</CENTER>';
echo "</FORM>";
	...
	if($column=='REPORT_CARD_GRADE')
	{
		if(!isset($_REQUEST['_openSIS_PDF']))
		{
			...
			if($dbf[1]['DOES_BREAKOFF']=='Y'&& $select!=='')
                        {
//                            $grd_id=DBGet(DBQuery('SELECT TITLE,ID,BREAK_OFF FROM report_card_grades WHERE BREAK_OFF <='.$select.' AND SYEAR=\''.UserSyear().'\' AND SCHOOL_ID=\''.UserSchool().'\' AND GRADE_SCALE_ID=\''.$dbf[1]['GRADE_SCALE_ID'].'\' ORDER BY BREAK_OFF IS NOT NULL DESC,BREAK_OFF DESC,SORT_ORDER LIMIT 0,1'));
//                            $prog_config_RET = DBGet(DBQuery('SELECT TITLE,VALUE FROM program_user_config WHERE CAST(VALUE AS UNSIGNED)<='.$select.' AND TITLE LIKE \''.UserCoursePeriod().'-'.'\' AND USER_ID=\''.User('STAFF_ID').'\' AND PROGRAM=\'Gradebook\' ORDER BY CAST(VALUE AS UNSIGNED) DESC LIMIT 0,1'));
//                            $tc_grade='n';
...
{	global $THIS_RET,$current_commentsB_RET,$import_commentsB_RET,$commentsB_RET,$max_current_commentsB,$commentsB_select,$tabindex;
	...
	if(!isset($_REQUEST['_openSIS_PDF']))
		...
		$return = '<small>'._($commentsB_RET[$select][1]['TITLE']).'</small>';
