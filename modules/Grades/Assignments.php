...
//                print_r($columns);
                if((isset($columns['DUE_DATE']) || isset($columns['ASSIGNED_DATE'])) && $err==false)
                {
                    ...
                    if(isset($columns['DUE_DATE']) && isset($columns['ASSIGNED_DATE']))
                    {
                        ...
                        {
                            $msg='<Font color=red>'._('Assigned date and due date must be within the current marking periods start date and end date.').'</FONT>';
                            $go=false;
                        }
                    }
                    if(isset($columns['DUE_DATE']) && !isset($columns['ASSIGNED_DATE']))
                    {
                        ...
                        {
                            $msg='<Font color=red>'._('Due date must be within the current marking periods start date and end date.').'</FONT>';
                            $go=false;
                    ...
                    if(!isset($columns['DUE_DATE']) && isset($columns['ASSIGNED_DATE']))
                    {
                        ...
                        {
                            $msg='<Font color=red>'._('Assigned date must be within the current marking periods start date and end date.').'</FONT>';
                            $go=false;
...
if($err)
{
    unset($_REQUEST['tables']);
     ...
     $_REQUEST['ajax'] = true;
    echo '<Font color=red>'._('Due date must be greater than assigned date.').'</FONT>';
}
		...
		/*else
		{
			...
			#	$sql = "DELETE FROM gradebook_assignment_types WHERE ASSIGNMENT_TYPE_ID='$_REQUEST[assignment_type_id]'";
				...
				PopTable('header','Unable to Delete');
						DrawHeaderHome('<font color=red>Assignment Type cannot be deleted a.</font>');
...
//	}
	if($_REQUEST['assignment_type_id'] || $_REQUEST['assignment_id'])
        ...
        {   
         ...
         $data=DBGet(DBQuery('select assignment_id from gradebook_assignments where assignment_type_id='.$_REQUEST['assignment_type_id'].''));
         if(count($data)>0)
             UnableDeletePromptMod(_('Gradebook Assignment Type cannot be deleted because assignments are created in this assignment type.'),'','modfunc=&assignment_type_id='.$_REQUEST['assignment_type_id']);
        else
        {
...
//		$header .= '<TR><TD valign=top colspan=2>'.($RET['DATE_ERROR']=='Y'?'<Font color=red>Due date earlier than assigned date!</FONT>':'').'</TD></TR>';
	...
	if($_REQUEST['assignment_type_id'] && $_REQUEST['assignment_type_id']!='new' && count($types_RET))
	{
		...
		ListOutput($assn_RET,$columns,_('Assignment'),_('Assignments'),$link,array(),$LO_options);
...
elseif(!$course_id)
	echo '<BR>'.ErrorMessage(array(_('You don\'t have a course this period.')),'error');
