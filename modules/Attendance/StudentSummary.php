...
if(isset($_REQUEST['student_id']) )
{
	...
	DrawHeaderHome( _('Selected Student').': '.$RET[1]['FIRST_NAME'].'&nbsp;'.($RET[1]['MIDDLE_NAME']?$RET[1]['MIDDLE_NAME'].' ':'').$RET[1]['LAST_NAME'].'&nbsp;'.$RET[1]['NAME_SUFFIX'].' (<A HREF=Side.php?student_id=new&modcat='.$_REQUEST['modcat'].'><font color=red>'._('Deselect').'</font></A>) | <A HREF=Modules.php?modname='.$_REQUEST['modname'].'&search_modfunc=list&next_modname='.$_REQUEST['modname'].'&ajax=true&bottom_back=true&return_session=true&&day_start='.$_REQUEST[day_start].'&&month_start='.$_REQUEST[month_start].'&&year_start='.$_REQUEST[year_start].'&&period_id='.$_REQUEST[period_id].'&&myclasses='.$_REQUEST[myclasses].'&&chk=1 target=body>'._('Back to Student List').'</A>');
...
if($_REQUEST['search_modfunc'] || $_REQUEST['student_id'] || UserStudentID() || User('PROFILE')=='parent' || User('PROFILE')=='student')
{
    ...
    DrawHeaderHome('<table><tr><td>'.PrepareDateSchedule($start_date,'_start').'</td><td> - </td><td>'.PrepareDateSchedule($end_date,'_end').'</td><td> : <INPUT type=submit name=absence_go class=btn_medium value='._('Go').'></td><td>',$period_select.'</td><td>'.$myclasses.'</td></tr></table>');
    echo '</FORM>';
}
...
if(UserStudentID())
{
    ...
    $columns['TEACHER_COMMENT'] = _('Teacher Comment');
    foreach($periods_RET as $period)
