...
if(!$_REQUEST['modfunc'])
{
	...
	DrawHeaderHome('<table><tr><td>'.PrepareDate($start_date,'_start').'</td><td> - </td><td>'.PrepareDate($end_date,'_end').'</td><td> - </td><td>'.$advanced_link,' : <INPUT type=submit value='._('Go').' class=btn_medium></td></tr></table>');
	echo '</FORM>';
	...
	else
	{
		...
		$student_days_possible = DBGet(DBQuery('SELECT ssm.GRADE_ID,
		 ...
		 $columns = array('GRADE_ID'=>''._('Grade').'','STUDENTS'=>''._('Students').'','DAYS_POSSIBLE'=>''._('Days Possible').'','ATTENDANCE_POSSIBLE'=>''._('Attendance Possible').'','PRESENT'=>''._('Present').'','ABSENT'=>''._('Absent').'','NOT_TAKEN'=>''._('Not Taken').'','ADA'=>''._('ADA').'','AVERAGE_ATTENDANCE'=>''._('Avg Attendance').'','AVERAGE_ABSENT'=>''._('Avg Absent').'');
...
ListOutput($student_days_possible,$columns,''._('Grade level').'',''._('Grade levels').'',$link);
