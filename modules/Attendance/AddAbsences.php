...
define('LANG_RECORDS_ADDED_CONFIRMATION',''._('Absence records were added for the selected students.').'.');
define('LANG_CHOOSE_STUDENT_ERROR',''._('You must choose at least one period and one student.').'');
...
if(!$_REQUEST['modfunc'])
{
	...
	if(optional_param('search_modfunc','',PARAM_NOTAGS)=='list')
	{
		...
		echo '<CENTER><TABLE><TR><TD align=right>'._('Add absence to periods').'</TD>';
		echo '<TD><TABLE><TR>';
		...
		echo '<TABLE><TR>';
		echo '<TH>'._('S').'</TH><TH>'._('M').'</TH><TH>'._('T').'</TH><TH>'._('W').'</TH><TH>'._('Th').'</TH><TH>'._('F').'</TH><TH>'._('S').'</TH></TR><TR>';
	...
	if(optional_param('search_modfunc','',PARAM_ALPHA)=='list')
		echo '<BR><CENTER>'.SubmitButton(_('Save'),'','class=btn_medium onclick="formload_ajax(\'addAbsences\');"')."</CENTER></FORM>";
}
