...
if(isset($_REQUEST['student_id']) && optional_param('student_id','',PARAM_ALPHANUM)!='new')
{
	...
	$schedule_RET = DBGet(DBQuery('SELECT
									...
									WHERE
										...
										AND ac.CALENDAR_ID=cp.CALENDAR_ID AND ac.SCHOOL_DATE=\''.$date.'\' AND ac.MINUTES!=0
									ORDER BY p.SORT_ORDER'),$functions);
	$columns = array('PERIOD_TITLE'=>''._('Period').'','COURSE'=>''._('Course').'','ATTENDANCE_CODE'=>''._('Attendance Code').'','ATTENDANCE_TEACHER_CODE'=>''._('Teacher\'s Entry').'','ATTENDANCE_REASON'=>''._('Comment').'');
        $tmp_req=$_REQUEST;
	...
	if(isset($_REQUEST['student_id']) )
{
	...
	DrawHeaderHome(''._('Selected Student: ').''.$RET[1]['FIRST_NAME'].'&nbsp;'.($RET[1]['MIDDLE_NAME']?$RET[1]['MIDDLE_NAME'].' ':'').$RET[1]['LAST_NAME'].'&nbsp;'.$RET[1]['NAME_SUFFIX'].' (<A HREF=Side.php?student_id=new&modcat='.$_REQUEST['modcat'].'><font color=red>'._('Deselect').'</font></A>) | <A HREF=Modules.php?modname='.$_REQUEST['modname'].'&search_modfunc=list&next_modname=Students/Student.php&codes[]='.$_SESSION[code][0].'&ajax=true&bottom_back=true&month_date='.$_REQUEST[month_date].'&day_date='.$_REQUEST[day_date].'&year_date='.$_REQUEST[year_date].' target=body  style="text-decoration:none"><strong>'._('Back to Student List').'</strong></A>');
	#-----------------------------------------------------newly added attendance code and the date in back to list--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	ListOutput($schedule_RET,$columns,_('Course'),_('Courses'));
                  echo '<BR><CENTER>'.SubmitButton(_('UPDATE'),'','class=btn_wide').'</CENTER>';
	echo '</FORM>';
}
else
{
	...
	$extra['columns_after']['DAILY_COMMENT'] = _('Comment');
	$extra['columns_after']['PHONE'] = _('Phone');
	...
	DrawHeader('<TABLE><TR><TD>'.PrepareDate($date,'_date',false,array('submit'=>true)).'</TD><TD>&nbsp;&nbsp;&nbsp;&nbsp;</TD><TD>'.SubmitButton(_('Go'),'','class=btn_medium').'</TD><TR></TABLE>','<TABLE><TR><TD>'.$current_student_link.button('add','',"# onclick='javascript:addHTML(\"".str_replace('"','\"',_makeCodeSearch())."\",\"code_pulldowns\"); return false;'").'</TD><TD><DIV id=code_pulldowns>'.$code_pulldowns.'</DIV></TD></TR></TABLE>');
	...
	if(count($categories_RET))
	{
		...
		$header .= '<TD width=10></TD><TD>'.DrawTab(_('Attendance'),$tmp_PHP_SELF.'&amp;table=0',$tabcolor,$textcolor,'_circle',array('textcolor'=>'#000000')).'</TD>';
...
{	global $codes_RET,$code_search_selected;
	...
	$return = '<SELECT name=codes[]><OPTION value="A"'.(($value=='A')?' SELECTED':'').'>'._('Not').''._('Present').'</OPTION>';
	if(count($codes_RET))
	{
		foreach($codes_RET as $code)
		{
