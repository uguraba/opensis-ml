...
if(optional_param('delete','',PARAM_ALPHA)=='true')
{
        ...
        if(DeletePrompt(_('Duplicate Attendance Record')))
        {
	     ...
	     $sid = optional_param('studentidx','',PARAM_ALPHANUM);
                ...
                echo _("The duplicate record(s) has been deleted.");
        ...
        if(count($RET))
        {
	       ...
	       $num_rows = $totalrows;
			...
			if($num_rows > $rows_per_page){
				...
				echo "<small>"._of." $totalpages "._pages."</small>";
			}
               ...
               echo "<TD class=grid_header>"._('Student (Student ID)')."</td>";
               echo "<TD class=grid_header>"._('Course (Course Period ID)')."</td>";
               echo "<TD class=grid_header>"._('Course Start Date')."</td>";
               echo "<TD class=grid_header>"._('Course End Date')."</td>";
               echo "<TD class=grid_header>"._('Attendance Date')."</td></tr>";
			...
			if($URIcount > $startrow && $URIcount < $endrow){
               ...
               if($count == 0){
                  echo "<tr class=odd><td colspan=6><b>"._('No Duplicates Found')."</td></tr>";
                  echo "</table>";
                }else{
                  echo "</table>";
                  echo "<br><input type=submit class=btn_medium name=submit value='._('Delete').'>";
                }
				DrawHeader('','',SubmitButton(''._('Delete').'','','class=btn_medium'));
               echo "</form>";
               $RET = " ";
