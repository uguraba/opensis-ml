...
if($_REQUEST['modfunc']=='search')
{
	...
	if(User('PROFILE')=='admin')
		echo '<CENTER><INPUT type=checkbox name=_search_all_schools value=Y'.(Preferences('DEFAULT_ALL_SCHOOLS')=='Y'?' CHECKED':'').'><font color=black>'._('Search All Schools').'</font></CENTER><BR>';
	echo '<CENTER>'.Buttons(_('Submit')).'</CENTER>';
...
if(!$_REQUEST['modfunc'])
{
   ...
   echo "<FORM action=Modules.php?modname=$_REQUEST[modname]&list_by_day=$_REQUEST[list_by_day] method=POST>";
    ...
    DrawHeaderHome('<table><tr><td>'.PrepareDate($start_date,'_start').'</td><td> - </td><td>'.PrepareDate($end_date,'_end').'</td><td> - </td><td>'.$advanced_link.'</td><td>&nbsp;&nbsp;&nbsp;&nbsp;'._('All School').' &nbsp;&nbsp; <input type="checkbox" value="Y" name="all_school" id="all_school" /></td><td> '._(':').' <INPUT type=submit value='._('Go').' class=btn_medium></td></tr></table>');
    echo '</FORM>';
	...
	//  $in_schools_raw = $sql_school[1]['SCHOOLS'];
        ...
        foreach ($sql_school as $school_key => $school_value) {
          ...
          #$sql_school = DBGet(DBQuery('SELECT CURRENT_SCHOOL_ID FROM staff WHERE STAFF_ID='.User('STAFF_ID')));
	  #$in_schools_raw = $sql_school[1]['CURRENT_SCHOOL_ID'];
	  ...
	  $sch_count = count($permited_schools);
		...
		*/
		...
		if($sch_count == 1)
		{
			...
			$columns = array('SCHOOL_DATE'=>''._('Date').'','STUDENTS'=>''._('Student Name').'','GRADE'=>''._('Grade').'','PERIOD'=>''._('Period Name').'','STATUS'=>''._('Attendance Status').'');
		}
		else
		{
			...
			$columns = array('SCHOOL_DATE'=>''._('Date').'','STUDENTS'=>''._('Student Name').'','GRADE'=>''._('Grade').'','PERIOD'=>''._('Period Name').'','STATUS'=>''._('Attendance Status').'','SCHOOL'=>''._('School').'');
		}
    ...
    else
    {
       ...
       $extra['WHERE'] .= ' GROUP BY ssm.GRADE_ID,ssm.CALENDAR_ID';
        ...
        $columns = array('GRADE_ID'=>''._('Grade').'','STUDENTS'=>''._('Total Attendance').'','DAYS_POSSIBLE'=>''._('Days Possible').'','PRESENT'=>''._('Present').'','ABSENT'=>''._('Absent').'','ADA'=>''._('ADA').'','AVERAGE_ATTENDANCE'=>''._('Average Attendance').'','AVERAGE_ABSENT'=>''._('Average Absent').'');
