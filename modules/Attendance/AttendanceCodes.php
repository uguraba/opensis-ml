	...
	*/	
    ...
    if($_REQUEST['table']!=='new') {
        if(count($attendance_codes_RET)==0) {
            ...
            if($_REQUEST['table']!=0)
                echo '<BR><CENTER>'.button('remove',''._('Delete this category').'',"Modules.php?modname=$_REQUEST[modname]&modfunc=remove&table=$_REQUEST[table]");
            echo '<BR><CENTER>'.SubmitButton(_('Save'),'','class=btn_medium onclick="formcheck_attendance_codes();"').'</CENTER>';
        ...
        echo '<BR><CENTER>'.SubmitButton(_('Save'),'','class=btn_medium onclick="formcheck_attendance_category();"').'</CENTER>';
        echo PopTable('footer');
    }
    echo '</FORM>';
}
