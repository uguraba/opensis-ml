...
if(isset($_REQUEST['student_id']) )
{
	...
	DrawHeaderHome( _('Selected Student').': '.$RET[1]['FIRST_NAME'].'&nbsp;'.($RET[1]['MIDDLE_NAME']?$RET[1]['MIDDLE_NAME'].' ':'').$RET[1]['LAST_NAME'].'&nbsp;'.$RET[1]['NAME_SUFFIX'].' (<A HREF=Side.php?student_id=new&modcat='.$_REQUEST['modcat'].'><font color=red>'._('Deselect').'</font></A>) | <A HREF=Modules.php?modname='.$_REQUEST['modname'].'&search_modfunc=list&next_modname='.$_REQUEST['modname'].'&ajax=true&bottom_back=true&return_session=true target=body>'._('Back to Student List').'</A>');
...
if($_REQUEST['attendance'] && ($_POST['attendance'] || $_REQUEST['ajax']) && AllowEdit())
{
    foreach($_REQUEST['attendance'] as $student_id=>$values)
    {
        foreach($values as $school_date=>$columns)
        {
           ...
           // $sql = substr($sql,0,-1) . " WHERE SCHOOL_DATE='".date('Y-m-d',strtotime($school_date))."' AND PERIOD_ID='".$_REQUEST['period_id']."' AND STUDENT_ID='".$student_id."'";
...
if($_REQUEST['search_modfunc'] || $_REQUEST['student_id'] || UserStudentID() || User('PROFILE')=='parent' || User('PROFILE')=='student')
{
    ...
    if(!UserStudentID() && !$_REQUEST['student_id'])
    {
       $periods_RET = DBGet(DBQuery('SELECT PERIOD_ID,TITLE FROM school_periods WHERE SYEAR=\''.UserSyear().'\' AND SCHOOL_ID=\''.UserSchool().'\' ORDER BY SORT_ORDER'));
        if(count($periods_RET)>1)
        {
            ...
            {
                $period_select .= "<OPTION value=".$period['PERIOD_ID'].(($_REQUEST['period_id']==$period['PERIOD_ID'])?' SELECTED':'').">"._($period['TITLE']).'</OPTION>';
            }
        }
    ...
    if(User('PROFILE')=='teacher')
    {
        ...
        $myclasses .='<OPTION value=""'.($_REQUEST['myclasses']==''?' SELECTED':'').'>'._('All course periods').'</OPTION>';
        $myclasses .='<OPTION value="my_classes"'.($_REQUEST['myclasses']=='my_classes'?' SELECTED':'').'>'._('All my course periods').'</OPTION>';
        $myclasses .='<OPTION value="selected_class"'.($_REQUEST['myclasses']=='selected_class'?' SELECTED':'').'>'._('Selected course period').'</OPTION>';
        $myclasses .='</SELECT>';
    ...
    DrawHeaderHome('<table><tr><td>'.PrepareDate($start_date,'_start').'</td><td> - </td><td>'.PrepareDate($end_date,'_end').'</td><td> : </td><td>'.$period_select.' : <INPUT type=submit class=btn_medium value='._('Go').'>'.(($_REQUEST['period_id'])? '<td>'.$myclasses.'</td>' : '').'</tr></table>');
}
...
$MP_TYPE='FY';
    ...
    {$_REQUEST['myclasses']='';
        $schedule_RET[1] = array('COURSE_PERIOD'=>''._('Daily Attendance').'','COURSE_PERIOD_ID'=>'0');
        if($_REQUEST['myclasses']!=''){
	...
	echo '<div style="width:820px; overflow:auto; overflow-x:scroll;">';
...
else
{
    ...
    if(!$_REQUEST['period_id'])
    {$_REQUEST['myclasses']='';
      # $sql= "SELECT ad.SCHOOL_DATE,'0' AS COURSE_PERIOD_ID,ad.STATE_VALUE AS STATE_CODE,".db_case(array('ad.STATE_VALUE',"'0.0'","'A'","'1.0'","'P'","'H'"))." AS SHORT_NAME FROM attendance_day ad WHERE ad.SCHOOL_DATE BETWEEN '".date('Y-m-d',strtotime($start_date))."' AND '".date('Y-m-d',strtotime($end_date))."'";
//working
        if($_REQUEST['myclasses']!='')
        {
