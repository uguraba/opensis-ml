...
if($function(_('Confirm Scheduler Run'),_('Are you sure you want to run the scheduler?'),
    '<TABLE>
        
    <TR><TD><INPUT type=checkbox name=test_mode   value=Y onclick=showhidediv("div1",this);></TD><TD>'._('Schedule Unscheduled Requests').'</TD></TR>
    <TR><TD></TD><TD><div id=div1 style=display:none>'.PrepareDateSchedule($date,'_date',false,'').'</div></TD></TR>
 
    <TR><TD><INPUT type=checkbox name=delete_mode value=Y></TD><TD>'._('Delete Current Schedules').'</TD></TR>
    </TABLE>'))
{
	#echo '<BR>';
  ...
  $cp_parent_RET = DBGet(DBQuery($sql),array(),array('PARENT_ID'));
	...
	foreach($requests_RET as $request_id=>$request)
	{
		...
		if($percent>$last_percent)
		{
			...
			echo 'addHTML("'.$percent.''._('% Done').'","percentDIV",true);'."\r";
			echo '</script>';
			ob_flush();
			flush();
				...
				foreach($course_periods as $period_id=>$course_period)
				{
					$scount++;
                                            if($course_period['AVAILABLE_SEATS']!=0)
                                            {
                                                    ...
                                                    $get_is=DBGet(DBQuery('SELECT IGNORE_SCHEDULING FROM school_periods sp,course_periods cp WHERE cp.PERIOD_ID=sp.PERIOD_ID AND cp.COURSE_PERIOD_ID=\''.$course_period['COURSE_PERIOD_ID'].'\''));
                                                    if($existing_cp_RET && $get_is[1]['IGNORE_SCHEDULING']!='Y')
                                                        $cp_conflicts=true;
...
//print_r($course_period);
	...
	if(count($check_request)>0)
		$warn = ''._('Following Students cannot be accommodated as No More Seats Available or Periods Conflict').'';
	//if($_REQUEST['delete_mode']=='Y' || count($check_request)==0 || $slice['AVAILABLE_SEATS']>0)
        if($_REQUEST['delete_mode']=='Y' || count($check_request)==0)
	{
		echo '<script language="javascript">'."\r";
		echo 'addHTML("<IMG SRC=assets/check.gif> <B>'._('Done.').'</B>","percentDIV",true);'."\r";
		echo '</script>';
		ob_end_flush();
	}
	elseif($warn)
	{
		echo '<script language="javascript">'."\r";
		echo 'addHTML("<B><font color=red>'._('Warning').'</font><br>'.$warn.'</B>","percentDIV",true);'."\r";
		echo '</script>';
		ob_end_flush();
	}
	else
	{
		echo '<script language="javascript">'."\r";
		echo 'addHTML("<B><font color=red>'._('Error').'</font><br>'._('No Seats Available').'</B>","percentDIV",true);'."\r";
		echo '</script>';
		ob_end_flush();
	}
...
function Prompt_Home_Schedule($title='Confirm',$question='',$message='',$pdf='')
{
   	...
   	if(!$_REQUEST['delete_ok'] &&!$_REQUEST['delete_cancel'])
	{
		...
		PopTable('header',$title);
                    ...
                    echo "<CENTER><h4>$question</h4><FORM name=run_schedule action=$PHP_tmp_SELF&delete_ok=1 METHOD=POST onSubmit='return confirmAction();'>$message<BR><BR><INPUT type=submit class=btn_medium value="._('OK')." >&nbsp;<INPUT type=button class=btn_medium name=delete_cancel value="._('Cancel')." onclick='window.location=\"Modules.php?modname=misc/Portal.php\"'></FORM></CENTER>";
		PopTable('footer');
		return false;
	}
	else
