...
if($_REQUEST['course_modfunc']=='search')
{
	...
	if($_REQUEST['search_term'])
	{
		...
		ListOutput($subjects_RET,array('TITLE'=>''._('Subject').''),''._('Subject').'',''._('Subjects').'',$link);
		echo '</TD><TD valign=top>';
		$link['TITLE']['link'] = "Modules.php?modname=$_REQUEST[modname]&modfunc=$_REQUEST[modfunc]";
		//$link['TITLE']['link'] = "#"." onclick='check_content(\"ajax.php?modname=$_REQUEST[modname]&modfunc=$_REQUEST[modfunc]\");'";
		$link['TITLE']['variables'] = array('subject_id'=>'SUBJECT_ID','course_id'=>'COURSE_ID');
		ListOutput($courses_RET,array('TITLE'=>''._('Course').''),''._('Course').'',''._('Courses').'',$link);
		echo '</TD><TD valign=top>';
		$link['TITLE']['link'] = "Modules.php?modname=$_REQUEST[modname]&modfunc=$_REQUEST[modfunc]";
		//$link['TITLE']['link'] = "#"." onclick='check_content(\"ajax.php?modname=$_REQUEST[modname]&modfunc=$_REQUEST[modfunc]\");'";
		$link['TITLE']['variables'] = array('subject_id'=>'SUBJECT_ID','course_id'=>'COURSE_ID','course_period_id'=>'COURSE_PERIOD_ID');
		ListOutput($periods_RET,array('TITLE'=>''._('Course Period').''),''._('Course Period').'',''._('Course Periods').'',$link);
		echo '</TD></TR></TABLE>';
	}
}
...
if((!$_REQUEST['modfunc'] || $_REQUEST['modfunc']=='choose_course') && !$_REQUEST['course_modfunc'])
{
	...
	if($_REQUEST['modfunc']!='choose_course')
	{
		...
		if($_REQUEST['course_period_id'])
		{
			...
			if($_REQUEST['course_period_id']!='new')
			$header .= '<TD><FONT color=green>' .$available_seats. '</FONT><BR><FONT color=gray><SMALL>'._('Available Seats').'</SMALL></FONT></TD>';
         ...
         //		--------------------------------------------- Temp Coment ------------------------------------------------- 	//
		     ...
		     # $header .= "<TD colspan=2><DIV id=course_div>".$parent."</DIV> ".($parent!='N/A'?"<A HREF=# onclick='window.open(\"for_window.php?modname=".$_REQUEST['modname']."&modfunc=choose_course\",\"\",\"scrollbars=yes,resizable=yes,width=800,height=400\");'>Choose</A><BR>":'')."<small><FONT color=".Preferences('TITLES').">Parent Course Period</FONT></small></TD>";
			  $header .= "<TD colspan=2><DIV id=course_div>".$parent."</DIV> ".($parent!='N/A'?"<A HREF=# onclick='window.open(\"for_window.php?modname=misc/ChooseParentCourse.php\",\"\",\"scrollbars=yes,resizable=yes,width=800,height=400\");'>Choose</A><BR>":'')."<small><FONT color=".Preferences('TITLES').">"._('Parent Course Period')."</FONT></small></TD>";
