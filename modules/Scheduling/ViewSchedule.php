...
if(UserStudentID())
{
        ...
        {
            case 'day_view':
                DrawHeaderHome('<table width="100%" cellpadding="2" cellspacing="2"><tr><td align="left" width="175" valign="top"><table><tr><td>'.PrepareDateSchedule($date,'_date',false,array('submit'=>true)).'</td><td><INPUT type=submit class=btn_medium value='._('Go').'></td></tr></table></td><td align="left">'._('Marking Period').' :  '.$mp.'</td><td align="right">'._('Calendar').' '._('View').' : '.$view_mode).'</td></tr></table>';
...
for($i=1;$i<=count($mp_data);$i++)
{
	...
	$columns = array('TIME_PERIOD'=>''._('Period').'','TITLE'=>''._('Course').'','PERIOD_PULLDOWN'=>''._('Period - Teacher').'','ROOM'=>''._('Room').'','DAYS'=>''._('Days of Week').'','COURSE_MARKING_PERIOD_ID'=>''._('Term').'');
	...
	if($_REQUEST['_openSIS_PDF'])
        ...
        case 'week_view':
            ...
            DrawHeaderHome('<table width="100%" cellpadding="3" cellspacing="2"><tr><td style="padding-right:20px;">'._('Marking Period').' : '.$mp.'</td><td style="padding-right:20px;" align="left">'.$week_range.'</td><td align="right">'._('Calendar')._(' ')._('View').' : '.$view_mode.'</td></tr></table>');
            $one_day=60*60*24;
	...
	$wk_schedule_RET = DBGet(DBQuery('SELECT sp.PERIOD_ID,CONCAT(sp.START_TIME,\''.' - '.'\',sp.END_TIME) AS TIME_PERIOD,sp.TITLE FROM school_periods sp WHERE sp.SYEAR=\''.UserSyear().'\' AND sp.SCHOOL_ID = \''.UserSchool().'\' ORDER BY sp.SORT_ORDER'));
				...
				INNER JOIN school_periods sp ON sp.SYEAR=acc.SYEAR AND sp.SCHOOL_ID=acc.SCHOOL_ID AND sp.PERIOD_ID=cp.PERIOD_ID
                  ...
                  $columns = array('TIME_PERIOD'=>''._('Period').'');
                 ...
                 if(count($week_RET))
                {
                    foreach($wk_schedule_RET as $course)
                    {
                        ...
                        for($j=$today;$j<=$today+$one_day*6;$j=$j+$one_day){
                            ...
                            if(!$day_RET)
                                $schedule_RET[$i][date('y-m-d',$j)] ='<div align=center title="'._('Schedule not available').'">--</div>';
                            else
                                $schedule_RET[$i][date('y-m-d',$j)] =(count($day_RET)>1?'<font title="'._('Conflict schedule').' ('.count($day_RET).')" color="red">'.$day_RET[1]['TITLE'].'<br />'._('Room').' :'.$day_RET[1]['ROOM'].'</font>' : '<spna title='.date("l",$j).'>'.$day_RET[1]['TITLE'].'<br />'._('Room').' :'.$day_RET[1]['ROOM'].'</span>');
                            }
                        }
                    }
                }
            ...
            case 'month_view':
                ...
                DrawHeaderHome('<table cellpadding="2" cellspacing="2" width="100%"><tr><td style="padding-right:20px;">'._('Marking Period').' :  '.$mp.'</td><td>'.$month_str.'</td><td align="right">'._('Calendar')._(' ')._('View').' : '.$view_mode.'</td></tr></table>');
                $fy_id = DBGet(DBQuery('SELECT MARKING_PERIOD_ID FROM school_years WHERE SYEAR=\''.UserSyear().'\' AND SCHOOL_ID=\''.UserSchool().'\''));
                $fy_id = $fy_id[1]['MARKING_PERIOD_ID'];
...
//                  ++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	echo "<TD class=white>"._('Monday')."</TD><TD class=white>"._('Tuesday')."</TD><TD class=white>"._('Wednesday')."</TD><TD class=white>"._('Thursday')."</TD><TD class=white>"._('Friday')."</TD><TD class=white>"._('Saturday')."</TD><TD width=99 class=white>"._('Sunday')."</TD>";
	echo "</TR><TR>";
                                    ...
                                    if($calendar_RET[$date][1]['MINUTES']){
                                        if(count($schedule_RET)>0){
                                            foreach($schedule_RET as $cp_link){
                                                echo "<a HREF=# title=Details onclick='javascript:window.open(\"for_window.php?modname=$_REQUEST[modname]&modfunc=detail&date=$date&marking_period_id=$_REQUEST[marking_period_id]&period=$cp_link[PERIOD_ID]\",\"blank\",\"width=600,height=450,scrollbars=1\"); return false;'>$cp_link[START_TIME] &nbsp;&nbsp;$cp_link[TITLE]<a><br />";
                                            }
                                        }
                                        else
                                            echo _('Schedule not available');
                                    }
                                    else
                                        echo '<font color=red>'._('Holiday').'</font>';
                                    echo "</td></tr>";
		echo "</table></TD>";
		$return_counter++;
...
//                  +++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	ListOutput($schedule_RET,$columns,''._('Period').'',''._('Periods').'',$link,array(),array('search'=>false));
                  if($_REQUEST['modfunc']=='detail')
                      echo '<br /><div align="center"><input type="button" class=btn_medium value="'._('Close').'" onclick="window.close();"></div>';
            }

            if($schedule_RET && $_REQUEST['view_mode']=='day_view')
                        DrawHeader( "<table><tr><td>&nbsp;&nbsp;</td><td>". (ProgramLinkforExport('Scheduling/PrintSchedules.php','<img src=assets/print.png>','&modfunc=save&st_arr[]='.UserStudentID().'&mp_id='.$mp_id.'&include_inactive='.$_REQUEST['include_inactive'].'&_openSIS_PDF=true target=_blank'))."</td><td>". (ProgramLinkforExport('Scheduling/PrintSchedules.php',''._('Print Schedule').'','&modfunc=save&st_arr[]='.UserStudentID().'&mp_id='.$mp_id.'&include_inactive='.$_REQUEST['include_inactive'].'&_openSIS_PDF=true target=_blank'))."</td></tr></table>");
	echo '</FORM>';
}
...
function CreateSelect($val, $name, $link='', $mpid)
	{
		...
		if($link!='')
		$html .= "<select title='"._('Marking periods')."' name=".$name." id=".$name." onChange=\"window.location='".$link."' + this.options[this.selectedIndex].value;\">";
		else
        ...
        {
            if($link!='')
                $html .= "<select title="._('View mode')." name='view_mode' id='view_mode' onChange=\"window.location='".$link."' + this.options[this.selectedIndex].value;\">";
            else
                $html .= "<select name='view_mode' id='view_mode'>";
            
            $html .= '<option value="day_view" '.($_REQUEST['view_mode']=='day_view'? 'selected' : '').' >'._('Day').'</option>';
            $html .= '<option value="week_view" '.($_REQUEST['view_mode']=='week_view'? 'selected' : '').'>'._('Week').'</option>';
            $html .= '<option value="month_view" '.($_REQUEST['view_mode']=='month_view'? 'selected' : '').'>'._('Month').'</option>';
            $html .= "</select>";
            return $html;
        }
...
function _makeMonths($link)
{
    ...
    if($link!=''){
      $html .= "<strong><a href='javascript:void(0);' title="._('Previous')." onClick=\"window.location='".$link.$prev."';\" style=\"font-size:12px;\">&lt;&lt; "._('Prev')."</a> &nbsp; &nbsp; <span style=\"font-size:12px;\">".date('F', $_REQUEST['month'])."&nbsp;".date('Y', $_REQUEST['month'])."</span> &nbsp; &nbsp; <a href='javascript:void(0);' title="._('Next')." onClick=\"window.location='".$link.$next."';\" style=\"font-size:12px;\">"._('Next')." &gt;&gt;</a></strong>";
    }
