...
//	{
		...
		# ------------------  Without Period Selection Request Entry Start  ------------------------------------ #
			...
			foreach($_REQUEST['student'] as $student_id=>$yes)
			{
				...
				{
					$duplicate = "<span class=red>"._('Duplicate Entry.Request already exists')."</span>";
			...
			{
				unset($_REQUEST['modfunc']);
				$note = ""._('That course has been added as a request for the selected students.')."";
			}
...
if($_REQUEST['modfunc']!='choose_course')
{
	...
	if($_REQUEST['search_modfunc']=='list')
	{
		...
		echo '<TABLE><TR><TD align=right>'._('Request to Add').'</TD><TD><DIV id=course_div>';
		if($_SESSION['MassRequests.php'])
		{
	...
	#	$extra['search'] .= "<TR><TD align=right width=120>Request</TD><TD><DIV id=request_div></DIV> <A HREF=# onclick='window.open(\"for_window.php?modname=misc/ChooseRequest.php\",\"\",\"scrollbars=yes,resizable=yes,width=800,height=400\");'><SMALL>Choose</SMALL></A></TD></TR>";		
		...
		echo '<DIV id=WITH_TEACHER_PERIOD ><TABLE><TR><TD align=right>'._('Teacher').'</TD><TD><SELECT name=with_teacher_id><OPTION value="">'._('N/A').'</OPTION>';
		...
		echo '<BR><DIV ID=WITHOUT_TEACHER_PERIOD><TABLE><TR><TD align=right>'._('Teacher').'</TD><TD><SELECT name=without_teacher_id><OPTION value="">'._('N/A').'</OPTION>';
		foreach($teachers_RET as $teacher)
			echo '<OPTION value='.$teacher['STAFF_ID'].'>'.$teacher['LAST_NAME'].', '.$teacher['FIRST_NAME'].' '.$teacher['MIDDLE_NAME'].'</OPTION>';
		echo '</SELECT></TD></TR><TR><TD align=right>'._('Period').'</TD><TD><SELECT name=without_period_id><OPTION value="">'._('N/A').'</OPTION>';
		foreach($periods_RET as $period)
			echo '<OPTION value='.$period['PERIOD_ID'].'>'.$period['TITLE'].'</OPTION>';
		echo '</SELECT></TD></TR></TABLE></DIV>';
		echo '</TD></TR>';
...
//	}
	...
	else
	{
		...
		//***WITH TEACHER_PERIOD*************************************************************
	    $tp_html='';
		$tp_html.='<TABLE><TR><TD align=right>'._('Teacher').'</TD><TD><SELECT name=with_teacher_id><OPTION>'._('N/A').'</OPTION>';
		...
		$tp_html.= '</SELECT></TD></TR><TR><TD align=right>'._('Period').'</TD><TD><SELECT name=with_period_id><OPTION>'._('N/A').'</OPTION>';
		...
		$tp_html_w='';
		$tp_html_w.='<BR><TABLE><TR><TD align=right>'._('Teacher').'</TD><TD><SELECT name=without_teacher_id><OPTION>'._('N/A').'</OPTION>';
		$corr_teachers = DBGet(DBQuery("SELECT Distinct s.FIRST_NAME,s.LAST_NAME,s.STAFF_ID AS TEACHER_ID FROM staff s,course_periods cp WHERE s.STAFF_ID=cp.TEACHER_ID AND cp.COURSE_ID='".$c."'"));
		foreach($corr_teachers as $teacher)
		{
		$tp_html_w.= '<OPTION value='.$teacher['TEACHER_ID'].'>'.$teacher['LAST_NAME'].', '.$teacher['FIRST_NAME'].'</OPTION>';
		}
		$tp_html_w.= '</SELECT></TD></TR><TR><TD align=right>'._('Period').'</TD><TD><SELECT name=without_period_id><OPTION>'._('N/A').'</OPTION>';
		$corr_periods = DBGet(DBQuery("SELECT Distinct p.TITLE,p.PERIOD_ID FROM school_periods p,course_periods cp WHERE p.PERIOD_ID=cp.PERIOD_ID AND cp.COURSE_ID='".$c."'"));
		foreach($corr_periods as $period)
		{
