...
if(UserStudentID())
{
	...
	DrawHeader('','<INPUT type=submit value='._('Save').' onclick=\'formload_ajax("vv")\';>');
}
...
if($_REQUEST['modfunc']=='choose')
{
	...
	foreach($subjects_RET as $key=>$value)
	{
		...
		$html[$key] .= "</SELECT></TD>";
		$html[$key] .= "<TD><SELECT name=course_weights[$key][]><OPTION value=''>"._('Not Specified')."</OPTION>";
	...
	if(count($requests_RET))
	{
		foreach($requests_RET as $key=>$requests)
		{
			foreach($requests as $value)
			{
				...
				$select_html[$key] .= "<TD><SELECT name=course_weights[$key][]><OPTION value=''>"._('Not Specified')."</OPTION>";
