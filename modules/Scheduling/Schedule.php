...
{   
    ...
    if( mysql_num_rows($association_query_attendance)>0 )
    {
    UnableDeletePrompt(''._('Cannot delete because students attendance are already taken.').'');  
    ...
    {
        UnableDeletePrompt(''._('Cannot delete because assignments grading are already given.').'');  
    #DBQuery("Delete from schedule where student_id='".UserStudentId() ."' and course_id=");
    unset($_REQUEST['del']);
    unset($_REQUEST['c_id']);
    }
    ...
    {
        UnableDeletePrompt(''._('Cannot delete because final grade is already given .').'');  
    #DBQuery("Delete from schedule where student_id='".UserStudentId() ."' and course_id=");
    unset($_REQUEST['del']);
    unset($_REQUEST['c_id']);
    }
...
$mp_RET = DBGet(DBQuery('SELECT MARKING_PERIOD_ID,TITLE,SORT_ORDER,1 AS TBL FROM school_years WHERE SYEAR=\''.UserSyear().'\' AND SCHOOL_ID=\''.UserSchool().'\' UNION SELECT MARKING_PERIOD_ID,TITLE,SORT_ORDER,2 AS TBL FROM school_semesters WHERE SYEAR=\''.UserSyear().'\' AND SCHOOL_ID=\''.UserSchool().'\' UNION SELECT MARKING_PERIOD_ID,TITLE,SORT_ORDER,3 AS TBL FROM school_quarters WHERE SYEAR=\''.UserSyear().'\' AND SCHOOL_ID=\''.UserSchool().'\' ORDER BY TBL,SORT_ORDER'));
  ...
  $mp = CreateSelect($mp_RET, 'marking_period_id', 'Modules.php?modname='.$_REQUEST['modname'].'&marking_period_id=', $_REQUEST['marking_period_id']);
	...
	$link['add']['title'] = ''._('Add a Course').'';
        ...
        $columns = array('ACTION'=>''._('Action').'','TITLE'=>''._('Course ').'','PERIOD_PULLDOWN'=>''._('Period - Teacher').'','ROOM'=>''._('Room').'','DAYS'=>''._('Days of Week').'','COURSE_MARKING_PERIOD_ID'=>''._('Term').'','SCHEDULER_LOCK'=>'<IMG SRC=assets/locked.gif border=0>','START_DATE'=>''._('Enrolled').'','END_DATE'=>''._('Dropped').'','SCHEDULE_ID'=>''._('More info').'');
	...
	if($_REQUEST['_openSIS_PDF'])
        ...
        {
             echo "<b style='color:red'>"._('Enrolled Date Cannot Be After Dropped Date')."</b>";
        
             unset($start_date_msg);
        }
        if($start_date_msg=="enroll")
        {
             echo "<b style='color:red'>"._('Course Enrolled Date Cannot Be Before Students School Start Date')."</b>";

             unset($start_date_msg);
        }
        if($start_date_msg=="prev_schdl_error")
        {
             echo "<b style='color:red'>"._('Course Enrolled Date Cannot Be Before Dropdate Of Previous Schedule')."</b>";
...
//             echo "<b style='color:red'>You Cannot Modify This Schedule As It Has Clash With Other Dropped Out Course</b>";
             echo "<b style='color:red'>"._('You cannot modify the schedule enrolled date, as its clashing with other dropped course')."</b>";
        ...
        if($end_date_msg=="end")
        {
            echo "<b style='color:red'>"._('Please enter proper dropped date. Dropped date must be greater than start date.')."</b>";
...
//        }
        if($sch_lock_msg=="end")
        {
            echo "<b style='color:red'>"._('This Schedule is locked,dropped date can not be changed.')."</b>";
        ...
        if($end_date_msg=="attn" && strtotime($end_d)<strtotime($_SESSION['last_attendance']))
        {
            ...
            echo "<b style='color:red'>"._('Course cannot be dropped because student has got attendance till ')."".$_SESSION['last_attendance'].".</b>";
        
            unset($end_date_msg);
        }
	VerifySchedule($schedule_RET);
	echo '<div style="width:820px; overflow:auto; overflow-x:scroll; padding-bottom:8px;">';
	ListOutput($schedule_RET,$columns,''._('Course').'',''._('Courses').'',$link);
	echo '</div>';
	...
	//    echo '<BR><CENTER>'.SubmitButton(''._('Save').'','','class=btn_medium onclick=\'formload_ajax("modify");\'').'</CENTER>';
	
             echo '<BR><CENTER>'.SubmitButton(''._('Save').'','','class=btn_medium ').'</CENTER>';
        }
...
if(clean_param($_REQUEST['modfunc'],PARAM_ALPHAMOD)=='more_info')
{
    $sql = 'SELECT
                                ...
                                $columns = array('TITLE'=>''._('Course ').'','PERIOD_PULLDOWN'=>''._('Period - Teacher').'','ROOM'=>''._('Room').'','DAYS'=>''._('Days of Week').'','COURSE_MARKING_PERIOD_ID'=>''._('Term').'','SCHEDULER_LOCK'=>'<IMG SRC=assets/locked.gif border=0>','START_DATE'=>''._('Enrolled').'','END_DATE'=>''._('Dropped').'','MODIFIED_NAME'=>''._('Modified By').'','MODIFIED_DATE'=>''._('Modified Date').'');
                                $options=array('search'=>false,'count'=>false,'save'=>false,'sort'=>false);

                                ListOutput($schedule_RET,$columns,''._('Course').'',''._('Courses').'',$link,'',$options);
...
{	global $_openSIS,$THIS_RET,$fy_id;
	...
	if(!$_openSIS['_makeMPSelect'])
	{
		...
		$_openSIS['_makeMPSelect'][$fy_id][1] = array('MARKING_PERIOD_ID'=>"$fy_id",'TITLE'=>''._('Full Year').'','SEMESTER_ID'=>'');
