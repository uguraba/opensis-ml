...
else
{
	...
	if(count($RET))
	{
		...
		$handle = PDFStart();
			...
			echo "<tr><td width=105>".DrawLogo()."</td><td  style=\"font-size:15px; font-weight:bold; padding-top:20px;\">". GetSchool(UserSchool())."<div style=\"font-size:12px;\">"._('Student Print Request')."</div></td><td align=right style=\"padding-top:20px;\">". ProperDate(DBDate()) ."<br \>"._('Powered by openSIS')."</td></tr><tr><td colspan=3 style=\"border-top:1px solid #333;\">&nbsp;</td></tr></table>";
		foreach($RET as $student_id=>$courses)
		{
			...
			{
			echo "<tr><td>"._('Student Mailling Label ').":</td>";
			...
			echo'</table>';
				//echo '<BR><BR><TABLE width=100%><TR><TD width=50> &nbsp; </TD><TD>'.$address[1]['MAILING_LABEL'].'</TD></TR></TABLE><BR>';
					ListOutputPrint($address,$columns,''._('Request').'',''._('Requests').'',array(),array(),array('center'=>false,'print'=>false));
					echo '<!-- NEW PAGE -->';				
				}
			}
			else
			...
			foreach($courses as $key=>$value){
				...
				if($courses[$key]['WITH_TEACHER_ID']){
                                    $get_mp_id=DBGet(DBQuery('SELECT MARKING_PERIOD_ID FROM course_periods WHERE COURSE_ID=\''.$value['COURSE_ID'].'\' AND PERIOD_ID=\''.$value['WITH_PERIOD_ID'].'\' '));
				...
				ListOutputPrint($courses,$columns,''._('Request').'',''._('Requests').'',array(),array(),array('center'=>false,'print'=>false));
				echo '<!-- NEW PAGE -->';
			}
		}
		PDFStop($handle);
