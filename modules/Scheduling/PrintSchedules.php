		...
		echo "<meta charset='UTF-8'><table width=100%  style=\" font-family:Arial; font-size:12px;\" >";
			...
			if($_REQUEST['mailing_labels']=='Y')
			{
				foreach($courses as $address)
				{
			       ...
			       {
					echo '<tr><td valign=top>'._('Malling Details').':</td>';
					...
					ListOutputPrint($address,$columns,''._('Course').'',''._('Courses').'',array(),array(),array('center'=>false,'print'=>false));
...
if(!$_REQUEST['modfunc'])
{
	...
	if($_REQUEST['search_modfunc']=='list')
	{
		...
		echo '<BR><CENTER><INPUT type=submit class=btn_xlarge value=\''._('Create Schedules').''._('for').''._('Selected Students').'\'></CENTER>';
		echo "</FORM>";
	}
}
