...
if($_REQUEST['modfunc']=='save')
{
	if(count($_REQUEST['cp_arr']))
	{
		...
		foreach($course_periods_RET as $course_period)
		{
			...
			echo "<tr><td width=105>".DrawLogo()."</td><td  style=\"font-size:15px; font-weight:bold; padding-top:20px;\">". GetSchool(UserSchool())."<div style=\"font-size:12px;\">"._('Student Class Pictures')."</div></td><td align=right style=\"padding-top:20px;\">". ProperDate(DBDate()) ."<br />"._('Powered by openSIS')."</td></tr><tr><td colspan=3 style=\"border-top:1px solid #333;\">&nbsp;</td></tr></table>";
...
if(!$_REQUEST['modfunc'])
{
	...
	//	echo "<FORM action=Modules.php?modname=$_REQUEST[modname]&modfunc=save&_openSIS_PDF=true method=POST>";
		...
		$extra['extra_header_left'] .= '<TD><INPUT type=checkbox name=last_year value=Y>'._('Use')._(' ')._('Last')._(' ')._("Year's")._(' ')._('if')._("Missing").'</TD></TR>';
	...
	if($_SESSION['count_course_periods']!=0)	
            echo '<BR><CENTER><INPUT type=submit class=btn_xxlarge value=\''._('Create Class Pictures for Selected Course Periods').'\'></CENTER>';
		echo "</FORM>";
	}
}
...
{	global $extra;
	...
	if(($_REQUEST['search_modfunc']=='search_fnc' || !$_REQUEST['search_modfunc']))
	{
		...
		foreach($RET as $subject)
			echo "<meta charset='UTF-8'><OPTION value=$subject[SUBJECT_ID]>$subject[TITLE]</OPTION>";
		echo '</SELECT>';
	...
	$_SESSION['count_course_periods'] = count($course_periods_RET);	
                $LO_columns = array('COURSE_PERIOD_ID'=>'</A><INPUT type=checkbox value=Y name=controller onclick="checkAll(this.form,this.form.controller.checked,\'cp_arr\');"><A>','TITLE'=>''._('Course Period').'');
