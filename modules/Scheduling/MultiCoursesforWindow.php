...
if($_REQUEST['course_modfunc']=='search')
{
	...
	if($_REQUEST['search_term'])
	{
		...
		ListOutput($subjects_RET,array('TITLE'=>''._('Subject').''),''._('Subject').'',''._('Subjects').'',$link,array(),array('search'=>false,'save'=>false));
		echo '</TD><TD valign=top>';
		$link['TITLE']['link'] = "for_window.php?modname=$_REQUEST[modname]&modfunc=$_REQUEST[modfunc]";
		//$link['TITLE']['link'] = "#"." onclick='check_content(\"ajax.php?modname=$_REQUEST[modname]&modfunc=$_REQUEST[modfunc]\");'";
		$link['TITLE']['variables'] = array('subject_id'=>'SUBJECT_ID','course_id'=>'COURSE_ID');
		ListOutput($courses_RET,array('TITLE'=>''._('Course').''),''._('Course').'',''._('Courses').'',$link,array(),array('search'=>false,'save'=>false));
		echo '</TD><TD valign=top>';
		$link['TITLE']['link'] = "for_window.php?modname=$_REQUEST[modname]&modfunc=$_REQUEST[modfunc]";
		//$link['TITLE']['link'] = "#"." onclick='check_content(\"ajax.php?modname=$_REQUEST[modname]&modfunc=$_REQUEST[modfunc]\");'";
		$link['TITLE']['variables'] = array('subject_id'=>'SUBJECT_ID','course_id'=>'COURSE_ID','course_period_id'=>'COURSE_PERIOD_ID');
		ListOutput($periods_RET,array('TITLE'=>''._('Course Period').''),''._('Course Period').'',''._('Course Periods').'',$link,array(),array('search'=>false,'save'=>false));
		echo '</TD></TR></TABLE>';
	}
}
...
if((!$_REQUEST['modfunc'] || $_REQUEST['modfunc']=='choose_course') && !$_REQUEST['course_modfunc'])
{
	...
	if($_REQUEST['modfunc']!='choose_course')
	{
		...
		if($_REQUEST['course_period_id'])
		{
			...
			DrawHeaderHome($title,$delete_button.SubmitButton(''._('Save').'','','class=btn_medium onclick="formcheck_scheduling_course_F2();"'));
			...
			$header .= '<TD>' . SelectInput($RET['PERIOD_ID'],'tables[course_periods]['.$_REQUEST['course_period_id'].'][PERIOD_ID]',''._('Period').'',$periods) . '</TD>';
			$header .= '<TD>';
			if($new==false)
			...
			$header .= '<TD>' . SelectInput($RET['MARKING_PERIOD_ID'],'tables[course_periods]['.$_REQUEST['course_period_id'].'][MARKING_PERIOD_ID]',''._('Marking Period').'',$options,false) . '</TD>';
			...
			$header .= '<TD>' . CheckboxInput($RET['DOES_ATTENDANCE'],'tables[course_periods]['.$_REQUEST['course_period_id'].'][DOES_ATTENDANCE]',''._('Takes Attendance').'',$checked,$new,'<IMG SRC=assets/check.gif height=15 vspace=0 hspace=0 border=0>','<IMG SRC=assets/x.gif height=15 vspace=0 hspace=0 border=0>') . '</TD>';
        ...
        echo '<center><p><font color=#FF0000><b>'._('Please Select A Course Period.').'</b></font></p></center>';
        ...
        //                                        $st_time = DBGet(DBQuery("SELECT START_TIME, END_TIME FROM SCHOOL_PERIODS WHERE PERIOD_ID = $period_id "));    /********* for homeroom scheduling*/
            ...
            {
                                    $course_title=DBGet(DBQuery('SELECT TITLE FROM course_periods WHERE COURSE_PERIOD_ID=\''.$select_cpid.'\' AND SCHOOL_ID=\''.UserSchool().'\' AND SYEAR=\''.UserSyear().'\''));
                                    $warnings.= ''._('There is a period time clash for ').''.$course_title[1]['TITLE'].'.';
                                    $clash[] = $warnings;
                                    continue;
            }
          }
                                    ...
                                    {
                                          $course_title=DBGet(DBQuery('SELECT TITLE FROM course_periods WHERE COURSE_PERIOD_ID=\''.$select_cpid.'\' AND SCHOOL_ID=\''.UserSchool().'\' AND SYEAR=\''.UserSyear().'\''));
                                          $warnings.= ''._('There is gender restriction for   ').''.$course_title[1]['TITLE'].'.';
                                          $clash[] = $warnings;
                                          continue;
                                    }
                                    ...
                                    if($pa_RET[1]['PARENT_ID']!=$select_cpid)
                                    {
                                        ...
                                        {
                                             $course_title=DBGet(DBQuery('SELECT TITLE FROM course_periods WHERE COURSE_PERIOD_ID=\''.$select_cpid.'\' AND SCHOOL_ID=\''.UserSchool().'\' AND SYEAR=\''.UserSyear().'\''));
                                             $warnings.= ''._('You are not scheduled in the parent course of  ').''.$course_title[1]['TITLE'].'.';
                                             $clash[] = $warnings;
                                             continue;
                                        }
		...
		{
                        $course_title=DBGet(DBQuery('SELECT TITLE FROM course_periods WHERE COURSE_PERIOD_ID=\''.$select_cpid.'\' AND SCHOOL_ID=\''.UserSchool().'\' AND SYEAR=\''.UserSyear().'\''));
			$warnings.= ''._('That section is already full for  ').''.$course_title[1]['TITLE'].'.';
                        $clash[] = $warnings;
                        continue;
                }
		...
		{
                        $course_title=DBGet(DBQuery('SELECT TITLE FROM course_periods WHERE COURSE_PERIOD_ID=\''.$select_cpid.'\' AND SCHOOL_ID=\''.UserSchool().'\' AND SYEAR=\''.UserSyear().'\''));
			$warnings .= ''._('There is already a course scheduled in that period for  ').''.$course_title[1]['TITLE'].'.';
                        $clash[] = $warnings;
                        continue;
                }
               //------------------------------------- same course period scheduling conflict -----------------------------------//
                ...
                if($days_conflict2)
                        {
                             $course_title=DBGet(DBQuery('SELECT TITLE FROM course_periods WHERE COURSE_PERIOD_ID=\''.$select_cpid.'\' AND SCHOOL_ID=\''.UserSchool().'\' AND SYEAR=\''.UserSyear().'\''));
                             $warnings .= ''._('student is already scheduled in that period  ').''.$course_title[1]['TITLE'].'.';
                             $clash[] = $warnings;
                            continue;
...
//                                        $st_time = DBGet(DBQuery("SELECT START_TIME, END_TIME FROM SCHOOL_PERIODS WHERE PERIOD_ID = $period_id "));    /********* for homeroom scheduling*/
 ...
 $child_mpid = $mps;
  ...
  $sql = 'SELECT COURSE_PERIOD_ID,START_DATE FROM schedule WHERE STUDENT_ID = \''.UserStudentID().'\' AND (END_DATE IS NULL OR END_DATE>CURRENT_DATE) AND SCHOOL_ID=\''.UserSchool().'\' AND COURSE_PERIOD_ID NOT IN (SELECT COURSE_PERIOD_ID FROM course_periods CP,school_periods SP WHERE CP.PERIOD_ID=SP.PERIOD_ID AND SP.IGNORE_SCHEDULING != \''.' '.'\')  AND MARKING_PERIOD_ID IN('.$child_mpid.'\')';
        ...
        if($clash)
                                    echo "<center><b>"._('There is a conflict. You cannot add this course period')." </b></center>".ErrorMessage($clash,'note')."";
	...
	ListOutput($subjects_RET,$columns,''._('Subject').'',''._('Subjects').'',$link,array(),$LO_options,'for_window');
	echo '</TD>';
	...
	if($_REQUEST['subject_id'] && $_REQUEST['subject_id']!='new')
	{
		...
		ListOutput($courses_RET,$columns,''._('Course').'',''._('Courses').'',$link,array(),$LO_options,'for_window');
		echo '</TD>';
		...
		if($_REQUEST['course_id'] && $_REQUEST['course_id']!='new')
		{
                ...
                $columns = array('CHECKBOX'=>'</A><INPUT type=checkbox value=Y name=controller  onclick="checkAll(this.form,this.form.controller.checked,\'course_period\');"><A>','TITLE'=>''._('Course Period').'');
                ...
                if(count($periods_RET))
                {
                    echo '<BR>'.SubmitButton(''._('Add').'','add_cid','class=btn_medium').'&nbsp; &nbsp;'.SubmitButton(''._('Confirm').'','confirm_cid','class=btn_medium ');
                }
                echo '</TD>';
            echo '</FORM>';
		}
