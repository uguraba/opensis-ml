...
if(clean_param($_REQUEST['modfunc'],PARAM_ALPHA) =='add')
{
    ...
    {
        echo "<font color='red'>"._('Please select a subject')."</font>";
        unset($_REQUEST['modfunc']);
            ...
            {
                echo "<font color='red'>"._('Please select a course')."</font>";
        unset($_REQUEST['modfunc']);
            }
            else{
	...
	DBQuery('INSERT INTO schedule_requests (SYEAR,SCHOOL_ID,STUDENT_ID,SUBJECT_ID,COURSE_ID,MARKING_PERIOD_ID) values(\''.UserSyear().'\',\''.UserSchool().'\',\''.UserStudentID().'\',\''.$subject_id.'\',\''.$course_id.'\',\''.$mp_id.'\')');
            ...
            echo "<font color='red'><b>".''._("You have already requested for this course").''."</b></font>";
	unset($_REQUEST['modfunc']);
            }
        }
}
...
if(!$_REQUEST['modfunc'] && UserStudentID())
{
	...
	$subjects= CreateSelect($subjects_RET, 'subject_id', ''._('Select Subject').'', 'Modules.php?modname='.$_REQUEST['modname'].'&subject_id=');
	
	if($_REQUEST['subject_id'])
	{
	$courses_RET = DBGet(DBQuery('SELECT c.COURSE_ID,c.TITLE FROM courses c WHERE '.($_REQUEST['subject_id']?'c.SUBJECT_ID=\''.$_REQUEST['subject_id'].'\' AND ':'').'UPPER(c.TITLE) LIKE \''.strtoupper($_REQUEST['course_title']).'%'.'\' AND c.SYEAR=\''.UserSyear().'\' AND c.SCHOOL_ID=\''.UserSchool().'\''));        
	$courses = CreateSelect($courses_RET, 'course_id', ''._('Select Course:').'', 'Modules.php?modname='.$_REQUEST['modname'].'&subject_id='.$_REQUEST['subject_id'].'&course_id=');
	...
	DrawHeaderHome(''._('Add a Request').' : &nbsp; '._('Subject').' '.$subjects.' &nbsp; '.$courses,SubmitButton(''._('Add').'','','class=btn_medium onclick=\'formload_ajax("ad");\''));
	...
	//$link['add']['span'] = '<small>Add a Request : </small> &nbsp; Subject '.$subjects.' &nbsp; '.$courses;
        ListOutput($requests_RET,$columns,''._('Request').'',''._('Requests').'',$link);
	if(!$requests_RET)
	echo '';
	else
	echo '<br><CENTER>'.SubmitButton(''._('Update').'','','class=btn_medium onclick=\'formload_ajax("up");\'').'</CENTER>';
	echo '</FORM>';
...
{	global $THIS_RET;
	...
	return ''._('With').': '.SelectInput($value,'values['.$THIS_RET['REQUEST_ID'].'][WITH_TEACHER_ID]','',$options).''._('Without').': '.SelectInput($THIS_RET['NOT_TEACHER_ID'],'values['.$THIS_RET['REQUEST_ID'].'][NOT_TEACHER_ID]','',$options);
}
...
{	global $THIS_RET;
	...
	return ''._('On').': '.SelectInput($value,'values['.$THIS_RET['REQUEST_ID'].'][WITH_PERIOD_ID]','',$options).''._('Not on ').': '.SelectInput($THIS_RET['NOT_PERIOD_ID'],'values['.$THIS_RET['REQUEST_ID'].'][NOT_PERIOD_ID]','',$options);
}
