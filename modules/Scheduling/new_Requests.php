...
if($_REQUEST['modfunc']=='choose')
{
	...
	DrawHeader('',SubmitButton(''._('Save').'','','class=btn_medium onclick=\'formload_ajax("vary");\''));

	$columns = array('');
	ListOutput($requests_RET,$columns,''._('Request').'',''._('Requests').'');

	echo '<CENTER>'.SubmitButton(''._('Save').'','','class=btn_medium onclick=\'formload_ajax("vary");\'').'</CENTER></FORM>';
}
?>
