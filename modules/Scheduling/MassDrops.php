...
if(clean_param($_REQUEST['modfunc'],PARAM_ALPHA)=='save')
{
               ...
               {
                        DrawHeader('<table><tr><td><IMG SRC=assets/x.gif></td><td>'._('The date you entered is not valid').'</td></tr></table>');
                        for_error_sch();
               }
               else
               {
                    ...
                    foreach($_REQUEST['student'] as $student_id=>$yes)
                    {
                                ...
                                if($start_end_RET[1]['SCHEDULER_LOCK']=='Y' || $start_end_RET[1]['START_DATE']>$end_date_mod)
                                    ...
                                    else
                                    {
                                         ...
                                         $note = ""._('Selected students have been dropped from the course period.')."";
                                    }
...
//                        UpdateMissingAttendance($_SESSION['MassDrops.php']['course_period_id']);
                        ...
                        if($inactive_schedule_found==1)
                            DrawHeaderHome('<IMG SRC=assets/warning_button.gif><br>'.$inactive_schedule.''._('have farthar schedule date').'');
                        if($inactive_schedule_found==2)
                             DrawHeaderHome('<IMG SRC=assets/warning_button.gif><br>'._('Dropped date can not be changed for ').''.$inactive_schedule2.'.'._('This schedule is locked.').'');
                   }
                   else 
                  {
                        unset($_REQUEST['modfunc']);
                        unset($_SESSION['MassDrops.php']);
                        DrawHeader('<table><tr><td><IMG SRC=assets/x.gif></td><td>'._('No Studetn selected').'</td></tr></table>');
                  }
                }
...
if(!$_REQUEST['modfunc'])
{
       if($_REQUEST['search_modfunc']=='list')
        ...
        $extra['search'] .= "<TR><TD align='left' width='160' valign='top'>"._('Course Period')."</TD><TD valign='top'><DIV id=course_div></DIV> <A HREF=# onclick='window.open(\"for_window.php?modname=$_REQUEST[modname]&modfunc=choose_course\",\"\",\"scrollbars=yes,resizable=yes,width=800,height=400\");'>"._('Choose')." </A></TD></TR>";

         if($_REQUEST['search_modfunc']=='search_fnc' || !$_REQUEST['search_modfunc'])
        {
            echo '<BR>';
            echo '<script language=JavaScript>parent.help.location.reload();</script>';
            PopTable('header',''._('Find Students to Drop').'');
            ...
            echo $extra['search'];
            echo '</TD></TR><TR><TD width="160">'._('Include advance schedule').'</TD><TD valign="top" align="left"> <INPUT type=checkbox name=include_inactive value=Y /></TD></TR></TABLE>';
            echo '<DIV id=cp_detail></DIV>';
            echo '<TABLE width=100%><TR><TD align=center><BR>';
            //echo Buttons('Submit','Reset');
            echo "<INPUT type=SUBMIT class=btn_medium id=submit value="._('Submit')." onclick='return formcheck_mass_drop();formload_ajax(\"search\");'>&nbsp<INPUT type=RESET class=btn_medium value="._('Reset')." onclick='document.getElementById(\"course_div\").innerHTML =\"\";document.getElementById(\"cp_detail\").innerHTML =\"\";' >";
            echo '</TD></TR>';
            echo '</TABLE>';
            echo '</FORM>';
            PopTable('footer');
...
//		$link = $name_link;
	...
	if(!$extra['columns_before'] && !$extra['columns_after'])
		$columns = $LO_columns;
                ...
                {
                        echo '<TABLE><TR><TD>'._('Drop Date').'</TD><TD>'.PrepareDate(DBDate(),'').'</TD></TR></TABLE>';
                }
