...
if($_REQUEST['modfunc']=='save')
{
	...
	foreach($course_periods_RET as $teacher_id=>$course_period)
	{
		...
		echo "<meta charset='UTF-8'><tr><td width=105>".DrawLogo()."</td><td  style=\"font-size:15px; font-weight:bold; padding-top:20px;\">". GetSchool(UserSchool())."<div style=\"font-size:12px;\">"._('Teacher Class List')."</div></td><td align=right style=\"padding-top:20px;\">". ProperDate(DBDate()) ."<br />"._('Powered by openSIS')."</td></tr><tr><td colspan=3 style=\"border-top:1px solid #333;\">&nbsp;</td></tr></table>";
		echo "<table >";
		echo '<table border=0>';
		echo '<tr><td>'._('Teacher Name').':</td>';
		echo '<td>'.$course_period['TEACHER'].'</td></tr>';
		echo '<tr><td>'._('Course Program Name').':</td>';
		echo '<td>'.$course_period['COURSE_TITLE'].'</td></tr>';
		echo '<tr><td>'._('Course Period Name').':</td>';
...
if(!$_REQUEST['modfunc'])
{
	...
	else
	{
		...
		echo '<TR><TD align=right>'._('Teacher').'</TD><TD>';
		echo "<SELECT name=teacher_id style='max-width:250;'><OPTION value=''>"._('N/A')."</OPTION>";
		foreach($RET as $teacher)
			echo "<meta charset='UTF-8'><OPTION value=$teacher[STAFF_ID]>$teacher[FULL_NAME]</OPTION>";
		echo '</SELECT>';
		echo '</TD></TR>';
		...
		echo '<TR><TD align=right>'._('Subject').'</TD><TD>';
...
//	echo "<FORM name=exp id=exp action=for_export.php?modname=$_REQUEST[modname]&modfunc=save&search_modfunc=list&_openSIS_PDF=true onsubmit=document.forms[0].relation.value=document.getElementById(\"relation\").value; method=POST target=_blank>";
	...
	$LO_columns = array('COURSE_PERIOD_ID'=>'</A><INPUT type=checkbox value=Y name=controller checked onclick="checkAll(this.form,this.form.controller.checked,\'cp_arr\');"><A>','TITLE'=>''._('Course Period').'');

	echo '<INPUT type=hidden name=relation>';
	ListOutput($course_periods_RET,$LO_columns,''._('Course Period').'',''._('Course Periods').'');
//	echo '<BR><CENTER><INPUT type=submit class=btn_xxlarge value=\'Create Class Lists for Selected Course Periods\' onclick=\'formload_ajax("exp");\'></CENTER>';
	...
	echo '<BR><CENTER><INPUT type=submit class=btn_xxlarge value=\''._('Create Class Lists for Selected Course Periods').'\'></CENTER>';
	echo "</FORM>";
}
