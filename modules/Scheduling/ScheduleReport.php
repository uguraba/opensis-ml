...
if($_REQUEST['subject_id'])
{
	...
	$header .= "<A HREF=Modules.php?modname=$_REQUEST[modname]>"._('Top')."</A> >> <A HREF=Modules.php?modname=$_REQUEST[modname]&modfunc=courses&subject_id=$_REQUEST[subject_id]>"._($RET[1]['TITLE']).'</A>';
	if($_REQUEST['course_id'])
	{
		...
		$header .= " >> <A HREF=Modules.php?modname=$_REQUEST[modname]&modfunc=course_periods&subject_id=$_REQUEST[subject_id]&course_id=$_REQUEST[course_id]>"._($RET[1]['TITLE']).'</A>';
...
//			$header .= " >> <A HREF=Modules.php?modname=$_REQUEST[modname]&modfunc=course_periods&subject_id=$_REQUEST[subject_id]&course_id=$_REQUEST[course_id]&course_weight=$_REQUEST[course_weight]>".$_REQUEST['course_weight'].'</A>';
		if($_REQUEST['course_period_id'])
		{
			...
			$header .= " >> <A HREF=Modules.php?modname=$_REQUEST[modname]&modfunc=students&subject_id=$_REQUEST[subject_id]&course_id=$_REQUEST[course_id]&course_period_id=$_REQUEST[course_period_id]>"._($RET[1]['TITLE']).'</A>';
		}
//		}
...
if($_REQUEST['modfunc']=='course_periods' || $_REQUEST['students']=='course_periods')
{
	...
	ListOutput($RET,array('TITLE'=>''._('Period - Teacher').'','OPEN_SEATS'=>''._('Open').'','TOTAL_SEATS'=>''._('Total').''),''._('Course Period').'',''._('Course Periods').'',$link,array(),$LO_options);
	echo '</TD>';
}
...
if($_REQUEST['students'])
{
	...
	$RET = DBGet(DBQuery($sql),array('BIRTHDATE'=>''._('Birthdate').'','GRADE_ID'=>''._('GetGrade').''));

	$link = array();
	$link['FULL_NAME']['link'] = "Modules.php?modname=Scheduling/Schedule.php";
	$link['FULL_NAME']['variables'] = array('student_id'=>'STUDENT_ID');
	ListOutput($RET,array('FULL_NAME'=>''._('Student').'','GRADE_ID'=>''._('Grade').'','BIRTHDATE'=>''._('Birthdate').''),''._('Student').'',''._('Students').'',$link,array(),$LO_options);
	echo '</TD>';
}
