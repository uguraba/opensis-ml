...
{   
	if($_SESSION['MassSchedule.php'])
	{
		...
		{
		ShowErr(""._('You cannot schedule a student into that course during the marking period that you chose.  This course meets on')." ".GetMP($course_mp).'.');
		#for_error();
		for_error_sch();
		}
...
// ----------------------------------------- Time Clash Logic Start ---------------------------------------------------------- //
		...
		function con_date($date)
		{
			...
			$temp_month = substr($mother_date, 3, 3);
                ...
                {
                 $start_date_q_clash=''._('Cannot schedule students before school start date').'';   
                }
                else
                {
...
{ #echo "<script>alert('a')</script>";
	...
	if($_REQUEST['search_modfunc']=='list')
	{
		...
		echo '</DIV>'."<A HREF=# onclick='window.open(\"for_window.php?modname=$_REQUEST[modname]&modfunc=choose_course\",\"\",\"scrollbars=yes,resizable=yes,width=800,height=400\");'>"._('Choose a Course')."</A></TD></TR>";
	...
	if($note)
        ...
        if($gender_conflict)
                DrawHeaderHome('<IMG SRC=assets/warning_button.gif><br>'.$gender_conflict.''._('have Gender Clash').'');
        if($parent_res)
                DrawHeaderHome('<IMG SRC=assets/warning_button.gif><br>'.$parent_res.''._('have Parent course restriction').'');
        ...
        if($stu_sd_err)
                ...
                {$put=''._('his/her').'';}
                 DrawHeaderHome('<IMG SRC=assets/warning_button.gif><br>'.$stu_sd_err.''._('cannot be scheduled before ').''.$put.''._(' enrolled date').'');
                 unset($stu_sd_err);
                 unset($stu_s_date_err_n);
                }
	if($period_res)
                DrawHeaderHome('<IMG SRC=assets/warning_button.gif><br>'.$period_res.''._('have already scheduled in that period').'');
        if($time_clash)
		DrawHeaderHome('<IMG SRC=assets/warning_button.gif><br>'.$time_clash.''._('have a period time clash').'');
...
//		DrawHeaderHome('<IMG SRC=assets/warning_button.gif>'.$clash." is already in the schedule");
		...
		DrawHeaderHome('<IMG SRC=assets/warning_button.gif>'.$request_clash.''._('already have unscheduled requests').'');
...
if(!$_REQUEST['modfunc'])
{
	...
	if($_REQUEST['search_modfunc']=='list')
	{
		...
		echo '<BR><CENTER>'.SubmitButton('','','class=btn_group_schedule').'</CENTER>';
...
if($_REQUEST['modfunc']=='choose_course')
{
	...
	else
	{
		...
		echo "<script language=javascript>opener.document.getElementById(\"course_div\").innerHTML = \"$course_title<BR>$period_title\";opener.document.getElementById(\"marking_period\").value=\"$mperiod\"; window.close();</script>";
                                        ...
                                        echo "<script language=javascript>opener.document.getElementById(\"course_div\").innerHTML = \"$course_title <BR>$period_title <br>"._('Gender').": ".($gender_res=='M'?''._('Male').'':''._('Female').'')." \";opener.document.getElementById(\"marking_period\").value=\"$mperiod\"; window.close();</script>";
...
if($_REQUEST['modfunc']=='seats')
	...
	echo '<form name=update_seats id=update_seats method=POST action=for_window.php?modname='.$_REQUEST['modname'].'&modfunc=seats&course_period_id='.$_REQUEST['course_period_id'].'&update=true>';
 	echo '<TABLE><TR>';
	echo '<td colspan=2 align=center><b>'._('Click on the number of seats to edit and click update').'</b></td></tr><tr><td colspan=2 align=center><table><tr><td>Total Seats</td><td>:</td><td>' . TextInput($RET['TOTAL_SEATS'],'tables[course_periods]['.$_REQUEST['course_period_id'].'][TOTAL_SEATS]','','size=10 class=cell_floating maxlength=5').'</td></tr></table>';
    echo '</TR><tr><td colspan=2 align=center><input type=submit value='._('Update').' class=btn_medium></td></tr></TABLE>';
	echo '</form>';
	echo '</div>';
	}
