...
{
	if(DeletePromptCommon(_('student from that user'),_('remove access to')))
	{
...
{	
	...
	if(UserStaffID())
	{
		...
		DrawHeader('<div class="big_font">'._('Associated')._(' ')._('Students')._(' ')._('With').' '.$RET[1]['FIRST_NAME'].'&nbsp;'.$RET[1]['LAST_NAME'].'</div>',$extra['header_right']);
	...
	//	$link['TITLE']['link'] = "#"." onclick='check_content(\"ajax.php?modname=$_REQUEST[modname]&modfunc=$_REQUEST[modfunc]&mp_term=FY\");'";		
		ListOutput($current_RET,array('FULL_NAME'=>_('Students')),'','',$link,array(),array('search'=>false));
		echo '</TD></TR></TABLE><div class="clear"></div><div style="width:830px;">';
		
			if(clean_param($_REQUEST['search_modfunc'],PARAM_ALPHAMOD)=='list')
			echo '<div style="margin-bottom:-28px;">'.DrawHeader('',SubmitButton(''._('Add Selected Students').'','','class=btn_large')).'</div>';
		...
		if(clean_param($_REQUEST['search_modfunc'],PARAM_ALPHAMOD)=='list')
			echo "<BR><CENTER>".SubmitButton(''._('Add Selected Students').'','','class=btn_large')."</CENTER></FORM>";
	}
}
