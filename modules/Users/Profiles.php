...
PopTable('header',''._('Permissions').'');
	...
	DrawHeaderHome(''._('Select the programs that users of this profile can use and which programs those users can use to save information.').'');
	echo '<BR>';
	...
	echo '<TR><TD colspan=3 style="border:1px; border-style: none none solid none;"><b>'._('Profiles').'</b></TD></TR>';
	...
	foreach($profiles_RET[$profiles] as $id=>$profile)
	{
		...
		echo '<b><a style="cursor: pointer; cursor:hand; text-decoration:none;">'.($id>3?'':'<b>')._($profile[1]['TITLE']).($id>3?'':'</b>').'</a></b> &nbsp;';
		echo '</TD>';
	...
	if($_REQUEST['profile_id']!='')
	{
		...
		echo '<TR><TD colspan=5 style="border:1px; border-style: none none solid none;"><b>'._('Permissions').'</b></TD></TR>';
		foreach($menuprof as $modcat=>$profiles)
		{
			...
			echo "<b>"._(str_replace('_',' ',$modcat))."</b></TD><TD width=3 class=grid>&nbsp;</TD>";
			echo "<td class=grid style='white-space: nowrap;  padding:2px 2px 2px 6px;'>"._('Can Use')."".(AllowEdit()?"<INPUT type=checkbox name=can_use_$modcat onclick='checkAll(this.form,this.form.can_use_$modcat.checked,\"can_use[$modcat\");'>":'')."</td>";
			if($xprofile=='admin' || $modcat=='Students')
				echo"<td class=grid style='white-space: nowrap; padding:2px 2px 2px 6px;' > &nbsp;"._('Can Edit')."".(AllowEdit()?"<INPUT type=checkbox name=can_edit_$modcat onclick='checkAll(this.form,this.form.can_edit_$modcat.checked,\"can_edit[$modcat\");'>":'')."</td>";
			else
	...
	echo '</FORM>';
	PopTable('footer');
	echo '<DIV id=new_id_content style="position:absolute;visibility:hidden;"><fieldset><legend>'._('Add a User')._('Profile').'</legend><table><tr><td width=30>'._('Title').' </td><td><INPUT type=text name=new_profile_title></td></tr>';
	echo '<tr><td width=30>'._('Type').'</td><td><SELECT name=new_profile_type><OPTION value=admin>'._('Administrator').'<OPTION value=teacher>'._('Teacher').'<OPTION value=parent>'._('Parent').'</SELECT>
	<br></td></tr><tr><td colspan=2 align=center><input type=submit value='._('Save').' class=btn_medium></td></tr></table></fieldset></DIV>';
}
?>