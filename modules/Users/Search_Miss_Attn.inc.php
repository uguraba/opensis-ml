...
if(User('PROFILE')=='admin')
{
		...
		echo '<div><STRONG><FONT color=red>'. $ERR. '</font></strong></DIV>';
                ...
                $header .= '<TD style="width:30px; vertical-align:top; text-align:left; padding-top:8px;">'._('From').'</TD><td valign="top" style="width:230px;">'.PrepareDate($From,'_From',true,array('short'=>true)).'</TD>';
                $header .='<TD style="width:20px; vertical-align:top; text-align:left; padding-top:8px;">'._('To').'</TD><td valign="top" style="width:230px;">'.PrepareDate($to,'_to',true,array('short'=>true)).'</TD>';
                $header .= '<td align=left><INPUT type=submit class=btn_medium name=go value='._('Go').' ></td>';
                $header .= '</tr></table>';
		...
		if(count($staff_RET))
		{
            echo '<font color=red><b>'._('Following teachers have missing attendance').'!.</b></font>';
            ListOutput($staff_RET,$columns,$singular,$plural,$link,false,$extra['options']);
        }
		else
		{
			echo '<font color=red><b>'._('No teachers have missing attendance for this school')._('Teacher was found').'!</b></font>';
		}
 }
