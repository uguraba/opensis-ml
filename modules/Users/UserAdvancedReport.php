...
{ 
	if(count($_SESSION['st_arr']))
	{
		...
		$extra['WHERE'] = ' AND s.STAFF_ID IN ('.$st_list.')';
			...
			echo "<tr><td width=105>".DrawLogo()."</td><td style=\"font-size:15px; font-weight:bold; padding-top:20px;\">". GetSchool(UserSchool())."<div style=\"font-size:12px;\">"._('User Advanced Report')."</div></td><td align=right style=\"padding-top:20px;\">". ProperDate(DBDate()) ."<br />"._('Powered by openSIS')."</td></tr><tr><td colspan=3 style=\"border-top:1px solid #333;\">&nbsp;</td></tr></table>";
			echo "<table >";
...
$_SESSION['st_arr']=$_REQUEST['st_arr'];
	...
	echo "<meta charset='UTF-8'><FORM action=for_export.php?modname=$_REQUEST[modname]&head_html=Staff+Advanced+Report&modfunc=save&search_modfunc=list&_openSIS_PDF=true&_dis_user=$_REQUEST[_dis_user]&_search_all_schools=$_REQUEST[_search_all_schools] method=POST target=_blank>";
	echo '<DIV id=fields_div></DIV>';
	echo '<br/>';
		include('modules/misc/UserExport.php');
	echo '<BR><CENTER><INPUT type=submit value='._('Create Report for Selected Users').' class=btn_xxlarge></CENTER>';
	echo "</FORM>";
}
...
if(!$_REQUEST['modfunc'] || $_REQUEST['modfunc']=='list')
{
	...
	{
                $extra['columns_after'] = array('LAST_LOGIN'=>_('Last Login'));
        ...
        echo "<meta charset='UTF-8'><FORM action=Modules.php?modname=$_REQUEST[modname]&modfunc=call method=POST>";
	echo '<DIV id=fields_div></DIV>';
