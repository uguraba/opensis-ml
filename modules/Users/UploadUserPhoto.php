...
//include("../Students/upload.class.php");

DrawBC(_('Users')." >> ".ProgramTitle());
PopTable ('header',''._("Upload Staff's Photo")).'';
...
if(clean_param($_REQUEST['action'],PARAM_ALPHAMOD)=='upload' && $_FILES['file']['name'])
{
    ...
    echo '<div align=center>'._('Select image file').': <input name="file" type="file" /><br /><br>
    <input type="submit" value='._("Upload").' class=btn_medium />&nbsp;<input type=button class=btn_medium value='._('Cancel').' onclick=\'load_link("Modules.php?modname=Users/User.php");\'></div>
    </form>';
    PopTable ('footer');
        }
  	else
    {
      ...
      echo "<b>"._('Copied file to')." " .$upload->destination_path."</b><p>";
...
{
echo ''._('Cannot upload parent\'s picture.').'';
PopTable ('footer');  
}
}
else
{
	echo ''._("Please select a user first! from the").' <b>'._("Users").'</b> '._('Tab').'';
	PopTable ('footer');
}
