...
DrawBC(_('Eligibility')." > ".ProgramTitle());
...
if(UserStudentID() && !$_REQUEST['modfunc'])
{
	...
	DrawHeaderHome('<SELECT name=start_date>'.$date_select.'</SELECT>','<INPUT type=submit value='._('Go').' class=btn_medium onclick=\'formload_ajax("elig_stud");\' >');
	echo '</FORM>';
...
//	$link['remove']['link'] = "#"." onclick='check_content(\"ajax.php?modname=$_REQUEST[modname]&modfunc=remove&start_date=$_REQUEST[start_date]\");'";
	...
	$link['add']['html']['TITLE'] = '<TABLE border=0 cellpadding=0 cellspacing=0><TR><TD>'.SelectInput('','new_activity','',$activities).' </TD><TD>&nbsp;<INPUT type=submit value='._('Add').' class=btn_medium onclick=\'formload_ajax("elig_stud");\'></TD></TR></TABLE>';
	...
	ListOutput($RET,$columns,_('Activity'),_('Activities'),$link);
	echo '</FORM>';
	...
	ListOutput($RET,$columns,_('Course'),_('Courses'));
