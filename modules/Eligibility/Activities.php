	...
	{	
		if($id!='new')
		{
			...
			DBQuery($sql);
                        ...
                        echo "<font color=red>"._('End date should be greater than begin date..')."</font>";
                    }
		}
		else
		{
...
{	
	...
	if($has_assigned>0){
	UnableDeletePrompt(_('Cannot delete because eligibility activities are associated.'));
	}else{
...
if($_REQUEST['modfunc']!='remove')
{
	...
	ListOutput($activities_RET,$columns,''._('Activity').'',''._('Activities').'',$link);
	echo '<br /><CENTER>'.SubmitButton(_('Save'),'','class=btn_medium onclick="formcheck_eligibility_activies();"').'</CENTER>';
	echo '</FORM>';
}
