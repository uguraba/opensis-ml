 ...
 if($_REQUEST['langname'])
                    {
                        ...
                        session_start();
                        $_SESSION['lang_var'] = $a;
                        $_SESSION['remember'] =$_REQUEST['remember_me'];
...
{
$error[] = ''._("Either your account is inactive or your access permission has been revoked. Please contact the school administration.").'';
}
if(optional_param('dis','',PARAM_ALPHAEXT)=='assoc_mis')
{
    $error[] = ''._("No student is associated with the parent. Please contact the school administration.").'';
}
if(isset($_GET['ins']))
...
{
    if($_REQUEST['remember_me']=='N')
    {
     $time=time()-3600;   
     setcookie('remember_me_username','',$time);
     setcookie('remember_me_password','',$time);
    }
    db_start();
...
$maintain_RET=DBGet(DBQuery("SELECT SYSTEM_MAINTENANCE_SWITCH FROM system_preference_misc LIMIT 1"));
	 $maintain=$maintain_RET[1];	
        $get_ac_st=0;
        $get_tot_st=0;
         $login_Check=DBGet(DBQuery("SELECT USERNAME,STAFF_ID FROM staff WHERE UPPER(USERNAME)=UPPER('$username') AND UPPER(PASSWORD)=UPPER('$password')"));
         ...
         if($login_Check[1]['USERNAME']!='' && $get_ac_st<$get_tot_st)
         {
         ...
         else
         {
             if($get_ac_st==$get_tot_st && $get_tot_st!=0)
              $error_dis = "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"._('Your opensis account is inactive.')."";    
              else   
	...
	elseif(($login_RET && $login_RET[1]['IS_DISABLE']=='Y') || ($student_RET && $student_RET[1]['IS_DISABLE']=='Y'))
	{
            ...
            if(isset($login_RET) && count($login_RET)>0)
            {
                if($ad_f_cnt && $ad_f_cnt!=0 && $login_RET[1]['FAILED_LOGIN']<$ad_f_cnt && $login_RET[1]['PROFILE']!='admin')
                  ...
                  $error[] = ''._("Due to excessive incorrect login attempts your account has been disabled. Contact the school administration to enable your account.").'';  
            }
...
//                $get_rec=DBGet(DBQuery("SELECT COUNT(1) as RECORD FROM students s,student_enrollment se WHERE UPPER(s.USERNAME)=UPPER('".optional_param('USERNAME', 0, PARAM_RAW)."') AND s.STUDENT_ID=se.STUDENT_ID AND (se.DROP_CODE='4' OR se.DROP_CODE IS NULL) AND (se.END_DATE<='".date('Y-m-d')."' OR se.END_DATE IS NULL) AND se.SYEAR=(SELECT MAX(SYEAR) FROM student_enrollment WHERE STUDENT_ID=s.STUDENT_ID ) "));
                $get_rec=DBGet(DBQuery("SELECT COUNT(1) as RECORD FROM students s,student_enrollment se WHERE UPPER(s.USERNAME)=UPPER('".optional_param('USERNAME', 0, PARAM_RAW)."') AND s.STUDENT_ID=se.STUDENT_ID AND (se.DROP_CODE='4' OR (se.DROP_CODE IS NULL AND se.END_DATE<='".date('Y-m-d')."')) AND (se.END_DATE<='".date('Y-m-d')."' OR se.END_DATE IS NULL) AND se.SYEAR=(SELECT MAX(SYEAR) FROM student_enrollment WHERE STUDENT_ID=s.STUDENT_ID ) "));
...
elseif($_REQUEST['modfunc']!='create_account')
{
    ...
    if(($_REQUEST['remember_me']=='Y') || ($_REQUEST['remember_me_checkbox'] && $_REQUEST['remember_me']==''))
    {
     setcookie('remember_me_username',$_REQUEST['USERNAME'],time()+3600*24*4);
     setcookie('remember_me_password',$_REQUEST['PASSWORD'],time()+3600*24*4);
     setcookie('remember_me_langname',$_REQUEST['langname'],time()+3600*24*4);
    }
    if($_REQUEST['remember_me']=='N')
    {
     $time=time()-3600;   
     setcookie('remember_me_username','',$time);
     setcookie('remember_me_password','',$time);
     setcookie('remember_me_langname','',$time);
    }
    echo "
        <HTML>
