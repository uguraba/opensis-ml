...
{	global $DatabaseServer,$DatabaseUsername,$DatabasePassword,$DatabaseName,$DatabasePort,$DatabaseType;
	...
	if($connection === false)
	{
		...
		db_show_error("",""._("Could not Connect to Database").": $DatabaseServer",$errstring);
	}
	return $connection;
}
...
{	global $openSISTitle,$openSISVersion,$openSISNotifyAddress;
	...
	/*echo "
		...
		<TABLE CELLSPACING=10 BORDER=0>
			...
			<TD align=right></TD>
			<TD>"._('openSIS has encountered an error that could have resulted from any of the following').":
			<br/>
			<ul>
			<li>"._('Invalid data input')."</li>
			<li>"._('Database SQL error')."</li>
			<li>"._('Program error')."</li>
			</ul>
			
			"._('Please take this screen shot and send it to your openSIS representative for debugging and resolution.')."
			</TD>
		</TR>
	...
	{
		$message = ""._('System').": $openSISTitle \n";
		$message .= ""._('Date').": ".date("m/d/Y h:i:s")."\n";
		$message .= ""._('Page').": ".$_SERVER['PHP_SELF'].' '.ProgramTitle()." \n\n";
		$message .= ""._('Failure Notice').":  $failnote \n";
		$message .= ""._('Additional Info').": $additional \n";
		$message .= "\n $sql \n";
		$message .= ""._('Request Array').": \n".ShowVar($_REQUEST,'Y', 'N');
		$message .= "'\n\n'"._('Session Array').": \n".ShowVar($_SESSION,'Y', 'N');
		mail($openSISNotifyAddress,_('openSIS Database Error'),$message);
