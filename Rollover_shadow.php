...
//exit;
$tables = array('staff'=>''._('Users').'','school_periods'=>''._('School Periods').'','school_years'=>''._('Marking Periods').'','attendance_calendars'=>''._('Calendars').'','report_card_grade_scales'=>''._('Report Card Grade Codes').'','course_subjects'=>''._('Subjects').'','courses'=>''._('Courses').'','course_periods'=>''._('Course Periods').'','student_enrollment'=>''._('Students').'','honor_roll'=>''._('Honor Roll Setup').'','attendance_codes'=>''._('Attendance Codes').'','student_enrollment_codes'=>''._('Student Enrollment Codes').'','report_card_comments'=>''._('Report Card Comment Codes').'','NONE'=>''._('none').'');
...
//			DBQuery('INSERT INTO attendance_calendars (SYEAR,SCHOOL_ID,TITLE,DEFAULT_CALENDAR,ROLLOVER_ID) SELECT SYEAR+1,SCHOOL_ID,TITLE,DEFAULT_CALENDAR,CALENDAR_ID FROM attendance_calendars WHERE SYEAR=\''.UserSyear().'\' AND SCHOOL_ID=\''.UserSchool().'\'');
                        DBQuery("INSERT INTO attendance_calendars (SYEAR,SCHOOL_ID,TITLE,DEFAULT_CALENDAR,ROLLOVER_ID) SELECT SYEAR+1,SCHOOL_ID,CONCAT(TITLE,'_',SYEAR+1),DEFAULT_CALENDAR,CALENDAR_ID FROM attendance_calendars WHERE SYEAR='".UserSyear()."' AND SCHOOL_ID='".UserSchool()."'");
                        //------------------newly added-------------------
			...
			if($_SESSION['custom_date']=='Y')
                        {
                            ...
                            DBQuery('INSERT INTO school_years (MARKING_PERIOD_ID,SYEAR,SCHOOL_ID,TITLE,SHORT_NAME,SORT_ORDER,START_DATE,END_DATE,ROLLOVER_ID) VALUES (\''.$next_mp_id[1]['SEQ'].'\',\''.(UserSyear()+1).'\',\''.UserSchool().'\',\''. $get_sch_yr[1]['TITLE'].'\',\''. $get_sch_yr[1]['SHORT_NAME'].'\',\''. $get_sch_yr[1]['SORT_ORDER'].'\',\''.$next_s_start_date.'\',\''.$next_s_end_date.'\',\''.$get_sch_yr[1]['MARKING_PERIOD_ID'].'\')');
                            ...
                            foreach($get_sem as $ind=>$data)
                            {
                                ...
                                $next_mp_id=DBGet(DBQuery('SELECT '.db_seq_nextval('marking_period_seq').' as SEQ'));
                                DBQuery('INSERT INTO school_semesters (MARKING_PERIOD_ID,YEAR_ID,SYEAR,SCHOOL_ID,TITLE,SHORT_NAME,SORT_ORDER,START_DATE,END_DATE) VALUES (\''.$next_mp_id[1]['SEQ'].'\',\''.$y_id[1]['MARKING_PERIOD_ID'].'\',\''.(UserSyear()+1).'\',\''.UserSchool().'\',\''.$data['TITLE'].'\',\''.$data['SHORT_NAME'].'\',\''.$data['SORT_ORDER'].'\',\''.$_SESSION['sem_start'][$ind].'\',\''.$_SESSION['sem_end'][$ind].'\')');
                            }
                                if($_SESSION['total_qrt']!='' && $_SESSION['total_qrt']!=0)
                                {
                                    ...
                                    foreach($get_sem as $ind=>$data)
                                    {
                                        ...
                                        foreach($get_qrtr as $ind_q=>$data_q)
                                        {
                                            ...
                                            $next_mp_id=DBGet(DBQuery('SELECT '.db_seq_nextval('marking_period_seq').' as SEQ'));
                                            DBQuery('INSERT INTO school_quarters (MARKING_PERIOD_ID,SEMESTER_ID,SYEAR,SCHOOL_ID,TITLE,SHORT_NAME,SORT_ORDER,START_DATE,END_DATE) VALUES (\''.$next_mp_id[1]['SEQ'].'\',\''.$s_id[$ind]['MARKING_PERIOD_ID'].'\',\''.(UserSyear()+1).'\',\''.UserSchool().'\',\''.$data_q['TITLE'].'\',\''.$data_q['SHORT_NAME'].'\',\''.$data_q['SORT_ORDER'].'\',\''.$_SESSION['qrtr_start'][$qrtr].'\',\''.$_SESSION['qrtr_end'][$qrtr].'\')');
                                        }
                                    }
                                }
		...
		case 'student_enrollment_codes':
			...
			DBQuery('INSERT INTO '.$table.' (SYEAR'.$columns.') SELECT SYEAR+1'.$columns.' FROM '.$table.' WHERE SYEAR=\''.UserSyear().'\'');
                    ...
                    case 'NONE' :
                        ...
                        echo '<div style="padding-top:90px; text-align:center;"><span style="font-size:14px; font-weight:bold;">'._('The school year has been rolled.').'</span><br/><br/><input type=button onclick=document.location.href="index.php?modfunc=logout" value='._("Please login again").' class=btn_large ></div>';
...
//$c_m=substr($c_dt,5,2);
//$c_d=substr($c_dt,-2);
//$tomorrow  = mktime(0, 0, 0, date("m",strtotime($c_m))  , date("d",strtotime($c_d))+1, date("Y",strtotime($c_y)));
$c_dt_sec=strtotime($c_dt);
$c_dt=date('Y-m-d',($c_dt_sec+86400));
$c_dt_arr[]=$c_dt;
}
