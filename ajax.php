...
if(UserStudentID() && User('PROFILE')!='parent' && User('PROFILE')!='student' && substr(clean_param($_REQUEST['modname'],PARAM_NOTAGS),0,5)!='Atten' && substr(clean_param($_REQUEST['modname'],PARAM_NOTAGS),0,5)!='Users' && clean_param($_REQUEST['modname'],PARAM_NOTAGS)!='Students/AddUsers.php')// && substr(clean_param($_REQUEST['modname'],PARAM_NOTAGS),0,10)!='Attendance' )
{
	...
	DrawHeaderHome(''._( 'Selected Student').': '.$RET[1]['FIRST_NAME'].'&nbsp;'.($RET[1]['MIDDLE_NAME']?$RET[1]['MIDDLE_NAME'].' ':'').$RET[1]['LAST_NAME'].'&nbsp;'.$RET[1]['NAME_SUFFIX'].' (<A HREF=Side.php?student_id=new&modcat='.clean_param($_REQUEST['modcat'],PARAM_NOTAGS).'><font color=red>'._('Deselect').'</font></A>) | <A HREF=Modules.php?modname='.clean_param($_REQUEST['modname'],PARAM_NOTAGS).'&search_modfunc=list&next_modname=Students/Student.php&ajax=true&bottom_back=true&return_session=true target=body>'._('Back to Student List').'</A>');
	  /*  if($_REQUEST['modname']!='Attendance/Administration.php')
	{
		DrawHeaderHome( 'Selected Student: '.$RET[1]['FIRST_NAME'].'&nbsp;'.($RET[1]['MIDDLE_NAME']?$RET[1]['MIDDLE_NAME'].' ':'').$RET[1]['LAST_NAME'].'&nbsp;'.$RET[1]['NAME_SUFFIX'].' (<A HREF=Side.php?student_id=new&modcat='.$_REQUEST['modcat'].'><font color=red>Remove</font></A>) | <A HREF=Modules.php?modnamDesee='.$_REQUEST['modname'].'&search_modfunc=list&next_modname='.$_REQUEST['modname'].'&ajax=true&bottom_back=true&return_session=true target=body>Back to Student List</A>');
	}*/
...
if(UserStaffID() && User('PROFILE')=='admin' && substr(clean_param($_REQUEST['modname'],PARAM_NOTAGS),0,6)!='Grades' && substr(clean_param($_REQUEST['modname'],PARAM_NOTAGS),0,8)!='Students' && substr(clean_param($_REQUEST['modname'],PARAM_NOTAGS),0,10)!='Attendance')
{
    ...
    $Modname_Pro = 'Users/TeacherPrograms.php?include=Grades/ProgressReports.php';
        if((!UserStudentID() || substr(clean_param($_REQUEST['modname'],PARAM_NOTAGS),0,5)=='Users') && substr(clean_param($_REQUEST['modname'],PARAM_NOTAGS),0,25)!=$Modname_Attn && clean_param($_REQUEST['modname'],PARAM_NOTAGS)!='Users/AddStudents.php' && !clean_param($_REQUEST['miss_attn'],PARAM_NOTAGS))
        {
            ...
            DrawHeaderHome(''._( 'Selected User').': '.$RET[1]['FIRST_NAME'].'&nbsp;'.$RET[1]['LAST_NAME'].' (<A HREF=Side.php?staff_id=new&modcat='.clean_param($_REQUEST['modcat'],PARAM_NOTAGS).'><font color=red>'._(Deselect).'</font></A>) | <A HREF=Modules.php?modname='.clean_param($_REQUEST['modname'],PARAM_NOTAGS).'&search_modfunc=list&next_modname=Users/User.php&ajax=true&bottom_back=true&return_session=true target=body>'._('Back to User List').'</A>');
        }
}
