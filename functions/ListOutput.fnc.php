...
function ListOutputMod($result,$column_names,$singular='',$plural='',$link=false,$group=false,$options=false)
{
	...
	if($_LIST['output']!=false)
	{
		...
		{
			if(($result_count==0 || $display_count==0) && $plural)
				echo "<div style=text-align:left><table cellpadding=5 cellspacing=5 class=alert_box ><tr><td class=alert></td><td class=alert_msg ><b>"._('No')." $plural "._('were found.')."</b></td></tr></table></div>";
			elseif($result_count==0 || $display_count==0)
				echo '<div style=text-align:left><table cellpadding=5 cellspacing=5 class=alert_box ><tr><td class=alert></td><td class=alert_msg ><b>'._('None were found.').'</b></td></tr></table></div>';
		}
		if($result_count != 0 || ($_REQUEST['LO_search'] && $_REQUEST['LO_search']!='Search'))
		{
			if(!isset($_REQUEST['_openSIS_PDF']))
...
function ListOutputPrint_Report($result,$column_names,$singular='',$plural='',$link=false,$group=false,$options=false)
{
	...
	if($_LIST['output']!=false)
	{
		...
		{
			if(($result_count==0 || $display_count==0) && $plural)
				echo "<div style=text-align:left><table cellpadding=1 cellspacing=0 ><tr><td ></td><td ><b>"._('No')." $plural "._('were found.')."</b></td></tr><tr><td colspan=2 ></td></tr></table></div>";
			elseif($result_count==0 || $display_count==0)
				echo '<div style=text-align:left><table cellpadding=1 cellspacing=0 ><tr><td ></td><td ><b>'._('None were found.').'</b></td></tr><tr><td colspan=2></td></tr></table></div>';
		}
		if($result_count != 0 || ($_REQUEST['LO_search'] && $_REQUEST['LO_search']!='Search'))
		{
			if(!isset($_REQUEST['_openSIS_PDF']))
			{
				...
				if($result_count > $num_displayed)
				{
					$where_message = "<SMALL>"._('Displaying')." $start "._('through')." $stop</SMALL>";
					echo ""._('Go to Page')." ";
					if(ceil($result_count/$num_displayed) <= 10)
					{
						for($i=1;$i<=ceil($result_count/$num_displayed);$i++)
				...
				if(isset($_REQUEST['_openSIS_PDF']) && $count%$repeat_headers==0)
				{
					if($count!=0)
					{
						...
						echo "<table width=100%  style=\" font-family:Arial; font-size:12px;\" >";
                                                                                                            echo "<tr><td width=105>".DrawLogo()."</td><td style=\"font-size:15px; font-weight:bold; padding-top:20px;\">". GetSchool(UserSchool())."<div style=\"font-size:12px;\">".$_SESSION['_REQUEST_vars'][0]."</div></td><td align=right style=\"padding-top:20px;\">". ProperDate(DBDate()) ."<br />"._('Powered by openSIS')."</td></tr><tr><td colspan=3 style=\"border-top:1px solid #333;\">&nbsp;</td></tr></table>";
					...
					if($count!=0)
					{
						...
						echo "<table width=100%  style=\" font-family:Arial; font-size:12px;\" >";
                                                if($_REQUEST['modname']=='Grades/AdminProgressReports.php' || $_REQUEST['modname']=='Grades/ProgressReports.php' || $_REQUEST['modname']=='Users/TeacherPrograms.php?include=Grades/ProgressReports.php')
                                                        ...
                                                        echo "<tr><td width=105>".DrawLogo()."</td><td style=\"font-size:15px; font-weight:bold; padding-top:20px;\">". GetSchool(UserSchool())."<div style=\"font-size:12px;\">"._('Add / Drop Report')."</div></td><td align=right style=\"padding-top:20px;\">". ProperDate(DBDate()) ."<br />"._('Powered by openSIS')."</td></tr><tr><td colspan=3 style=\"border-top:1px solid #333;\">&nbsp;</td></tr></table>";
						echo '<TABLE cellpadding=6 width=100% cellspacing=1 border="1px solid #a9d5e9 " style="border-collapse:collapse" align=center>';
	...
	if($_LIST['output']!=false)
	{
		...
		if($result_count != 0 || ($_REQUEST['LO_search'] && $_REQUEST['LO_search']!='Search'))
		{
			if(!isset($_REQUEST['_openSIS_PDF']))
			{
				...
				{
                                       echo "<br>";
									   echo "<div style=text-align:center><strtong>"._('Displaying')." $start "._('through')." $stop</strong></div>";
					echo "<div style=text-align:right;padding-right:40px;margin-top:-15px;><strong>"._('Go to Page')." ";
	...
	if($_LIST['output']!=false)
	{
		...
		if($result_count != 0 || ($_REQUEST['LO_search'] && $_REQUEST['LO_search']!='Search'))
		{
			if(!isset($_REQUEST['_openSIS_PDF']))
			{
				...
				{
					echo '<br>';
                                        echo $where_message = "<strtong>"._('Displaying')." $start "._('through')." $stop</strong>";
					echo "<div style=text-align:right;margin-top:-15px;padding-right:29px;><strong>"._('Go to Page')." ";
					if(ceil($result_count/$num_displayed) <= 10)
					{
						for($i=1;$i<=ceil($result_count/$num_displayed);$i++)
						{
