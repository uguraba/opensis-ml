						...
						case 'text':
							...
							else
							{
								$string .= ' and LOWER(s.'.$field_name.') LIKE \''.strtolower('.$value.').'%\' ';
                                                                if($exp==1)
								$_openSIS['Search'] .= '<font color=gray><b>'.$fields[$id][1]['TITLE'].' '._('starts with').': </b></font>'.$value.'<BR>';
                                                            elseif($exp==2){
                                                                $_openSIS['SearchTerms'] .= '<font color=gray><b>'.$fields[$id][1]['TITLE'].' '._('starts with').': </b></font>'.$value.'<BR>';
                                                            }
                                                             else {
                                                                          $_openSIS['SearchTerms'] .= '<font color=gray><b>'.$fields[$id][1]['TITLE'].' '._('starts with').': </b></font>'.$value.'<BR>';
                                                            }
			...
			foreach($_REQUEST['cust_begin'] as $id => $value)
			{
				...
				if($_REQUEST['cust_begin'][$field_name]!='' && $_REQUEST['cust_end'][$field_name]!='')
				{
					...
					if($fields[$id][1]['TYPE']=='date')
						$_openSIS['SearchTerms'] .= '<font color=gray><b>'.$fields[$id][1]['TITLE'].' '._('between').': </b></font>'.ProperDate($value).' &amp; '.ProperDate($_REQUEST['cust_end'][$field_name]).'<BR>';
					else
						$_openSIS['SearchTerms'] .= '<font color=gray><b>'.$fields[$id][1]['TITLE'].' '._('between').': </b></font>'.$value.' &amp; '.$_REQUEST['cust_end'][$field_name].'<BR>';
				}
			}
		}
