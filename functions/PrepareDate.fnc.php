...
{	global $_openSIS;
	...
	if($options['M'])
	{
		$return .= "<div style=white-space:nowrap ><SELECT style=width:90px; NAME=month".$title." id=monthSelect".$_openSIS['PrepareDate']." SIZE=1 $extraM>";
		//  -------------------------------------------------------------------------- //
	...
	if($options['D'])
	{
		$return .="<SELECT NAME=day".$title." id=daySelect".$_openSIS['PrepareDate']." SIZE=1 $extraD>";
		if($allow_na)
		{
	...
	if($options['Y'])
	{
		...
		$return .="<SELECT NAME=year".$title." id=yearSelect".$_openSIS['PrepareDate']." SIZE=1 $extraY>";
		if($allow_na)
		{
	...
	if($options['C']){
	if($cal_img!='N'){
		$return .= '<span style="padding-top:4px"><img src="assets/calendar.gif" id="trigger'.$_openSIS['PrepareDate'].'" style="cursor: pointer;" onmouseover=this.style.background=""; onmouseout=this.style.background=""; onClick='."MakeDate('".$_openSIS['PrepareDate']."',this);".' /></span></div>';
	}
	}
...
{	global $_openSIS;
	...
	if($options['M'])
	{
		$return .= "<div style=white-space:nowrap ><SELECT style=width:90px; NAME=month".$title." id=monthSelect".$selectid." SIZE=1 $extraM>";
		//  -------------------------------------------------------------------------- //
	...
	if($options['D'])
	{
		$return .="<SELECT NAME=day".$title." id=daySelect".$selectid." SIZE=1 $extraD>";
		if($allow_na)
		{
			if($day=='00'){$return .= "<OPTION value=\"\" SELECTED>"._('N/A')."";}else{$return .= "<OPTION value=\"\">"._('N/A')."";}
		}
	...
	if($options['Y'])
	{
		...
		$return .="<SELECT NAME=year".$title." id=yearSelect".$selectid." SIZE=1 $extraY>";
		if($allow_na)
		{
			if($year=='00'){$return .= "<OPTION value=\"\" SELECTED>"._('N/A')."";}else{$return .= "<OPTION value=\"\">"._('N/A')."";}
		}
...
{	global $_openSIS;
	...
	if($options['Y'])
	{
		...
		if($allow_na)
		{
			if($year=='00'){$return .= "<OPTION value=\"\" SELECTED>N/A";}else{$return .= "<OPTION value=\"\">"._('N/A')."";}
		}
