...
#############################################################################################
		...
		{
			$extra['columns_after'] = array('PHONE'=>''._('Phone').'','GENDER'=>''._('Gender').'','ETHNICITY'=>''._('Ethnicity').'','ADDRESS'=>''._('Mailing Address').'','CITY'=>''._('City').'','STATE'=>''._('State').'','ZIPCODE'=>''._('Zipcode').'') + $extra['columns_after'];
...
//			else
                 ...
                 //$sql .= " AND ssm.SYEAR='".UserSyear()."' AND ('".date('Y-m-d',strtotime($extra['DATE']))."'>=ssm.START_DATE AND ('".date('Y-m-d',strtotime($extra['DATE']))."'<=ssm.END_DATE OR ssm.END_DATE IS NULL)) ";
                                                        if($_REQUEST['address_group'])
                                                            $extra['columns_after']['CHILD'] = ''._('Parent').'';
...
//				if(User('SCHOOLS'))
				...
				$functions['LIST_SCHOOL_ID'] = ''._('GetSchool').'';
			}
...
//                       if(User('SCHOOLS'))
		...
		case 'teacher':
			...
			else
			{
				...
				if($_REQUEST['include_inactive']=='Y')
				{
					$sql .= ','.db_case(array('(ssm.START_DATE IS NOT NULL AND (\''.$extra['DATE'].'\'<=ssm.END_DATE OR ssm.END_DATE IS NULL))','true',"'<FONT color=green>"._('Active')."</FONT>'","'<FONT color=red>Inactive</FONT>'")).' AS ACTIVE';
					$sql .= ','.db_case(array('(ssm.START_DATE IS NOT NULL AND (\''.$extra['DATE'].'\'<=ss.END_DATE OR ss.END_DATE IS NULL))','true',"'<FONT color=green>"._('Active')."</FONT>'","'<FONT color=red>Inactive</FONT>'")).' AS ACTIVE_SCHEDULE';
				}
			}
...
{	global $_openSIS;
	...
	if($_REQUEST['stuid'])
	{
		...
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Student ID:').' </b></font>'.$_REQUEST['stuid'].'<BR>';
	}
         if($_REQUEST['altid'])
	{
	...
	if($_REQUEST['last'])
	{
		...
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Last Name starts with:').' </b></font>'.trim($_REQUEST['last']).'<BR>';
	}
	if($_REQUEST['first'])
	{
		$sql .= ' AND LOWER(s.FIRST_NAME) LIKE \''.str_replace("'","\'",strtolower(trim($_REQUEST['first']))).'%\' ';
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('First Name starts with:').' </b></font>'.trim($_REQUEST['first']).'<BR>';
	}
	if($_REQUEST['grade'])
	{
		$sql .= ' AND ssm.GRADE_ID IN(SELECT id FROM school_gradelevels WHERE title= \''.str_replace("'","\'",$_REQUEST[grade]).'\')';
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Grade:').' </b></font>'.$_REQUEST['grade'].'<BR>';
	}
	if($_REQUEST['addr'])
	{
		...
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Address contains:').' </b></font>'.trim($_REQUEST['addr']).'<BR>';
	}
	if($_REQUEST['preferred_hospital'])
	{
		$sql .= ' AND LOWER(s.PREFERRED_HOSPITAL) LIKE \''.str_replace("'","\'",strtolower($_REQUEST['preferred_hospital'])).'%\' ';
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Preferred Medical Facility starts with:').' </b></font>'.$_REQUEST['preferred_hospital'].'<BR>';
	}
	if($_REQUEST['mp_comment'])
	{
		$sql .= ' AND LOWER(smc.COMMENT) LIKE \''.str_replace("'","\'",strtolower($_REQUEST['mp_comment'])).'%\' AND s.STUDENT_ID=smc.STUDENT_ID ';
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Comments starts with:').' </b></font>'.$_REQUEST['mp_comment'].'<BR>';
	}
	if($_REQUEST['goal_title'])
	{
		...
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Goal Title starts with:').' </b></font>'.$_REQUEST['goal_title'].'<BR>';
	}
		if($_REQUEST['goal_description'])
	{
		...
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Goal Description starts with:').' </b></font>'.$_REQUEST['goal_description'].'<BR>';
	}
		if($_REQUEST['progress_name'])
	{
		...
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Progress Period Name starts with:').' </b></font>'.$_REQUEST['progress_name'].'<BR>';
	}
	if($_REQUEST['progress_description'])
	{
		...
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Progress Assessment starts with:').' </b></font>'.$_REQUEST['progress_description'].'<BR>';
	}
	if($_REQUEST['doctors_note_comments'])
	{
		...
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Doctor\'s Note starts with:').' </b></font>'.$_REQUEST['doctors_note_comments'].'<BR>';
	}
	if($_REQUEST['type'])
	{
		$sql .= ' AND LOWER(sm.TYPE) LIKE \''.str_replace("'","\'",strtolower($_REQUEST['type'])).'%\' AND s.STUDENT_ID=sm.STUDENT_ID ';
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Type starts with:').' </b></font>'.$_REQUEST['type'].'<BR>';
	}
	if($_REQUEST['imm_comments'])
	{
		...
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Comments starts with:').' </b></font>'.$_REQUEST['imm_comments'].'<BR>';
	}
...
$imm_date=$_REQUEST['imm_year'].'-'.$_REQUEST['imm_month'].'-'.$_REQUEST['imm_day'];
		...
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Immunization Date:').' </b></font>'.$imm_date.'<BR>';
...
$med_date=$_REQUEST['med_year'].'-'.$_REQUEST['med_month'].'-'.$_REQUEST['med_day'];
		...
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Medical Date').' </b></font>'.$med_date.'<BR>';
...
$ma_date=$_REQUEST['ma_year'].'-'.$_REQUEST['ma_month'].'-'.$_REQUEST['ma_day'];
		...
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Medical Alert Date: ').'</b></font>'.$ma_date.'<BR>';
...
$nv_date=$_REQUEST['nv_year'].'-'.$_REQUEST['nv_month'].'-'.$_REQUEST['nv_day'];
		...
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Nurse Visit Date:').' </b></font>'.$nv_date.'<BR>';
	...
	if($_REQUEST['med_alrt_title'])
	{
		...
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Alert starts with:').' </b></font>'.$_REQUEST['med_alrt_title'].'<BR>';
	}
	if($_REQUEST['reason'])
	{
		$sql .= ' AND LOWER(smv.REASON) LIKE \''.str_replace("'","\'",strtolower($_REQUEST['reason'])).'%\' AND s.STUDENT_ID=smv.STUDENT_ID ';
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Reason starts with:').'  </b></font>'.$_REQUEST['reason'].'<BR>';
	}
	if($_REQUEST['result'])
	{
		$sql .= ' AND LOWER(smv.RESULT) LIKE \''.str_replace("'","\'",strtolower($_REQUEST['result'])).'%\' AND s.STUDENT_ID=smv.STUDENT_ID ';
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Result starts with:').'  </b></font>'.$_REQUEST['result'].'<BR>';
	}
	if($_REQUEST['med_vist_comments'])
	{
		...
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Nurse Visit Comments starts with:').' </b></font>'.$_REQUEST['med_vist_comments'].'<BR>';
	}
	...
	$date_from=$_REQUEST['month_from_birthdate'].'-'.$_REQUEST['day_from_birthdate'];
		...
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Birthday Starts from').' '.$date_from.''._(' to').' '.$date_to.'</b></font>';
	}	
	// test cases start
	...
	switch(User('PROFILE'))
	{
		case 'admin':
			...
			else
			{
				...
				if($_REQUEST['include_inactive']=='Y')
				$sql .= ','.db_case(array('(ssm.SYEAR=\''.UserSyear().'\' AND ( (ssm.START_DATE IS NOT NULL AND \''.date('Y-m-d',strtotime($extra['DATE'])).'\'>=ssm.START_DATE) AND(\''.date('Y-m-d',strtotime($extra['DATE'])).'\'<=ssm.END_DATE OR ssm.END_DATE IS NULL)))','true',"'<FONT color=green>"._('Active')."</FONT>'","'<FONT color=red>Inactive</FONT>'")).' AS ACTIVE ';
                                $_SESSION['new_sql'] .= ','.db_case(array('(ssm.SYEAR=\''.UserSyear().'\' AND ( (ssm.START_DATE IS NOT NULL AND \''.date('Y-m-d',strtotime($extra['DATE'])).'\'>=ssm.START_DATE) AND(\''.date('Y-m-d',strtotime($extra['DATE'])).'\'<=ssm.END_DATE OR ssm.END_DATE IS NULL)))','true',"'<FONT color=green>"._('Active')."</FONT>'","'<FONT color=red>"._('Inactive')."</FONT>'")).' AS ACTIVE ';
			}
		...
		case 'teacher':
			...
			else
			{
                                ...
                                if($_REQUEST['include_inactive']=='Y')
				{
					...
					$sql .= ','.db_case(array('(ssm.START_DATE IS NOT NULL AND (\''.$extra['DATE'].'\'<=ss.END_DATE OR ss.END_DATE IS NULL))','true',"'<FONT color=green>"._('Active')."</FONT>'","'<FONT color=red>"._('Inactive')."</FONT>'")).' AS ACTIVE_SCHEDULE';
                                        $_SESSION['new_sql'] .= ','.db_case(array('(ssm.START_DATE IS NOT NULL AND (\''.$extra['DATE'].'\'<=ssm.END_DATE OR ssm.END_DATE IS NULL))','true',"'<FONT color=green>"._('Active')."</FONT>'","'<FONT color=red>"._('Inactive')."</FONT>'")).' AS ACTIVE';
					$_SESSION['new_sql'] .= ','.db_case(array('(ssm.START_DATE IS NOT NULL AND (\''.$extra['DATE'].'\'<=ss.END_DATE OR ss.END_DATE IS NULL))','true',"'<FONT color=green>"._('Active')."</FONT>'","'<FONT color=red>"._('Inactive')."</FONT>'")).' AS ACTIVE_SCHEDULE';
...
{	global $_openSIS;
	...
	if($_REQUEST['grade'])
	{
		...
		if(!$extra['NoSearchTerms'])
			$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Grade:').' </b></font>'.GetGrade($_REQUEST['grade']).'<BR>';
	}
	if($_REQUEST['addr'])
	{
		$sql .= ' AND (LOWER(a.ADDRESS) LIKE \'%'.str_replace("'","\'",strtolower(trim($_REQUEST['addr']))).'%\' OR LOWER(a.CITY) LIKE \''.str_replace("'","\'",strtolower(trim($_REQUEST['addr']))).'%\' OR LOWER(a.STATE)=\''.str_replace("'","\'",strtolower(trim($_REQUEST['addr']))).'\' OR ZIPCODE LIKE \''.trim(str_replace("'","\'",$_REQUEST['addr'])).'%\')';
...
function _make_Parents($value,$column)
{
    ...
    $parents_RET=DBGet(DBQuery('SELECT pt.STAFF_ID AS STAFF_ID,CONCAT(pt.LAST_NAME,\' \',pt.FIRST_NAME) AS PARENT FROM students_join_users sju,staff pt, staff_school_relationship ssr  WHERE pt.staff_id=ssr.staff_id AND sju.staff_id=pt.staff_id AND sju.student_id=\''.$value.'\' AND ssr.syear=\''.UserSyear().'\''));
