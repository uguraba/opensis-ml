	...
	if(User('PROFILE')=='admin' || User('PROFILE')=='teacher' || $allow_widget)
	{
		switch($item)
		{
			case 'course':
				...
				if($_REQUEST['w_course_period_id'])
				{
					if($_REQUEST['w_course_period_id_which']=='course')
					{
						...
						$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Course').': </b></font>'.$course[1]['COURSE_TITLE'].'<BR>';
					}
					else
					{
						$extra['FROM'] .= ',schedule w_ss';
						...
						$course = DBGet(DBQuery('SELECT c.TITLE AS COURSE_TITLE,cp.TITLE,cp.COURSE_ID FROM course_periods cp,courses c WHERE c.COURSE_ID=cp.COURSE_ID AND cp.COURSE_PERIOD_ID=\''.$_REQUEST['w_course_period_id'].'\''));
						$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Course Period:').' </b></font>'.$course[1]['COURSE_TITLE'].': '.$course[1]['TITLE'].'<BR>';
					}
				}
			#	$extra['search'] .= "<TR><TD align=right width=120>Course</TD><TD><DIV id=course_div></DIV> <A HREF=# onclick='window.open(\"Modules.php?modname=misc/ChooseCourse.php\",\"\",\"scrollbars=yes,resizable=yes,width=800,height=400\");'><SMALL>Choose</SMALL></A></TD></TR>";
				$extra['search'] .= "<TR><TD align=right width=120>"._('Course')."</TD><TD><DIV id=course_div></DIV> <A HREF=# onclick='window.open(\"for_window.php?modname=misc/ChooseCourse.php\",\"\",\"scrollbars=yes,resizable=yes,width=800,height=400\");'><SMALL>"._('Choose')."</SMALL></A></TD></TR>";
				}
			break;
			...
			case 'request':
				...
				if($_REQUEST['request_course_id'])// && $_REQUEST['request_course_weight'])
				{
					...
					if(!$_REQUEST['not_request_course'])
					{
						...
						$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Request:').' </b></font>'.$course[1]['TITLE'].'<BR>';
					...
					else
					{
                        ...
                        //."' AND sr.COURSE_WEIGHT='".$_REQUEST['request_course_weight']."') ";
						$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Missing Request:').' </b></font>'.$course[1]['TITLE'].'<BR>';
                        //.' - '.$_REQUEST['request_course_weight'].'<BR>';
					}
				}
		#		$extra['search'] .= "<TR><TD align=right width=120>Request</TD><TD><DIV id=request_div></DIV> <A HREF=# onclick='window.open(\"Modules.php?modname=misc/ChooseRequest.php\",\"\",\"scrollbars=yes,resizable=yes,width=800,height=400\");'><SMALL>Choose</SMALL></A></TD></TR>";
				$extra['search'] .= "<TR><TD align=right width=120>"._('Request')."</TD><TD><DIV id=request_div></DIV> <A HREF=# onclick='window.open(\"for_window.php?modname=misc/ChooseRequest.php\",\"\",\"scrollbars=yes,resizable=yes,width=800,height=400\");'><SMALL>"._('Choose')."</SMALL></A></TD></TR>";
				}
			break;
			...
			case 'absences':
				if(is_numeric($_REQUEST['absences_low']) && is_numeric($_REQUEST['absences_high']))
				{
					...
					$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Days Absent').' '.$term.' between: </b></font>'.$_REQUEST['absences_low'].' &amp; '.$_REQUEST['absences_high'].'<BR>';
				}
				$extra['search'] .= "<TR><TD align=right width=120>"._('Days Absent')."<BR><INPUT type=radio name=absences_term value=FY checked>YTD<INPUT type=radio name=absences_term value=SEM>".GetMP(GetParentMP('SEM',UserMP()),'SHORT_NAME')."<INPUT type=radio name=absences_term value=QTR>".GetMP(UserMP(),'SHORT_NAME')."</TD><TD>"._('Between')." <INPUT type=text name=absences_low size=3 class=cell_small maxlength=5> &amp; <INPUT type=text name=absences_high size=3 maxlength=5 class=cell_small></TD></TR>";
			break;
			...
			case 'gpa':
				if(is_numeric($_REQUEST['gpa_low']) && is_numeric($_REQUEST['gpa_high']))
				{
					...
					$_openSIS['SearchTerms'] .= '<font color=gray><b>'.(($_REQUEST['gpa_weighted']=='Y')?'Weighted ':'')._('GPA between:').' </b></font>'.$_REQUEST['gpa_low'].' &amp; '.$_REQUEST['gpa_high'].'<BR>';
				}
                                ...
                                if(is_numeric($_REQUEST['cgpa_low']) && is_numeric($_REQUEST['cgpa_high']))
				{
					...
					{
						$extra['FROM'] .= ',student_gpa_running sgr';
						$extra['WHERE'] .= ' AND sgr.STUDENT_ID=s.STUDENT_ID ';
					}
					$extra['WHERE'] .= ' AND sgr.CGPA BETWEEN '.$_REQUEST[cgpa_low].' AND '.$_REQUEST[cgpa_high].' ';
					$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('CGPA between:').' </b></font>'.$_REQUEST['cgpa_low'].' &amp; '.$_REQUEST['cgpa_high'].'<BR>';
				}
                                ...
                                {
				$extra['search'] .= "<TR><TD align=right width=120><INPUT type=checkbox name=list_gpa value=Y>"._('Marking Period')._(' ')._('GPA')."</TD></TR>
                                                    <TR><TD align=right width=120><INPUT type=radio name=gpa_term value=".GetParentMP('SEM',UserMP()).">".GetMP(GetParentMP('SEM',UserMP()),'SHORT_NAME')."
                                                                                                                ...
                                                                                                                <TD>"._('Between')."<INPUT type=text name=gpa_low class=cell_small size=3 maxlength=5> &amp; <INPUT type=text name=gpa_high size=3 maxlength=5 class=cell_small></TD></TR>";
                               }
                                if($qrtrs_query[1]['QUARTER']<=1)
                                {
				$extra['search'] .= "<TR><TD align=right width=120><INPUT type=checkbox name=list_gpa value=Y>"._('Marking Period')._(' ')._('GPA')."</TD></TR>
                                ...
                                $extra['search'] .= "<TR><TD align=right width=120><INPUT type=checkbox name=cgpa value=Y>"._('CGPA')."</TD></TR>
                                                    <TR><TD align=right width=120></td><td>"._('Between')."<INPUT type=text name=cgpa_low class=cell_small size=3 maxlength=5> &amp; <INPUT type=text name=cgpa_high size=3 maxlength=5 class=cell_small></TD></TR>";
                                break;
			...
			case 'class_rank':
				if(is_numeric($_REQUEST['class_rank_low']) && is_numeric($_REQUEST['class_rank_high']))
				{
					...
					$_openSIS['SearchTerms'] .= '<font color=gray><b>Class Rank between: </b></font>'.$_REQUEST['class_rank_low'].' &amp; '.$_REQUEST['class_rank_high'].'<BR>';
                                ...
                                if($qrtrs_query[1]['QUARTER']>1)
                                {
				$extra['search'] .= "<TR><TD align=right width=120>"._('Class Rank')."<BR><INPUT type=radio name=class_rank_term value=CUM checked>"._('Cumulative')."<INPUT type=radio name=class_rank_term value=".GetParentMP('SEM',UserMP()).">".GetMP(GetParentMP('SEM',UserMP()),'SHORT_NAME')."<INPUT type=radio name=class_rank_term value=".UserMP().">".GetMP(UserMP(),'SHORT_NAME')."";
                                }
                                if($qrtrs_query[1]['QUARTER']<=1)
                                {
                                $extra['search'] .= "<TR><TD align=right width=120>"._('Class Rank')."<BR><INPUT type=radio name=class_rank_term value=CUM checked>"._('Cumulative').""."<INPUT type=radio name=class_rank_term value=".UserMP().">".GetMP(UserMP(),'SHORT_NAME')."";
                                }
				...
				$extra['search'] .= "</TD><TD>"._('Between')."<INPUT type=text name=class_rank_low size=3 maxlength=5 class=cell_small> &amp; <INPUT type=text name=class_rank_high size=3 maxlength=5 class=cell_small></TD></TR>";
			break;

			case 'letter_grade':
				if(count($_REQUEST['letter_grade']))
				{
					$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('With').''.($_REQUEST['letter_grade_exclude']=='Y'?'out':'').''._(' Report Card Grade:').' </b></font>';
					...
					$_openSIS['SearchTerms'] .= '<BR>';
                                ...
                                if($qrtrs_query[1]['QUARTER']>1)
                                {
				$extra['search'] .= "<TR><TD align=right width=120>"._('Letter Grade')."<BR><INPUT type=checkbox name=letter_grade_exclude value=Y>"._('Did not receive')."<BR><INPUT type=radio name=letter_grade_term value=".GetParentMP('SEM',UserMP()).">".GetMP(GetParentMP('SEM',UserMP()),'SHORT_NAME')."<INPUT type=radio name=letter_grade_term value=".UserMP().">".GetMP(UserMP(),'SHORT_NAME')."";
                                }
                                if($qrtrs_query[1]['QUARTER']<=1)
                                {
                                    $extra['search'] .= "<TR><TD align=right width=120>"._('Letter Grade')."<BR><INPUT type=checkbox name=letter_grade_exclude value=Y>"._('Did not receive')."<BR>"."<INPUT type=radio name=letter_grade_term value=".UserMP().">".GetMP(UserMP(),'SHORT_NAME')."";
			...
			case 'eligibility':
				if($_REQUEST['ineligible']=='Y')
				{
					...
					$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Eligibility:').' </b></font>Ineligible<BR>';
				}
			...
			case 'activity':
				if($_REQUEST['activity_id'])
				{
					...
					$_openSIS['SearchTerms'] .= '<font color=gray><b>'._('Activity:').' </b></font>'.$activity[1]['TITLE'].'<BR>';
				}
				if($_REQUEST['search_modfunc']=='search_fnc' || !$_REQUEST['search_modfunc'])
...
//					$extra['FROM'] = ' LEFT OUTER JOIN students_join_address sam ON (sam.STUDENT_ID=ssm.STUDENT_ID AND sam.MAILING=\'Y\')'.$extra['FROM'];
                                        $extra['FROM'] = ' LEFT OUTER JOIN students_join_address sam ON (sam.STUDENT_ID=ssm.STUDENT_ID)'.$extra['FROM'];
			...
			case 'balance':
				...
				$extra['search'] .= "<TR><TD align=right width=120>"._('Student Billing Balance')."<BR></TD><TD>Between<INPUT type=text name=balance_low size=5 maxlength=10 class=cell_small> &amp; <INPUT type=text name=balance_high size=5 maxlength=10 class=cell_small></TD></TR>";
			break;
                        ############################ ##########################################################
                                                    case 'parents':

                                                    $extra['search'] .= "<TR><TD align=right width=120>"._('Show Parents & Contacts')."<BR></TD><TD><INPUT type=radio name=show value=P></TD></TR>";
                                                    break;
                                                ############################  ##########################################################
                                                    case 'staff':

                                                    $extra['search'] .= "<TR><TD align=right width=120>"._('Show Staff')."<BR></TD><TD><INPUT type=radio name=show value=S></TD></TR>";
                                                    break;
