...
//		}
		if($_REQUEST['_dis_user']!='Y')
			$sql .= ' AND (s.IS_DISABLE<>\'Y\' OR  s.IS_DISABLE IS NULL) AND (ssr.END_DATE>=\''._(date('Y-m-d')).'\' OR ssr.END_DATE=\'0000-00-00\' )';
		if($_REQUEST['username'])
			$sql .= 'AND UPPER(s.USERNAME) LIKE \''._(str_replace("'","\'",strtoupper($_REQUEST['username']))).'%\' ';
		if($_REQUEST['last'])
			$sql .= 'AND UPPER(s.LAST_NAME) LIKE \''._(str_replace("'","\'",strtoupper($_REQUEST['last']))).'%\' ';
		if($_REQUEST['first'])
			$sql .= 'AND UPPER(s.FIRST_NAME) LIKE \''._(str_replace("'","\'",strtoupper($_REQUEST['first']))).'%\' ';
		if($_REQUEST['profile'])
                {
...
{	global $THIS_RET,$profiles_RET;
	...
	elseif($value=='admin')
		$return = ''._('Administrator w/Custom').'';
	elseif($value=='teacher')
		$return = ''._('Teacher w/Custom').'';
	elseif($value=='parent')
		$return = ''._('Parent w/Custom').'';
	elseif($value=='none')
		$return = ''._('No Access').'';
	else $return = $value;
